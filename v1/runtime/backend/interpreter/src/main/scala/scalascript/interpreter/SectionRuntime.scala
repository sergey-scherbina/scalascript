...
private[interpreter] object SectionRuntime:
  ...
  def runImport(imp: Content.Import, interp: Interpreter): Unit =
    ...
    val reboundChildCtx: Map[String, Value] = rawChildCtx.map { case (name, value) =>
    ...
    val childCtx = bindModuleFunctions(reboundChildCtx, moduleTopLevelDefNames(childModule))
