...
| JavaScript/TypeScript lexical direct control (T1 second repair implemented; rereview pending) | Zero-production-dependency `@scalascript/control-direct` package in [`v2/host/js/control-direct`](v2/host/js/control-direct): a TypeScript 5.9 consumer-owned compiler transform plus real packed `ssc-control-tsc` bin, lowering closed synchronous `direct.reset` blocks and top-level `const`/`let` shift bindings to the one explicit ABI. Clean emit removes the build-time marker import and preserves authored declarations; checker-resolved runtime-value ownership rejects suffix capture through shorthand, runtime marker shorthand/exports, selected-file direct eval (including import-only erasure), surviving marker values, async/generator/cleanup/loop/branch/callback/class, and arbitrary marker frames while erased type-only exports remain accepted. Package-local differentials cover vectors 18/22/23/24 without claiming the pending generated-JS lane. See the [bounded contract](specs/javascript-typescript-control-direct.md) |
