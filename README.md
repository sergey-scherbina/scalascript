...
| [Markdown Content Introspection](specs/markdown-content-introspection.md) | Markdown-to-frontend content layer: Markdown/YAML/fenced language blocks lower to `std/ui` through `contentView(contentDocument())`; broader `std/content` metadata lookup APIs remain planned |
...
| `runtime/std/ui` script toolkit | Declarative widget DSL from a `.ssc` file — `vstack/hstack`, `textField`, `checkbox`, `signalButton`, `actionButton`, `badge`, `spinner`, `card`, `modal`, `table`, `router` + `hashRouter`, `dataTable`, `fetchAction`, and Markdown-driven `contentView(contentDocument())`; `lower(tree, theme)` + `serve(view, port)` pattern; `frontend: react` front-matter |
...
| [content-introspection.ssc](examples/content-introspection.ssc) | Markdown-to-frontend authoring: Markdown body content, YAML data, section ids, `contentDocument()`, and `contentView(...)` lowering |
