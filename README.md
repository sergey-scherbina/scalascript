...
| Fetch primitives | `fetchUrlSignal` — live GET binding (v2: real fetch + headers); `fetchAction/fetchActionClear` — POST/PUT/DELETE on button click with optional `headers: Signal[String]` for bearer tokens; `fetchTableView` with headers; `incSignal` — manual refresh; `emptyHeaders` sentinel |
