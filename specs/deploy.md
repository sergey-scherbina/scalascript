...
v1.52 fills that gap **at the design level**. It produces `specs/deploy.md` (this file) and planning-file updates. No code lands in v1.52; implementation follows as v1.52.1–v1.52.7 after sign-off.
...
Reuses the resolver chain from `specs/config-system.md` and `backend/config/src/main/scala/scalascript/config/SubstitutionEngine.scala`. Available in all `deploy:`, `groups:`, and `environments:` values:
...
Error format mirrors `specs/algebraic-effects.md §9` — each error has a type tag, source location in the manifest, and a "fix:" hint.
