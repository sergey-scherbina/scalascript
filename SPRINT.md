...
- [ ] **tkv2-tri-state** - add the toolkit-v2 P2 loading/empty/error helper
      for fetched views as a pure `.ssc` module, not a new intrinsic. Spec:
      `specs/tkv2-tri-state.md`. Plan: implement `std.ui.state` with
      `LoadState`, `loadState`, `triState`, and `triStateText`, composing the
      existing `showWhen`, `eqSignal`, `computedSignal`, `signalText_`, and
      typography helpers. Add `tests/conformance/tkv2-tri-state.ssc`
      (INT==JS) covering loading, error, empty, ready, and reactive error-text
      updates; add `examples/std-ui/tri-state-demo.ssc`; update README and
      docs/user-guide. Rejected scope: no fetch runtime changes, no new
      `TkNode`, no backend intrinsic. Done when `installBin`,
      `tests/conformance/run.sh --only 'tkv2-tri-state' --no-memo`, and
      `git diff --check` pass.
