...
- [ ] The bridge-generated CoreIR for `bench/corpus/pattern-match-heavy.ssc`
      is recorded or summarized before implementation.
- [ ] At least one concrete execution-lane blocker is identified from CoreIR,
      generated source/bytecode inspection, or a focused profile; speculative
      hand paths without measured evidence are rejected.
- [ ] If a narrow fix lands, it preserves generic fallback semantics and adds
      focused regression coverage for the recognized shape.
- [ ] The production gate is only marked improved/closed when final public bench
...
Pending. Fill after measurement and implementation with exact SHAs, before/after
numbers, inspected blocker, rejected alternatives, and gates.
