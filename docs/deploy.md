...
Yet two explicit future-work markers exist: `docs/cluster-management.md:54-56` says *"what you'd build on top of a Kubernetes deployment of ScalaScript, not how Kubernetes itself deploys ScalaScript"*; `docs/electron-jvm-rest-backend.md:699` calls deploy *"a deployment-time concern that warrants its own milestone."*
...
v1.52 fills that gap **at the design level**. It produces `docs/deploy.md` (this file) and planning-file updates. No code lands in v1.52; implementation follows as v1.52.1–v1.52.7 after sign-off.
...
Reuses the resolver chain from `docs/config-system.md` and `backend/config/src/main/scala/scalascript/config/SubstitutionEngine.scala`. Available in all `deploy:`, `groups:`, and `environments:` values:
...
Error format mirrors `docs/algebraic-effects.md §9` — each error has a type tag, source location in the manifest, and a "fix:" hint.
