...
class EncryptedLocalVaultTest extends AnyFunSuite:
  ...
  test("create: vault file is written to disk") {
    val path = tmpPath()
    Await.result(EncryptedLocalVault.create(path, TestMnemonic, TestPassword,
  ...
  test("create: vault starts unlocked after creation") {
    val path  = tmpPath()
    val vault = Await.result(EncryptedLocalVault.create(path, TestMnemonic, TestPassword,
      kdfM = 256, kdfT = 1, kdfP = 1), 30.seconds)
  ...
  test("load + unlock with correct password") {
    ...
    Await.result(EncryptedLocalVault.create(path, TestMnemonic, TestPassword,
      kdfM = 256, kdfT = 1, kdfP = 1), 30.seconds)
    val v2 = Await.result(EncryptedLocalVault.load(path), 5.seconds)
    assert(v2.isLocked)
  ...
  test("key derivation is identical after load+unlock") {
    ...
    val path2  = tmpPath()
    val vault2 = Await.result(EncryptedLocalVault.create(path2, TestMnemonic, "differentpw",
      kdfM = 256, kdfT = 1, kdfP = 1), 30.seconds)
  ...
  test("generate: produces a 24-word mnemonic and unlocked vault") {
    val path         = tmpPath()
    val (vault, mne) = Await.result(EncryptedLocalVault.generate(path, TestPassword), 30.seconds)
