...
| [JVM Desktop Frontend](docs/jvm-desktop-frontend.md) | Partially implemented: `frontend-swing` backend, ServiceLoader registration, `ssc run --frontend swing`, static Swing toolkit subset emission, and local signal action bridge; backend transport remains planned |
...
| JVM desktop frontend | `ssc run --frontend swing app.ssc` launches the JDK-only Swing path through `scala-cli`; backend transport dispatch and JavaFX/Compose adapters remain planned |
...
| JVM desktop sub-backend | `frontend-swing` | Partially implemented JDK-only JVM desktop frontend. It provides SPI discovery, `ssc run --frontend swing`, Swing source emission for text/buttons/fields/toggles/stacks/spacers/dividers/scroll views, basic style hints, and local signal actions; backend transport dispatch remains planned. See [`docs/jvm-desktop-frontend.md`](docs/jvm-desktop-frontend.md). |
