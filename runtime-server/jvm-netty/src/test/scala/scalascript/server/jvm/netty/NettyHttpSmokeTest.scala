...
/** End-to-end smoke test: the Netty backend serves a plain HTTP request
 *  with the body our HttpHandler produced. */
class NettyHttpSmokeTest extends AnyFunSuite:

  test("Netty backend — plain HTTP GET returns handler's PlainResp body") {
  ...
  test("Netty backend — Reject result yields the requested status code") {
    val backend = new NettyServerBackend()
    val handler = new HttpHandler:
      override def onHttpRequest(req: Request): HttpResult =
