...
(`docs/specs/http-server-spi-plan.md`): a thin trait at the network /
...
- `backend-scalajs` registers as target `scalajs-spa` (see
  `docs/architecture.md` and `docs/specs/backend-spi.md`).
...
- **HTTP/WS server SPI** (`docs/specs/http-server-spi-plan.md`): orthogonal.
  Frontend SPI is for the SPA backend; HTTP/WS SPI is for the JVM
  backend.  An ssc app can use both — e.g., Solid frontend +
  Jetty backend.
- **v2.0 separate compilation** (`docs/specs/separate-compilation-plan.md`):
