...
object UniAlphabet:
  ...
  /** Case has NO tableless definition beyond ASCII, and §3's table does not give it one.
    ...
    */
  def isTypeNameStart(char: Char): Boolean =
    isAsciiUpper(char) || (char >= 128 && bmpUpper(char.toInt))

  private def bmpUpper(cp: Int): Boolean =
    var lo = 0
    var hi = upperRanges.length / 2 - 1
    var found = false
    while lo <= hi && !found do
      val mid = (lo + hi) / 2
      val start = upperRanges(mid * 2)
      val end = upperRanges(mid * 2 + 1)
  ...
  /** 606 BMP ranges where the character is uppercase, sorted, as `[start0, end0, start1, end1, …]`.
    ...
    */
  private val upperRanges: Vector[Int] = Vector(
