...
/** End-to-end smoke test: the Netty backend upgrades a WS connection
 *  and echoes a single text frame back to the client. */
class NettyWsSmokeTest extends AnyFunSuite:

  test("Netty backend — WS echo round-trip") {
    val backend = new NettyServerBackend()
  ...
  test("Netty backend — WS upgrade Reject produces non-101 status") {
    val backend = new NettyServerBackend()
    val handler = new HttpHandler:
      override def onHttpRequest(req: Request): HttpResult =
