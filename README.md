...
| Client IndexedDB | JS/browser/Electron code can use `IndexedDb.store[A]("store", "dbName", "keyField")` for Promise-based typed local object storage, awaited with `awaitClient(...)`; native IndexedDB is used when available, with a lightweight fallback for Node/tests. Server `ObjectStore` and sync remain planned |
