...
# `bin/lib/tower`. The fake `ssc` prints its own resolved location and its argv, so
# the assertions below can tell "the launcher ran the unpacked binary" from "the launcher ran
# something".
stage="$TMP/stage"
mkdir -p "$stage/lib" "$stage/bin/lib/tower/bin"
...
: > "$stage/bin/lib/tower/bin/fsub.ssc"
printf 'README\n' > "$stage/README.md"
...
if [[ ! -d "$PREFIX/lib/scalascript/bin/lib/tower" ]]; then
