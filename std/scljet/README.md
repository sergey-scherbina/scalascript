...
one or more columns, multi-leaf when needed, with exact INTEGER/REAL/TEXT/BLOB
ordering that reference SQLite validates and uses for query planning), and the
...
verified against reference `integrity_check`). `CREATE UNIQUE INDEX` validates
existing rows and stored unique-index metadata protects later INSERT/UPDATE
rebuilds; composite keys, distinct NULLs, numeric equivalence, and byte-equal
BLOBs match SQLite.
...
(schema row + empty root) that reference sqlite reads and `.schema` shows. `CREATE [UNIQUE] INDEX`
writes a reference-readable B-tree; unique indexes reject duplicate existing rows and conflicting
INSERT/UPDATE proposals before any new image is returned. Inner joins
...
`scljet-unique-index`, `scljet-full`).
The JVM
