...
## std-ui-button-variant — `variant` param on std/ui button primitives (2026-07-14, busi UX audit)

busi's owner-facing app found every button ("Refresh", no consequence, and "Prepare a real
payment", real financial action) renders in the same hardcoded primary-blue — no visual hierarchy
for stakes. `runtime/std/ui/lower.ssc`'s four button cases hardcode
`background:${theme.colors.primary}` with no variant selection, even though `Theme.colors`
(`runtime/std/ui/theme.ssc`) already has `secondary`/`danger`/`success`/`warning`. This is the same
kind of small, additive, JS-custom-frontend-only slice as `std-ui-select`/`selectFrom`
(`specs/std-ui-select.md`) — mirror its scope discipline.

- [ ] Spec: `specs/std-ui-button-variant.md` — interface (`variant: String = "primary"` appended
      after `disabled` on `signalButton`/`actionButton`/`signalLabelButton`/`signalActionButton`;
      accepted values `primary|secondary|danger|success|warning`, unknown → `primary`), the
      on-color-pairing decision (`ColorPalette` only has `onPrimary`, no `onSecondary`/`onDanger`/
      etc — reuse `onPrimary` uniformly, matching the EXISTING `BadgeNode`/`TagNode`/`PillNode`
      convention in `lower.ssc` which already hardcodes `color:#ffffff` on all `_statusColor`
      variants; do not add new ColorPalette tokens, per task scope), design, decisions.
- [ ] `nodes.ssc` — add `variant: String` field to the four button node case classes.
- [ ] `input.ssc` — add `variant: String = "primary"` param to the four constructors, threaded to
      the node unchanged (fallback-to-primary normalization happens in `lower.ssc`, not here — see
      spec Decisions).
- [ ] `lower.ssc` — new `_buttonColor(variant, theme)` resolver (`secondary|danger|success|warning`
      → that token, else `primary`); the four button cases use it for `background`, keep
      `theme.colors.onPrimary` for `color`.
- [ ] `examples/frontend/` — runnable example, a row of buttons in different variants side by side.
- [ ] conformance test mirroring `std-ui-jobpanel.ssc`'s TkNode-field-assertion style (View is
      opaque to `.ssc`, so a `.ssc`-level test can only assert the `variant` field threads through
      the constructor + that `lower()` doesn't throw on `[int, js]`, not inspect the CSS string
      directly) + a manual `emit-js` + grep empirical check that the emitted bundle's background
      color literal actually differs per variant (documented in the spec's Results section).
- [ ] `README.md` / `docs/user-guide.md` — capabilities-table / widget-catalog rows.
- [ ] `tests/conformance/run.sh --only '<the new test glob>,tkv2-*,std-ui-*'` green before push.

Explicitly out of scope (per task brief): busi-side wiring (which button gets which variant — a
separate follow-up, not this slice), other backends (JVM/Swing/SwiftUI-native lowering — matches
`select`'s own "zero new backend code" precedent since this reuses `element()`), new ColorPalette
tokens, touching `checkbox`/`textField`.
