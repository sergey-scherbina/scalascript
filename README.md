...
| [Markdown Content Introspection](specs/markdown-content-introspection.md) | Planned Markdown-to-frontend content layer: Markdown/YAML/fenced language blocks lower to `std/ui` without hand-written markup first, then expose the same `DocumentContent` snapshot through `std/content` metadata APIs |
...
| Markdown frontend from content | `contentView(contentDocument())` is the first planned target: prose/list/link/image Markdown nodes, YAML/front-matter, and fenced embedded language blocks lower to the existing frontend toolkit without hand-written markup generation. The same `DocumentContent` snapshot later powers `contentCurrentSection()`, `contentSection(id)`, `contentBlock(id)`, `contentData(id)`, and other metadata APIs. Phase 0 landed the spec, docs, example sketch, and pending conformance fixture; compiler/runtime support remains planned |
...
| [content-introspection.ssc](examples/content-introspection.ssc) | Planned Markdown-to-frontend authoring sketch: Markdown body content, YAML data, section ids, and `contentView(...)` lowering |
