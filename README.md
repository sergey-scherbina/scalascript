...
| [Graph storage](docs/graph-storage.md) | Planned, partially implemented: `graphs:` front matter parses into AST/IR, and JVM codegen exposes a `Graph.*` facade over `backend/graph` in-memory property/RDF storage; TinkerGraph/RDF4J/database adapters remain planned |
...
| Graph storage | `backend/graph` now provides `GraphCapabilities`, `PropertyGraphBackend`, `RdfGraphBackend`, `GraphBackend`, and `GraphRuntime.inMemory()` for portable JVM property/RDF storage through typed codecs. `graphs:` front matter survives AST/IR/.sscc, and JVM codegen emits a `Graph.*` facade for declared in-memory graph stores. TinkerGraph/TinkerPop, RDF4J, and production adapters such as Neo4j, JanusGraph/TinkerPop providers, and RDF4J-compatible repositories remain planned |
