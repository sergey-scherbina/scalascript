...
/** Confirms NettyServerBackend is registered via
 *  META-INF/services/scalascript.server.spi.HttpServerSpi and that
 *  the SPI picks it up.  After S3 also exercises `start` on an
...
class NettyServerBackendDiscoveryTest extends AnyFunSuite:
  ...
  test("NettyServerBackend — start binds an ephemeral port and stop cleans up") {
