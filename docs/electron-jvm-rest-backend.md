...
> support remains a planned HTTP SPI/runtime change. Phase 3 landed: same-origin
> API fetches route to the JVM backend via `__sscBackendBaseUrl`; fetch overlay
> in generated app.js handles relative paths; `InProcessBackendTransport` response
> type coverage verified. Phase 4 landed: per-run desktop security token (UUID)
> generated by CLI, set in `SSC_DESKTOP_TOKEN` env for JVM process, injected as
> `globalThis.__sscDesktopToken` in Electron bundle, added as
> `x-scalascript-desktop-token` header to all backend fetches, validated by
> `injectDesktopTokenMiddleware` middleware in the generated JVM script. Phase 5
> evaluated: desktop build packaging deferred to a dedicated milestone — dev-mode
> is complete; packaging needs scala-cli fat-JAR support, electron-builder
> integration, and platform signing investigation.
...
### Phase 3 - Fetch Routing And Mixed Content (landed 2026-05-26)

- Route same-origin API fetches to the backend base URL via `__sscBackendBaseUrl`
  and the `_spaFetchPath` overlay already present in the JsGen core runtime.
- Preserve local frontend navigation (non-relative URLs bypass the overlay).
- `InProcessBackendTransport` extended with test coverage for `Response.text`,
  `Response.json`, explicit status codes, and non-2xx pass-through.

### Phase 4 - Security Token And CORS (landed 2026-05-26)

- Per-run UUID token generated by CLI at JVM REST dev startup.
- Token passed as `SSC_DESKTOP_TOKEN` env var to the JVM backend process.
- `injectDesktopTokenMiddleware` appends a `use { ... }` middleware block to
  the generated JVM script that reads `SSC_DESKTOP_TOKEN` and rejects requests
  missing or providing the wrong `x-scalascript-desktop-token` header.
- Token injected as `globalThis.__sscDesktopToken` into the Electron bundle;
  fetch overlay adds the header automatically on every backend request.
- CORS for distributed browser clients remains a planned open question.

### Phase 5 - Desktop Build Packaging (evaluated — deferred)

Evaluation: packaging a self-contained `desktop-jvm` artifact requires three
independent workstreams that each deserve focused attention:

1. **JVM fat JAR**: `scala-cli --power package --assembly` produces a fat JAR,
   but it needs the ScalaScript runtime and all dependencies bundled. The
   generated script format (source file + `//> using dep` directives) is
   designed for dev-mode `scala-cli run`. Switching to assembly packaging
   requires either a pre-built launcher or native-image ahead of time.
2. **Electron packaging**: `electron-builder` (already in `package.json`) can
   produce `.dmg`, `.exe` (NSIS), and `.AppImage` from the existing bundle
   directory. The tricky part is embedding and auto-starting the JVM backend as
   a side-car process from within the packaged app.
3. **Platform signing**: macOS notarization, Windows Authenticode, and Linux
   AppImage signing each require different tooling and credentials that can't
   be scripted generically.

**Decision**: defer to a dedicated milestone. The dev-mode workflow (Phase 1–4)
is complete and delivers the full development cycle. Packaging is a
deployment-time concern that warrants its own milestone after JVM fat-JAR and
Electron side-car process startup are validated separately.
