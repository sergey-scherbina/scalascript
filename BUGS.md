...
- **FIXED (2026-06-15):** added a Range `for i <- (lo until/to hi) do body` recognizer to BOTH CPS emitters (`JvmGenCpsTransform.emitCpsExpr` + `JsGenCpsCodegen.genCpsExpr`) that desugars to an index `var`/`let` + the same while-trampoline the `while` form uses, so the body's `perform`s thread through `_bind`. Covers `until` (exclusive) + `to` (inclusive) + bodies that read the loop var. Non-range / multi-generator / guarded for-do falls through to the existing (raw / `_forEach`) path unchanged. Guard: `CrossBackendPropertyTest` "effect perform in for-do loop cross-backend" (interp == JS == JVM). Still unsupported (rare): effect performed inside a `for x <- someCollection do` (non-Range generator) — would need a CPS `foreach`.
- **Repro:**
  ```scalascript
  effect Counter:
    def tick(): Int
