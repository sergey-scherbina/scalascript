...
| Typed route clients | `apiClients:` / `api-clients:` front matter preserves endpoint method/path/request/response metadata in AST and JVM codegen. JVM/Swing now generates callable in-process clients that encode case class inputs, dispatch through generated `BackendTransport`, and decode typed JSON responses. HTTP clients for Electron/browser/split modes remain planned |
