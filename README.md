...
| Rozum / agent loop (`std.agent`) | `runAgent`, `runAgentPool`, `runAgentStream`, `runAgentStreamPool`, `collectAgentStream`, `collectAgentStreamPool`, `AgentEndpoint`, `AgentEndpointPool`, `AgentTool`, `RunOptions`, `ToolResult`, `AgentEvent` — OpenAI-compatible tool-call loop for stateless rozum gateways; apps own prompts, validation, streaming callbacks, failover policy, and side effects |
