...
/** A2d E2E — show/hide counter via the emitted Solid JS.
 *  Requires `--conditions=browser` to enable Solid's reactive
 *  runtime (default `node` export is SSR-only). */
class ShowHideE2ETest extends AnyFunSuite:
  ...
  test("ShowSignal + ToggleSignal — show/hide counter via emitted Solid JS") {
    ...
    val tmp = Files.createTempDirectory("ssc-showhide-solid-")
    try
