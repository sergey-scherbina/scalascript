...
| v1-side/better-output skipped | 4 |
...
- `dsl-calc-parser.ssc` is now classified as v1-side/better-output: v2 prints
  ...
  first number.
- `distributed-streams.ssc` is now classified as v1-side/better-output: v2 runs
  all 12 documented stream sections to completion, while rollback v1 stops in
  the ETL section on missing `String.toIntOption`.
- `streams.ssc` is now classified as v1-side/better-output: v2 runs all 8
  documented stream sections to completion, while rollback v1 stops in the
  `stream { emit(...) }` section.
...
| long-running skipped | 0 |
| backend-lane skipped | 33 |
| nondeterministic-output skipped | 3 |
| total examples seen | 195 |

os-env classification notes:

- `os-env.ssc` is now excluded from strict byte-parity as host-dependent output
  by design. The example still prints real platform/CWD/env values; only the
  parity lane classification changed.
- No deterministic v2-error row reappeared; the parity command still exits
  nonzero because 8 known output mismatches remain.
- Remaining mismatches in the full default-lane gate:
  `distributed-streams.ssc`, `dsl-calc-parser.ssc`, `effects.ssc`,
  `lang-split.ssc`, `mcp-server-protected.ssc`,
  `oauth-mcp-full-stack.ssc`, `scala-js-demo.ssc`, and `streams.ssc`.
- The remaining rows are now secrets, parser/DSL, and stream-shape families.

## Previous full corpus re-measure — 2026-07-09, after async timing normalization

Built/staged from
`/Users/sergiy/work/my/scalascript-wt-v2-async-parallel-timing-parity` with:
...
Async timing normalization notes:

- `async-parallel-demo.ssc` is now output-identical: v1/v2 compare the
  deterministic result lines instead of live wall-clock milliseconds.
- No deterministic v2-error row reappeared; the parity command still exits
  nonzero because 9 known output mismatches remain.
- Remaining mismatches in the full default-lane gate:
  `distributed-streams.ssc`, `dsl-calc-parser.ssc`, `effects.ssc`,
  `lang-split.ssc`, `mcp-server-protected.ssc`,
  `oauth-mcp-full-stack.ssc`, `os-env.ssc`, `scala-js-demo.ssc`, and
  `streams.ssc`.
- The remaining rows are now secrets, OS placeholder, parser/DSL, and
  stream-shape families.

## Previous full corpus re-measure — 2026-07-09, after graph plugin display fix
