...
class SparkGenTest extends AnyFunSuite:
  ...
  test("generated header documents Spark version and run instructions") {
    ...
    // `_2.13` suffix pinned explicitly because Spark only publishes
    // `_2.13` cross-builds; Scala 3 reads them via the TASTy bridge.
    // `::` would expand to a non-existent `_3` artifact.
    assert(code.contains("""--dep "org.apache.spark:spark-core_2.13:4.0.0""""),
      "expected spark-core_2.13 dep in header")
    assert(code.contains("""--dep "org.apache.spark:spark-sql_2.13:4.0.0""""),
      "expected spark-sql_2.13 dep in header")
  }
