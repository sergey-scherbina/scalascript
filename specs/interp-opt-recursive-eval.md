...
**Prerequisite:** `specs/direct-style-eval-spec.md` (Direction C, Phase 1)
...
Direction C (specified in `specs/direct-style-eval-spec.md`) compiles the SSC function `eval(e: Expr): Long` into a JVM method that works directly on a compact data representation — either:
...
the active ASM worktree has landed, then implement `specs/direct-style-eval-spec.md`
...
- `runtime/backend/interpreter/src/main/scala/scalascript/interpreter/EvalRuntime.scala`
- See `specs/direct-style-eval-spec.md` for the full file list.
