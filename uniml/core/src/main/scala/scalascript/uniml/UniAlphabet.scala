...
object UniAlphabet:
  ...
  /** Case has NO tableless definition beyond ASCII, and §3's table does not give it one.
    ...
    */
  // DELEGATED to the one shared copy — `alphabet/src/Alphabet.scala`, a source directory this
  // build includes rather than a library it depends on. The 606 ranges lived here and were
  // hand-copied into v3's kernel for a day, with a gate sweeping the BMP to catch drift; not being
  // able to drift is better than watching for it.
  def isTypeNameStart(char: Char): Boolean = scalascript.alphabet.Alphabet.isUpperStart(char)


  /** 606 BMP ranges where the character is uppercase, sorted, as `[start0, end0, start1, end1, …]`.
    *
    * Generated from `java.lang.Character.isUpperCase` on JDK 21 (Unicode 15.0), the same way
    * `MarkdownLexer.punctRanges` is generated from `Character.getType`. `UniAlphabetSweepSpec`
    * proves the equivalence over the whole `Char` range, so a JDK whose Unicode version moves turns
    * this red rather than letting the table go quietly stale.
    *
    * Note the shape: 1,143 code points in 606 ranges — MORE fragmented than the 378 ranges that
    * cover all 48,913 non-ASCII letters, because case alternates character by character through
    * Latin Extended. The case table is the expensive one, not the letter table §3 declined.
    */
