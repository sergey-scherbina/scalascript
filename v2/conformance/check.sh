...
MV="42"
ssc run bin/ssctc-hm.ssc0 examples/hm-eff-multiop.hm > "${TMPDIR:-/tmp}/mo48.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then ssc run bin/ssct-hm-js.ssc0 examples/hm-eff-multiop.hm > "${TMPDIR:-/tmp}/mo48.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/mo48.js" 2>/dev/null | tail -1); if [ "$got" = "$MV" ]; then printf 'ok   %-26s => %s (node)\n' "handleM -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "handleM JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/ssct-hm-rust.ssc0 examples/hm-eff-multiop.hm > "${TMPDIR:-/tmp}/mo48.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/mo48.rs" -o "${TMPDIR:-/tmp}/mo48-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/mo48-bin"); else got="(rustc err)"; fi; if [ "$got" = "$MV" ]; then printf 'ok   %-26s => %s (rustc)\n' "handleM -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "handleM Rust" "$got"; fail=1; fi; fi
echo -n "ok   handleM row composition => "; printf 'effect QA { ask : Dyn -> Int , tell : String -> String } in let prog = doE { x <- ask ; u <- logE "seen" ; t <- tell "hello" ; pureE (x * 2) } in runE (runLogE (handleM "QA" prog { | ask a k => k 21 | tell a k => k "done" } (fun v => pureE v)))' > "${TMPDIR:-/tmp}/mo48compose.hm"; mc=$(ssc run bin/ssct-hm.ssc0 "${TMPDIR:-/tmp}/mo48compose.hm" | tail -1); ssc run bin/ssctc-hm.ssc0 "${TMPDIR:-/tmp}/mo48compose.hm" > "${TMPDIR:-/tmp}/mo48compose.coreir" 2>/dev/null; got=$(ssc run-ir "${TMPDIR:-/tmp}/mo48compose.coreir" | tail -1); if [ "$mc" = '"(Int, [Dyn])"' ] && [ "$got" = 'Pair(42, Cons("seen", Nil))' ]; then echo "handled QA composes with Log"; else echo "FAIL type [$mc] run [$got]"; fail=1; fi
echo -n "ok   handleM wrong-type rejected => "; printf 'effect QA { ask : Dyn -> Int , tell : String -> String } in runE (handleM "QA" (doE { x <- ask ; u <- tell "hi" ; pureE (x * 2) }) { | ask a k => k 21 | tell a k => k 99 } (fun v => pureE v))' > "${TMPDIR:-/tmp}/mo48b.hm"; mb=$(ssc run bin/ssct-hm.ssc0 "${TMPDIR:-/tmp}/mo48b.hm" | tail -1); if echo "$mb" | grep -q TypeError; then echo "tell arm k 99 (Int, not String) rejected (correct)"; else echo "FAIL [$mb]"; fail=1; fi
echo -n "ok   handleM missing-arm rejected => "; printf 'effect QA { ask : Dyn -> Int , tell : String -> String } in runE (handleM "QA" (doE { x <- ask ; u <- tell "hi" ; pureE x }) { | ask a k => k 21 } (fun v => pureE v))' > "${TMPDIR:-/tmp}/mo48missing.hm"; mm=$(ssc run bin/ssct-hm.ssc0 "${TMPDIR:-/tmp}/mo48missing.hm" | tail -1); if echo "$mm" | grep -q TypeError; then echo "missing tell arm rejected (correct)"; else echo "FAIL [$mm]"; fail=1; fi
echo -n "ok   handleM foreign-arm rejected => "; printf 'effect QA { ask : Dyn -> Int , tell : String -> String } in effect Other { ping : Dyn -> Int } in runE (handleM "QA" (doE { x <- ask ; u <- tell "hi" ; pureE x }) { | ask a k => k 21 | tell a k => k "ok" | ping a k => k 1 } (fun v => pureE v))' > "${TMPDIR:-/tmp}/mo48foreign.hm"; mf=$(ssc run bin/ssct-hm.ssc0 "${TMPDIR:-/tmp}/mo48foreign.hm" | tail -1); if echo "$mf" | grep -q TypeError; then echo "foreign ping arm rejected (correct)"; else echo "FAIL [$mf]"; fail=1; fi
echo -n "ok   handleM duplicate-arm rejected => "; printf 'effect QA { ask : Dyn -> Int , tell : String -> String } in runE (handleM "QA" (doE { x <- ask ; u <- tell "hi" ; pureE x }) { | ask a k => k 21 | ask a k => k 22 | tell a k => k "ok" } (fun v => pureE v))' > "${TMPDIR:-/tmp}/mo48dup.hm"; md=$(ssc run bin/ssct-hm.ssc0 "${TMPDIR:-/tmp}/mo48dup.hm" | tail -1); if echo "$md" | grep -q TypeError; then echo "duplicate ask arm rejected (correct)"; else echo "FAIL [$md]"; fail=1; fi
