...
**JS one-shot via `function*`.** `docs/specs/coroutines.md:236-256` already promises
...
`Out(E)` and `++` are part of the v1.60 tuple monoid (`docs/specs/tuple-monoid.md`).
...
1. **JS**: emit `function*`/`yield`/`iter.next(v)` for one-shot effect bodies
   in `JsGen.scala` (closes the gap documented in `docs/specs/coroutines.md:236-256`).
...
- JS generators are a native one-shot continuation primitive that closes a
  documented gap in `docs/specs/coroutines.md`.
- Every backend already advertises `Feature.AlgebraicEffects`.
