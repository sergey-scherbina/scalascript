...
| Rozum / agent loop (`std.agent`) | `runAgent`, `runAgentStream`, `collectAgentStream`, `AgentEndpoint`, `AgentTool`, `RunOptions`, `ToolResult`, `AgentEvent` — OpenAI-compatible tool-call loop for stateless rozum gateways; apps own prompts, validation, streaming callbacks, and side effects |
