...
object Compiler:
  ...
  /** Conservative auto-thread-Op guard: these terms may evaluate to an auto-thread
   * Op and therefore must stay on the effect-aware compiler path when used as
   * a primitive argument. The base compiler still checks the actual Value, so
  ...
  /** The compiler's half of bounded capsule handling; `Reader.MaxDepth` is the codec's
    ...
    *
    * `C.compile`, `mayProduceAutoThreadOp` and `collectRegfields` all
...
object Prims:
  ...
  private def resolveBuiltinRaw(op: String): Fn = op match
    ...
    case "__effect__" => resolve("__method__")   // alias: effect-receiver ops route through __method__
  ...
  /** COMPACT numeric arith for hot seams (the bytecode lane's Emit.arith,
   *  resolve "__arith__"). Small enough for the
  ...
  /** Dispatch `__method__(name, recv, args...)` without trampoline. */
