 ...
 *
 *  Cross-compile note: the keccak256-dependent tests
 *  (`Abi.encodeFunctionCall` / `Selector.forFunction`) need a
 *  registered `CryptoBackend`.  On JVM, `crypto-bouncycastle`
 *  auto-registers via `ServiceLoader`; on Scala.js, the JS-side
 *  `AbiCodecTest` subclass installs `crypto-noble-js` explicitly.
 *  The platform-specific concrete classes mix in that setup; the
 *  test bodies themselves live here. */
abstract class AbiCodecTestBase extends AnyFunSuite:
