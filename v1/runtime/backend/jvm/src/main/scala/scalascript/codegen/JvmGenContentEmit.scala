  ...
  private[codegen] def emitContentRuntime(document: Option[DocumentContent], includeToolkit: Boolean): String =
    ...
    (if includeToolkit then
      s"""
       ...
       |            std.ui.nodes.SignalButtonNode(signal, value, _ssc_tk_link_label(link), disabled, "primary", "md"),
       |            std.ui.nodes.SignalButtonNode(signal, value, _ssc_tk_link_label(link), true, "primary", "md"))
       |        case None =>
       |          std.ui.nodes.SignalButtonNode(signal, value, _ssc_tk_link_label(link), disabled, "primary", "md")
       |    case "signaltext" =>
       ...
       |      std.ui.nodes.SignalButtonNode(_ssc_tk_signal(env, name, "button"), value, _ssc_tk_str(_ssc_tk_field(obj, "label", "button"), "button.label"), _ssc_tk_opt_bool(obj, "disabled"), "primary", "md")
       |    case "badge" =>
