...
## build-rust-indexof-on-string — indexOf on a String lowers to .iter().position(…); String has no iter, so build-rust emits code that cannot compile while run is correct
<!-- triage: new
     reported-by: rozum
...
`indexOf` on a String is lowered as if the receiver were a list, and the emitted Rust does not
compile. `run` answers correctly, so the two lanes disagree.

REPRO (4 lines, `r1-indexof.ssc`):
...
- `ssc run`        → prints `3`
- `ssc-tools build-rust` → rc=101

```
error[E0599]: no method named `iter` found for struct `String` in the current scope
  --> src/generated/r1_indexof.rs:9:16
   |
 9 |     let i = (s.iter().position(|__e| *__e == "</head>".to_string()).map(|__i| __i as i64).unwrap_or(-1));
```

The list lowering for `indexOf` is being applied to a String receiver. Rust's own suggestion
(`chars()`) is not the fix — that would index by codepoint, not by the substring asked for; the
substring case wants `str::find`.

WHERE IT BIT: injecting a `<script>` before `</head>` in a served HTML page. Worked out in the
end via `split` + a hand-rolled join, so this is not blocking me — but the workaround is only
discoverable after rustc blames a line the author did not write.

Either lowering is fine by me. Refusing at compile time with "indexOf on a String is not
supported" would also be an improvement over emitting code that cannot build.
## build-rust-concat-list-element-with-toplevel-val — a list element concatenated with a top-level val emits String + String; build-rust fails where run is correct, and it is the origin of the left operand that decides
<!-- triage: new
     reported-by: rozum
     reported-at: 2026-08-13
     ssc-version: 61eaefc57
...
Concatenating a list element with a top-level `val` emits `String + String`, which Rust rejects.
`run` is correct, `build-rust` fails.

REPRO (4 lines, `w1.ssc`):
...
error[E0308]: mismatched types
  --> src/generated/w1.rs:10:64
   |
10 |     crate::runtime::_println((parts[(0i64) as usize].clone() + SEP));
   |                                                                ^^^ expected `&str`, found `String`
```

NARROWED — it is the ORIGIN of the left operand, not the arity and not the list:
...
So a value taken from a list is treated as an owned `String` on the left of `+` while the
right-hand top-level `val` is also owned, and `String + String` has no impl. The literal-initialised
case takes a different path and works.

WHERE IT BIT: rebuilding a string around a separator after `split`. Workaround is to keep every
concatenation to exactly two operands and avoid a top-level `val` on the right — which is a rule no
author could guess from the source.

NOTE, unrelated to the above and offered only as information: I could not re-check either report
against current `main` (04b2f12fc). A fresh detached worktree does not bootstrap — `./bin/ssc run`
prints `Could not find or load main class scalascript.cli.StandardMain` AND exits 0. Both reports
