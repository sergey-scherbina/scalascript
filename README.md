...
| [Typed route clients](docs/typed-route-clients.md) | Planned, not implemented yet: generated typed frontend clients over backend routes, using `BackendTransport` for JVM/Swing in-process first and HTTP later |
...
| Typed route clients | Generated typed frontend clients will encode case class inputs, call backend routes through `BackendTransport`, and decode typed responses. JVM/Swing in-process is the first planned target; HTTP clients for Electron/browser split modes follow |
