...
| [Markdown Content Introspection](specs/markdown-content-introspection.md) | Planned `std/content` API for reading Markdown-hosted content, YAML/front-matter, and fenced embedded language blocks as typed metadata, then lowering it through `std/ui` without hand-written markup |
...
| Markdown content introspection | `contentDocument()`, `contentCurrentSection()`, `contentSection(id)`, `contentBlock(id)`, `contentData(id)`, and `contentView(...)` will expose prose/list/link/image Markdown nodes, YAML/front-matter, and fenced embedded language blocks as typed metadata and renderable frontend input. Phase 0 landed the spec, docs, example sketch, and pending conformance fixture; compiler/runtime support remains planned |
...
| [content-introspection.ssc](examples/content-introspection.ssc) | Planned `std/content` authoring sketch: Markdown body metadata, section ids, and frontend `contentView(...)` lowering |
