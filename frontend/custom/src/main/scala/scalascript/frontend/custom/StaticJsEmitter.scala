...
private[custom] object StaticJsEmitter:
  ...
  private final class Ctx:
    ...
    def compile(view: View[?]): String | Null = view match
                ...
                case None =>
                  col.kind match
                    case ColumnKind.StatusBadge(_) | ColumnKind.Link(_) =>
                      val cellExpr = customFormattedExpr("__row", col.fieldPath, col.kind)
            ...
            if dt.actions.nonEmpty then
              ...
              dt.actions.foreach {
                ...
                case RowActionDef.RowPost(label, method, url, payload, tick, actHOpt) =>
                  ...
                  val actHeaders = actHOpt.map(h => s", headers: JSON.parse(__ssc_signals[${jsString(h.id)}].value || '{}')").getOrElse("")
                  val bodyJs     = customRowPayloadExpr("r", payload)
                  ...
                  statements += s"    $btnVar.addEventListener('click', ((r) => () => fetch($actUrlJs, {method: $actMethod, body: $bodyJs$actHeaders}).then(resp => resp.text()).then(_ => __setSignal($tickJs, __ssc_signals[$tickJs].value + 1)))(__row));"
                  statements += s"    $tdActVar.appendChild($btnVar);"
      ...
      case View.FormattedField(varName, fieldPath, kind, _) =>
        val v = freshVar()
        val expr = customFormattedExpr(varName, fieldPath, kind)
        kind match
          case ColumnKind.StatusBadge(_) | ColumnKind.Link(_) =>
    ...
    private def compileEventHandler(targetVar: String, eventName: String, handler: EventHandler): Unit =
      handler match
        ...
        case EventHandler.ItemAction(method, url, payload, tick, hOpt) =>
          if currentItemVar.nonEmpty then
            val tickJs    = jsString(tick.id)
            val bodyJs    = customRowPayloadExpr(currentItemVar, payload)
            val urlJs     = jsString(url)
            val methodJs  = jsString(method)
            val headersJs = hOpt.map(h => s", headers: JSON.parse(__ssc_signals[${jsString(h.id)}].value || '{}')").getOrElse("")
            statements += s"$targetVar.addEventListener(${jsString(eventName)}, () => " +
  ...
  /** Build the JS body expression for a RowPayload (Custom/imperative DOM style). */
  private def customRowPayloadExpr(itemVar: String, payload: RowPayload): String = payload match
  ...
  private def customFormattedExpr(varName: String, fieldPath: String, kind: ColumnKind): String =
    kind match
