...
| `ssc run --native file.ssc` | `v2-native` | Opt-in ScalaScript 2.1 path: staged self-hosted frontend → CoreIR → v2 VM, with no Scala CLI/scalac/javac process. Standard host, crypto, `std.fs`, `std.os`, typed `std.json`, and outbound HTTP/`Response` intrinsics load through the core-free native provider SPI; native HTTP server calls fail explicitly until the server-host SPI lands. `--native --bytecode` selects direct ASM; unresolved runtime dispatch/effects fail instead of printing `Stub`/`Op`; `--compat-frontend` explicitly selects the Scalameta bridge during migration. |
