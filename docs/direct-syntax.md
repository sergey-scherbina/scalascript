...
| **DS-7** | Error handling — `MonadError` or thrown exceptions? | **Monadic** — `M.fail(...)` / `M.recover(...)` for monadic failure; the inferred `M` must have a `MonadError[M, E]` instance (from `std/monaderror.ssc`).  Thrown exceptions are NOT auto-wrapped into monadic failure — avoids the two-fault-model trap (where some errors are monadic, others are JVM exceptions, and users can never remember which is which). |
...
| Auto-wrap thrown exceptions into `M.fail` | The two-fault-model trap — locked under DS-7. |
