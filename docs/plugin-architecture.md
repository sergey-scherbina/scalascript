...
META-INF/services/
  scalascript.backend.spi.Backend   ← "sscplugin.crypto.CryptoBackendPlugin\nsscplugin.crypto.CryptoInterpreterPlugin"
```
...
5. **First `.sscpkg` — UI primitives** — ship the 9 `extern def`s from the frontend toolkit spec (`docs/frontend-toolkit-spec.md §Phase 7a`) as `std-packages/ui-prim.sscpkg`.  Resumes the toolkit refactor.
