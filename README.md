...
| Typed route clients | `apiClients:` / `api-clients:` front matter preserves endpoint method/path/request/response metadata in AST and JVM/JS codegen. JVM/Swing generates callable in-process clients that encode case class inputs, dispatch through generated `BackendTransport`, and decode typed JSON responses. JS/browser codegen emits Promise-returning HTTP clients over `fetch`; `emit-spa --server-url` / client mode can route those relative calls to a JVM backend. Electron/distributed e2e and shared codecs remain planned |
