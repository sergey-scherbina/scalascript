...
class TypedRouteDistributedExampleCliTest extends AnyFunSuite:
  ...
  test("distributed typed client example renders SPA with raw JS and generated HTTP client"):
    ...
    assert(html.contains("async function _ssc_api_request(methodRaw, pathTemplate, input, requestType, responseType)"))
    assert(html.contains("const Messages = {"))
    assert(html.contains("""create(input) { return _ssc_api_request("POST", "/api/messages", input, "CreateMessage", "Message"); }"""))
    assert(html.contains("""list() { return _ssc_api_request("GET", "/api/messages", undefined, "Unit", "List[Message]"); }"""))
    assert(html.contains("""get(input) { return _ssc_api_request("GET", "/api/messages/:id", input, "Int", "Message"); }"""))
