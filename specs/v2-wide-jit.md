...
>
> **BUILT, MEASURED, AND REVERTED — the second hypothesis is refuted too.** On-demand linking
> (compile a cold callee at link time, bounded by an `inProgress` cycle guard and a depth of 3, and
> install it into the callee's site so the interpreter benefits) works exactly as designed:
> cross-unit links **956 → 3745** (839 formed on demand), units compiled 558 → 1396. And the row
> does not move: AOT 8.81 / 7.60 / 9.00 against JIT 10.1 / 10.7 / 10.8 — **1.21×, unchanged from
> 1.31× within noise** — while compile time TRIPLED, 159 → 456 ms.
>
> So the hot call was already linked before the change, and the 839 new links are in code the
> measured window never executes. The arithmetic that motivated it (12.6 ns × 500 000 ≈ the gap)
> was plausible and wrong: it assumed the calls were unlinked without checking that they were.
> **Reverted** — measurable cost, no measured benefit — and recorded here so the next person does
> not rebuild it on the same reasoning. `V2LinkBench` stays; it is what makes both refutations
> cheap.
>
> `pattern-match-heavy`'s 1.2–1.3× is therefore still UNEXPLAINED, with two suspects eliminated by
> measurement. The next probe should compare the two lanes' generated bytecode for `workload`
> directly (`javap` on the AOT class against a dumped JIT unit) rather than reason from timings —
> the shapes are supposed to be identical, and if they are not, the diff names the cause outright.
>
> **Done, and it eliminates a third suspect.** `SSC_V2_JIT_DUMP=<dir>` now writes every class BOTH
> lanes emit; `javap` on the pair says:
>
> | method | AOT | JIT unit |
> |---|---|---|
> | `area` | 185 instructions, 29 `FloatV` refs | **185 instructions, 29 `FloatV` refs** |
> | `workload` | 62 instructions, 1 `Emit.app`, **0 inline-foreach** | 38 instructions, 0 `Emit.app`, **0 inline-foreach** |
>
> `area` — the 500 000-times-per-run callee, and the row's whole point — is **identical**. So the
> emitter is not treating the two lanes differently where it matters, which retires the third
> hypothesis in a row.
>
> The surprise is the last column: **neither lane inlines the `foreach`.** The `pureDefs` machinery
> both now share does not reach it here, and the work lives in the deferred Seq/Let chain methods
> that `workload` calls rather than in `workload` itself. That is where the next comparison goes —
> the two `workload` bodies differ in size (62 vs 38) precisely because they defer differently.
>
> **Found by accident, and it is a J-9 defect:** a plain `ssc run` with the JIT armed dumps 437
> units named after FRONT functions (`walkTop`, `varNamesOf`, `argDepth`). J-9 stopped arming the
