...
| [Markdown Content Introspection](specs/markdown-content-introspection.md) | Markdown-to-frontend content layer: Markdown/YAML/fenced language blocks lower to `std/ui` through `contentToolkitNode()` or explicit `contentToolkitBlock(id)` / `contentToolkitSection(id)` selectors, including declarative `yaml @ui=toolkit` controls and explicit `contentComponent(...)` registries for `component=<name>` metadata; low-level `contentView(contentDocument())` remains available and broader `std/content` metadata lookup APIs remain planned |
