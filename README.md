...
| JavaScript/TypeScript lexical direct control (T1 third repair implemented; rereview pending) | Zero-production-dependency `@scalascript/control-direct` package in [`v2/host/js/control-direct`](v2/host/js/control-direct): a TypeScript 5.9 consumer-owned compiler transform plus real packed `ssc-control-tsc` bin, lowering closed synchronous `direct.reset` blocks and top-level `const`/`let` shift bindings to the one explicit ABI. Clean JavaScript emit removes completed marker/type-only import and export edges while declaration emit retains authored types in both verbatim modes; runtime CommonJS `import = require` is rejected while `import type = require` erases. Checker-resolved ownership also rejects suffix capture through shorthand, runtime marker shorthand/exports, selected-file direct eval, surviving values, and unmanaged async/cleanup/control frames. Package-local differentials cover vectors 18/22/23/24 without claiming the pending generated-JS lane. See the [bounded contract](specs/javascript-typescript-control-direct.md) |
