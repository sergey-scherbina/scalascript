...
3. **Slice 3 — ✓ Tier-2 (general, static depth) DONE 2026-06-22.** Arbitrary handlers (resume any way,
   not a monad `flatMap`) over a single-op `multi effect`: `renderTier2General` emits the handler as a
   non-capturing nested `fn __h(<op-args>, __k: &dyn Fn(<opRet>) -> <progRet>) -> <progRet>` (handler body
