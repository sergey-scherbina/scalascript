 ...
 *
 *  Thread-safety: callers may register, lookup, and forget topics
 *  from arbitrary threads (relevant on JVM where the WC v2 ws-driven
 *  callbacks land on JDK virtual threads).  On Scala.js the
 *  synchronisation is a no-op — JS has no concurrent execution.
 *  Backing structure is a plain `mutable.HashMap` wrapped in
 *  `synchronized` so the cross-compile stays clean (Scala.js doesn't
 *  ship `scala.collection.concurrent.TrieMap`).  Intentionally
 *  in-memory; persistence (encrypted on-disk store) is a follow-up
 *  concern and slots in by replacing the backing map. */
final class WcSessionStore:

  import WcSessionStore.SessionEntry

  private val entries = mutable.HashMap.empty[String, SessionEntry]
  ...
  def register(topic: String, symKey: Array[Byte], peerPub: Option[Array[Byte]] = None): Unit =
    val entry = SessionEntry(symKey.clone(), peerPub.map(_.clone()))
    entries.synchronized:
      entries.update(topic, entry)
