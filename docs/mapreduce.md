...
(which is `docs/cluster-management.md`).
...
down.  See `docs/cluster-management.md` (deferred future
...
| **Cluster management** | The `Cluster` handle is a thin wrapper for v1.22; the larger story (discovery, membership, leader election) lives in `docs/cluster-management.md` deferred milestone. |
...
| **Implicit cluster discovery** | v1.22 takes an explicit `Cluster.connect(nodes…)`; auto-discovery is `docs/cluster-management.md` territory |
...
[`docs/cluster-management.md`](cluster-management.md), which
