...
/** Wallet-side WalletConnect v2 connector tests against a mock relay
 *  transport. We exercise the session-lifecycle + request-routing
 *  logic; the wire crypto / actual relay ws are out of scope here. */
class WalletConnectConnectorTest extends AnyFunSuite:
  ...
  /** Quick spin-wait helper for async assertions. */
