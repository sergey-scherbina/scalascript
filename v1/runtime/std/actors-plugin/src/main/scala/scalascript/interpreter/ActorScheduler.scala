...
/** Cooperative actor scheduler — moved from interpreter core into the bundled actors plugin.
 *
 *  Instantiated once per `open(host)` call by `ActorsInterpreterPlugin`'s provider.
 *  All interpreter services are accessed through `host: ActorRuntimeHost` — no direct
 *  `Interpreter` self-type is needed.  Cluster/distributed state lives per-scheduler
 *  instance so concurrent or embedded interpreters are isolated.
 */
class ActorScheduler(private val host: ActorRuntimeHost):
  ...
  private def callCoordFn(fn: Value, args: List[Value]): Value =
    fn match
      ...
      case _: Value.FunV =>
        try Computation.run(host.actorCallValue(fn, args, Map.empty))
  ...
  private def startElection(): Unit =
    if clusterDebug then
      ...
      host.out.println(s"[cluster:dbg] $localNodeId startElection peers=${peers.mkString(",")} quorum=$quorumSize visible=${peerChannels.size + 1}")
    if localNodeId.isEmpty then
      ...
      if higher.isEmpty then
        ...
        if !hasQuorum then
          if clusterDebug then host.out.println(
  ...
  private def spawnRegisteredBehavior(name: String, arg: Value): Either[String, Value] = this.synchronized {
    ...
    if rt == null then Left("actor runtime is not running")
      ...
      if behavior == null then Left(s"behavior not found: $name")
        ...
        val childBody =
          try host.actorCallValue1(behavior, arg, Map.empty)
  ...
  private def peerLost(peerNodeId: String, urlForReconnect: String, tokenForReconnect: String): Boolean =
    ...
    if clusterDebug then
      host.out.println(s"[cluster:dbg] $localNodeId peerLost($peerNodeId) wasPresent=$wasPresent currentLeader=${currentLeader.get()} autoReelect=$autoReelect")
    ...
    if wasPresent then
      ...
      if clusterDebug then
        host.out.println(s"[cluster:dbg] $localNodeId peerLost($peerNodeId) before=$before wasLeader=$wasLeader after=${currentLeader.get()}")
  ...
  private def connectPeer(url: String, token: String): Unit =
    ...
    Thread.ofVirtual().start { () =>
      try
        if clusterDebug then host.out.println(s"[cluster:dbg] $localNodeId -> connectPeer($url) starting")
        val sess = host.actorOpenWsClient(url, hdrs, ActorWireProtocol.clientProtocols)
        sess.connect()
        val proto = sess.negotiatedProtocol
        if clusterDebug then host.out.println(s"[cluster:dbg] $localNodeId -> connectPeer($url) connected proto=$proto")
        ...
        sess.recvText() match
          case None =>
            if clusterDebug then host.out.println(s"[cluster:dbg] $localNodeId -> connectPeer($url) closed during handshake")
          ...
          case Some(firstMsg) =>
            ...
            val peerNodeId = JsonParser.parseOption(firstMsg) match
              ...
              case _ => ""
            if clusterDebug then host.out.println(s"[cluster:dbg] $localNodeId -> connectPeer($url) handshake peerNodeId='$peerNodeId'")
        ...
        if clusterDebug then
          host.out.println(s"connectNode error [$url]: ${e.getMessage}")
  ...
  def run(initial: Computation): Computation =
    val rt = new ActorRuntime
    val wasActive = host.actorIsRuntimeActive
    actorRt = rt
    host.actorSetRuntimeActive(true)
  ...
  private def handleActorOp(
    ...
    case "spawn" =>
      ...
      rt.currentId = childId
      val childBody =
        try host.actorCallValue(thunk, Nil, Map.empty)
        finally rt.currentId = savedCurrent
      rt.pending(childId) = childBody
      rt.ready.enqueue(childId)
    ...
    case "actorRefTryLocal" => args match
      case List(pid @ Value.InstanceV("Pid", fields)) =>
        ...
        Right(k(if local then Value.OptionV(pid) else Value.NoneV))
    ...
    case "actorGroupRouter" => args match
      case List(nameV, policyV) =>
        ...
        this.host.actorNativeFeatureSet(s"actorGroup.$name", group)
        Right(k(group))
    ...
    case "actorGroupTell" => args match
      case List(Value.InstanceV("ActorGroup", fields), msg) =>
        ...
        val currentFields = fields.getOrElse("name", null) match
          case Value.StringV(n) => this.host.actorNativeFeatureGet(s"actorGroup.$n") match
      ...
      case List(Value.InstanceV("ActorGroup", fields), ref) =>
        fields.getOrElse("name", null) match
          case Value.StringV(n) =>
            val currentFields = this.host.actorNativeFeatureGet(s"actorGroup.$n") match
              case Some(g: Value.InstanceV) => g.fields
              case _                        => fields
            val newMembers = currentFields.getOrElse("members", null) match
              case Value.ListV(xs) => xs :+ ref
              case _               => ref :: Nil
            this.host.actorNativeFeatureSet(s"actorGroup.$n",
    ...
    case "receive" => args match
      case List(Value.IntV(specId)) =>
        val (cases, recvEnv) = host.actorReceiveSpec(specId)
    ...
    case "startNode" =>
      ...
      val peersRoute = Value.NativeFnV("_ssc-actors.handler", args => {
        ...
        def wsRecv(): Option[String] = wsFields.get("recv").flatMap { f =>
          Computation.run(host.actorCallValue(f, Nil, Map.empty)) match
            case Value.OptionV(Value.StringV(s)) => Some(s)
            case _ => None
        }
        def wsSendText(text: String): Unit = wsFields.get("send").foreach { f =>
          Computation.run(host.actorCallValue(f, List(Value.StringV(text)), Map.empty))
        }
        ...
        def wsSendBinary(bytes: Array[Byte]): Unit = wsFields.get("sendBytes").foreach { f =>
          Computation.run(host.actorCallValue(f, List(Value.StringV(new String(bytes, "ISO-8859-1"))), Map.empty))
        }
        val sendFn: String => Unit =
        ...
        if clusterDebug then host.out.println(s"[cluster:dbg] $localNodeId <- inbound handshake recv=${firstMsgOpt.map(s => if s.length > 80 then s.take(80) + "…" else s).getOrElse("<none>")}")
        ...
        if clusterDebug then host.out.println(s"[cluster:dbg] $localNodeId <- inbound peerNodeId=${peerNodeId.getOrElse("<none>")} proto=$negotiatedProto")
      ...
      host.actorRegisterWsRoute("/_ssc-actors", peersRoute, ActorWireProtocol.serverProtocols)
      ActorClusterRoutes.registerAll(this, host)
      Right(k(Value.UnitV))
    ...
    case "globalWhereis" => args match
      case List(Value.StringV(name)) =>
        val result = Option(globalRegistry.get(name)) match
          case Some(pid) => Value.OptionV(pid)
    ...
    case "clusterOf" => args match
      case List(seedResolver @ Value.InstanceV("SeedResolver", _)) =>
        ...
        Right(k(Value.InstanceV("ClusterCapability", Map(
          ...
          "codeIdentity" -> host.actorCodeIdentity
        ))))
  ...
  private def tryDeliver(
    ...
    while !mb.isEmpty do
      val msg = mb.peek()
      host.actorMatchReceive(cases, env, msg) match
        case Some(body) =>
          mb.poll()
        ...
        case None =>
          ...
          host.out.println(s"[dead-letter] actor=$id msg=${Value.show(mb.poll())}")
          resumeBlockedSender(rt, id)
    None
