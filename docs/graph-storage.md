...
`graphs:` front matter and emits a minimal `.ssc` `Graph.*` facade over declared
in-memory stores. The backend is embedded, non-persistent, non-remote, and
supports the portable API only.
...
This facade is currently generated by the JVM backend only. Interpreter
intrinsics and non-JVM frontend/client graph stores remain planned.
