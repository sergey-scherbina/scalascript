...
/** Decoder for `NativeCompilation(IrProg, List[Option[YamlResult]], paths)`.
 *  Both the CoreIR and YAML products are already structural values when they
 *  cross this boundary; the permanent Scala seed only validates and maps them. */
private[cli] object NativeV2Structural:
  def decode(value: Value, roots: List[java.io.File]): NativeStructuralFrontend = value match
    case DataV("NativeCompilation", IndexedSeq(programValue, manifestValues, sourceValues)) =>
      ...
      NativeStructuralFrontend(
        IrDecode.program(programValue),
        manifests,
        runtimeConfig(manifests))
    case other => abiError(s"expected NativeCompilation/3, got ${show(other)}")
