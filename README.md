...
| JavaScript/TypeScript lexical direct control (T1 implemented) | Zero-production-dependency `@scalascript/control-direct` package in [`v2/host/js/control-direct`](v2/host/js/control-direct): exact-import-aware TypeScript compiler transform plus `ssc-control-tsc`, lowering closed synchronous `direct.reset` blocks and top-level `const`/`let` shift bindings to the one explicit ABI. Stable source diagnostics reject async/generator/cleanup/loop/branch/callback/class and arbitrary marker frames; package-local differentials cover vectors 18/22/23/24 without claiming the pending generated-JS lane. See the [bounded contract](specs/javascript-typescript-control-direct.md) |
...
| Host/runner profiles (in progress) | The JS/TS explicit local leaf and bounded lexical direct transform are implemented; native typed bidirectional bridges and remaining host/runner slices for [Scala/JVM](specs/scala3-bidirectional-control.md), [JS/TS](specs/javascript-typescript-bidirectional-control.md), [Rust](specs/rust-bidirectional-control.md), and [Swift](specs/swift-bidirectional-control.md) stay mandatory, measured against the [portable-VM reference runner](specs/control-interop-profile-portable-vm.md), plus the [WASM/WASI runner](specs/wasm-wasi-control-runner.md) |
