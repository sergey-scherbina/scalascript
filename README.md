...
| `ssc run --native file.ssc` | `v2-native` | Opt-in ScalaScript 2.1 path: staged self-hosted frontend → CoreIR → v2 VM, with no Scala CLI/scalac/javac process. Standard host/crypto intrinsics load through the core-free native provider SPI; `--native --bytecode` selects direct ASM; unresolved runtime dispatch/effects fail instead of printing `Stub`/`Op`; `--compat-frontend` explicitly selects the Scalameta bridge during migration. |
