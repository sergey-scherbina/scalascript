...
class TypedRouteDistributedExampleCliTest extends AnyFunSuite:
  ...
  test("distributed typed client example renders SPA with raw JS and generated HTTP client"):
    ...
    assert(html.contains("async function _ssc_api_request(methodRaw, pathTemplate, input, requestType, responseType, callHeaders, cancelToken)"))
    assert(html.contains("const Messages = {"))
    assert(html.contains("""create(input, headers, cancelToken) { return _ssc_api_request("POST", "/api/messages", input, "CreateMessage", "Message", headers, cancelToken); }"""))
    assert(html.contains("""list(headers, cancelToken) { return _ssc_api_request("GET", "/api/messages", undefined, "Unit", "List[Message]", headers, cancelToken); }"""))
    assert(html.contains("""get(input, headers, cancelToken) { return _ssc_api_request("GET", "/api/messages/:id", input, "Int", "Message", headers, cancelToken); }"""))
