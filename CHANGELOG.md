...
- **v1.46** (Phases 0–7) — Typed Route Clients: generated frontend clients over backend routes. JVM/Swing in-process + JS HTTP transports. Auth/custom header injection, per-call header overrides, retry policy, cancellation tokens. Phase 7: SSE streaming via EventSource/fetch+ReadableStream (JS) and HttpURLConnection daemon-thread reader (JVM). Remaining: WS subscriptions, pagination, Phase 5 cross-file route derivation.
