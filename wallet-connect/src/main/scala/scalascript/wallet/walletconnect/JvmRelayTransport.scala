...
/** Production WalletConnect v2 relay transport.
 *
 *  Composes the three primitive layers that already landed:
 *
 *   - `RelayJwt`        — EdDSA(ed25519) auth JWT carried in the
 *                         ws query string (`?auth=…`).
 *   - `WcEnvelope`      — ChaCha20-Poly1305 sealing of every
 *                         JSON payload (Type 0 for steady-state,
 *                         Type 1 for the session-approve reply
 *                         that ships the responder's X25519 pubkey).
 *   - `WcKeyAgreement`  — X25519 + HKDF + `sha256(symKey)` topic.
 *
 *  …with `WsChannel` (the JDK `java.net.http.WebSocket` wrapper)
 *  and the JSON-RPC framing in `RelayJsonRpc` (`irn_publish` /
 *  `irn_subscribe` / `irn_subscription`).
 *
 *  Construct with:
 *  ```
 ...
 *
 *  For tests, inject a `MockWsChannel`. */
