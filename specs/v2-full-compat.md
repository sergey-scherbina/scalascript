...
The next production blockers are the parser/DSL output shape gap (`dsl-calc-parser`),
quoted macro body evaluation (`quoted-macro-interpreter`), the rozum server/batch
scope decision, and the deliberate `contentToolkitSection` batch stub left in the
v2 bridge until section-level toolkit lowering is parity-checked.
