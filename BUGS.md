...
- **Remaining busi standalone-bundle frontiers (separate from this effect cluster, newly exposed once the leak was fixed):** `trust.ssc` — an `Unhandled effect` from a per-construct CPS gap (`val x = query(...)` then `x match {…}` / command chain inside a handler body — a specific construct `genCpsExpr` doesn't fully thread); `qr.ssc` — a `Method not found` dispatch error; `ksef.ssc` — a duplicate global `const readFile` (the `jsgen-toplevel-name-vs-preamble`/#5 class). Tracked for follow-up; the effect-handler cluster (#3) itself is closed.
