...
function _mkRequest(req, params, bodyBuf) {
  ...
  const json = (body && body.length > 0)
    // Same parse as jsonParse: a request body's fractional numbers must be exact decimals here
    ...
    ? (() => { try { return _Some(_jsonConvert(_jsonParseRaw(body))); } catch (e) { return _None; } })()
    : _None;
  return {
    _type:   'Request',
    method:  req.method,
