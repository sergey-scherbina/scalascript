...
ENTITIES_INPUT = HERE / "whatwg-entities.json"
...
ENTITIES_RAW_SHA256 = "d741d877ac77c4194c4ad526b5b4a19aef8dfe411ab840a466891cdbb9f362e6"
BASELINE_TSV_SHA256 = "6948296949b6894d5021db2b75b0a010f3449c6415d7cbbaa602c7d16edd5b30"
...
CORPUS_FILE_ROLES = {
    ...
    "LICENSE.whatwg.txt": "upstream-license",
    "whatwg-entities.json": "upstream-snapshot",
...
def generated_files() -> dict[Path, str]:
    ...
    result[MAIN_OUT / "MarkdownEntitiesGenerated.scala"] = entities_source()
    return result
...
def manifest_source(
    ...
    for name in sorted(main_contents):
        file_entries.append(
            {
                "path": f"{MAIN_OUT_RELATIVE}/{name}",
                "role": "generated-entity-table",
                "size": len(main_contents[name]),
                "sha256": sha256_bytes(main_contents[name]),
            }
        )
...
MAIN_OUT_RELATIVE = (
    "uniml/markdown/src/main/scala/scalascript/uniml/dialect/markdown/generated"
)


def entities_source() -> str:
    """The WHATWG HTML5 named character references, as a compact encoded table.

    Emitted as ONE string rather than a 2125-entry Map literal on purpose: a
    literal that size is a single enormous static initialiser, which runs into
    the JVM's 64 KB method limit and is never JIT-compiled even when it fits.
    The encoding is `name\u0001value` records joined by \u0000, split once at
    class-init into an immutable Map.

    Only SEMICOLON-TERMINATED names are emitted. CommonMark 6.2 recognises no
    others, and the WHATWG file also lists 106 legacy names without the
    semicolon, which must stay literal text.
    """
    require_digest(ENTITIES_INPUT, ENTITIES_RAW_SHA256)
    data = json.loads(ENTITIES_INPUT.read_text(encoding="utf-8"))
    records: list[str] = []
    for name in sorted(data):
        if not name.startswith("&") or not name.endswith(";"):
            continue
        value = "".join(chr(cp) for cp in data[name]["codepoints"])
        if "\u0000" in value or "\u0001" in value:
            raise SystemExit(f"entity value collides with the record separators: {name}")
        records.append(f"{name[1:-1]}\u0001{value}")
    if not records:
        raise SystemExit("no semicolon-terminated entities found in the snapshot")
    encoded = "\u0000".join(records)
    lines = [
        "// GENERATED by uniml/corpus/markdown/generate.py — do not edit.",
        "//",
        "// Source: WHATWG HTML entities.json, pinned in uniml/corpus/markdown/",
        f"// whatwg-entities.json (sha256 {ENTITIES_RAW_SHA256}).",
        f"// {len(records)} semicolon-terminated named character references.",
        "package scalascript.uniml.dialect.markdown.generated",
        "",
        "private[markdown] object MarkdownEntitiesGenerated:",
        f"  val count: Int = {len(records)}",
        "",
        "  /** `name\\u0001value` records joined by `\\u0000`. */",
        f"  private val encoded: String = {scala_string(encoded)}",
        "",
...
def expected_contents(
    ...
    for path, content in files.items():
        ...
        elif path.parent == MAIN_OUT:
            main_contents[path.name] = encoded
        else:
    ...
    manifest = manifest_source(corpus_contents, scala_contents, main_contents)
    return corpus_contents, scala_contents, main_contents, manifest
...
def write(files: dict[Path, str]) -> None:
    ...
    corpus_contents, scala_contents, main_contents, manifest = expected_contents(files)
    ...
    errors = integrity_errors(
        HERE, SCALA_OUT, MAIN_OUT, corpus_contents, scala_contents, main_contents
    )
    if errors:
...
def integrity_errors(
    ...
    inspect("main-generated", main_root, expected_main)
    return errors
...
def check(files: dict[Path, str]) -> None:
    ...
    negative_integrity_probes(corpus_contents, scala_contents, main_contents)
    print(
