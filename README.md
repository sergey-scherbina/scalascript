...
| [Full-stack in-process transport](docs/fullstack-in-process-transport.md) | Planned, partially implemented: `BackendTransport` types, `ssc run --transport http|in-process` parsing/diagnostics, interpreter/test-harness `InProcessBackendTransport`, same-process Swing runtime, Swing `fetchAction`, `fetchTable`, and generated typed route clients dispatch through the generated JVM route registry without opening an HTTP socket; broader frontend selection remains planned |
...
| [Typed route clients](docs/typed-route-clients.md) | Planned, partially implemented: `apiClients:` / `api-clients:` front-matter endpoint metadata parses into AST and JVM codegen metadata; JVM/Swing now generates callable in-process client methods, while HTTP clients for Electron/browser/split modes remain planned |
...
|---------|---------------|
| Full-stack in-process transport | `ssc run` and `ssc run-jvm` recognize `--transport http|in-process`; interpreter route tests can use `InProcessBackendTransport`; Swing now runs in the same JVM process, and Swing `fetchAction` / `fetchTable` / typed route client calls can dispatch to generated JVM backend routes without opening an HTTP socket; `examples/frontend/swing-fullstack/` and `examples/frontend/swing-typed-client/` demonstrate the no-socket paths |
...
| Typed route clients | `apiClients:` / `api-clients:` front matter preserves endpoint method/path/request/response metadata in AST and JVM codegen. JVM/Swing now generates callable in-process clients that encode case class inputs, dispatch through the Swing route registry, and decode typed JSON responses. HTTP clients for Electron/browser/split modes remain planned |
