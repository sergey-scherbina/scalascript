...
if command -v node >/dev/null 2>&1; then ssc run bin/mira-js.ssc0 examples/hm-tuptype-field.hm > "${TMPDIR:-/tmp}/tf.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/tf.js" 2>/dev/null | tail -1); if [ "$got" = "$TFW" ]; then printf 'ok   %-26s => %s (node)\n' "tuple-type field -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "tuptype-field JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/mira-rust.ssc0 examples/hm-tuptype-field.hm > "${TMPDIR:-/tmp}/tf.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/tf.rs" -o "${TMPDIR:-/tmp}/tf-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/tf-bin"); else got="(rustc err)"; fi; if [ "$got" = "$TFW" ]; then printf 'ok   %-26s => %s (rustc)\n' "tuple-type field -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "tuptype-field Rust" "$got"; fail=1; fi; fi
...
if command -v node >/dev/null 2>&1; then $JX run bin/mira-js.ssc0 examples/hm-json.hm > "${TMPDIR:-/tmp}/json.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/json.js" 2>/dev/null | tail -1); if [ "$got" = "$JW" ]; then printf 'ok   %-26s => %s (node)\n' "json roundtrip -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "json JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then $JX run bin/mira-rust.ssc0 examples/hm-json.hm > "${TMPDIR:-/tmp}/json.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/json.rs" -o "${TMPDIR:-/tmp}/json-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/json-bin"); else got="(rustc err)"; fi; if [ "$got" = "$JW" ]; then printf 'ok   %-26s => %s (rustc)\n' "json roundtrip -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "json Rust" "$got"; fail=1; fi; fi
...
if command -v node >/dev/null 2>&1; then ssc run bin/mira-js.ssc0 examples/hm-either.hm > "${TMPDIR:-/tmp}/ei.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/ei.js" 2>/dev/null | tail -1); if [ "$got" = "$EIW" ]; then printf 'ok   %-26s => %s (node)\n' "either combinators -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "either JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/mira-rust.ssc0 examples/hm-either.hm > "${TMPDIR:-/tmp}/ei.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/ei.rs" -o "${TMPDIR:-/tmp}/ei-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/ei-bin"); else got="(rustc err)"; fi; if [ "$got" = "$EIW" ]; then printf 'ok   %-26s => %s (rustc)\n' "either combinators -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "either Rust" "$got"; fail=1; fi; fi
...
if command -v node >/dev/null 2>&1; then ssc run bin/mira-js.ssc0 examples/hm-eff-traverse.hm > "${TMPDIR:-/tmp}/tv.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/tv.js" 2>/dev/null | tail -1); if [ "$got" = "$TVV" ]; then printf 'ok   %-26s => %s (node)\n' "eff-poly traverse -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "eff-traverse JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/mira-rust.ssc0 examples/hm-eff-traverse.hm > "${TMPDIR:-/tmp}/tv.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/tv.rs" -o "${TMPDIR:-/tmp}/tv-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/tv-bin"); else got="(rustc err)"; fi; if [ "$got" = "$TVV" ]; then printf 'ok   %-26s => %s (rustc)\n' "eff-poly traverse -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "eff-traverse Rust" "$got"; fail=1; fi; fi
...
if command -v node >/dev/null 2>&1; then ssc run bin/mira-js.ssc0 examples/hm-eff-typed-resume.hm > "${TMPDIR:-/tmp}/tr.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/tr.js" 2>/dev/null | tail -1); if [ "$got" = "2" ]; then printf 'ok   %-26s => %s (node)\n' "typed resume -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "typed resume JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/mira-rust.ssc0 examples/hm-eff-typed-resume.hm > "${TMPDIR:-/tmp}/tr.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/tr.rs" -o "${TMPDIR:-/tmp}/tr-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/tr-bin"); else got="(rustc err)"; fi; if [ "$got" = "2" ]; then printf 'ok   %-26s => %s (rustc)\n' "typed resume -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "typed resume Rust" "$got"; fail=1; fi; fi
...
if command -v node >/dev/null 2>&1; then ssc run bin/mira-js.ssc0 examples/hm-eff-multiop.hm > "${TMPDIR:-/tmp}/mo48.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/mo48.js" 2>/dev/null | tail -1); if [ "$got" = "$MV" ]; then printf 'ok   %-26s => %s (node)\n' "handleM -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "handleM JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/mira-rust.ssc0 examples/hm-eff-multiop.hm > "${TMPDIR:-/tmp}/mo48.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/mo48.rs" -o "${TMPDIR:-/tmp}/mo48-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/mo48-bin"); else got="(rustc err)"; fi; if [ "$got" = "$MV" ]; then printf 'ok   %-26s => %s (rustc)\n' "handleM -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "handleM Rust" "$got"; fail=1; fi; fi
echo -n "ok   handleM row composition => "; printf 'effect QA { ask : Dyn -> Int , tell : String -> String } in let prog = doE { x <- ask ; u <- logE "seen" ; t <- tell "hello" ; pureE (x * 2) } in runE (runLogE (handleM "QA" prog { | ask a k => k 21 | tell a k => k "done" } (fun v => pureE v)))' > "${TMPDIR:-/tmp}/mo48compose.hm"; mc=$(ssc run bin/mira.ssc0 "${TMPDIR:-/tmp}/mo48compose.hm" | tail -1); ssc run bin/mirac.ssc0 "${TMPDIR:-/tmp}/mo48compose.hm" > "${TMPDIR:-/tmp}/mo48compose.coreir" 2>/dev/null; got=$(ssc run-ir "${TMPDIR:-/tmp}/mo48compose.coreir" | tail -1); if [ "$mc" = '"(Int, [Dyn])"' ] && [ "$got" = 'Pair(42, Cons("seen", Nil))' ]; then echo "handled QA composes with Log"; else echo "FAIL type [$mc] run [$got]"; fail=1; fi
...
if command -v node >/dev/null 2>&1; then ssc run bin/mira-js.ssc0 examples/hm-quad.hm > "${TMPDIR:-/tmp}/q.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/q.js" 2>/dev/null | tail -1); if [ "$got" = "10" ]; then printf 'ok   %-26s => %s (node)\n' "4-tuple -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "quad JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/mira-rust.ssc0 examples/hm-quad.hm > "${TMPDIR:-/tmp}/q.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/q.rs" -o "${TMPDIR:-/tmp}/q-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/q-bin"); else got="(rustc err)"; fi; if [ "$got" = "10" ]; then printf 'ok   %-26s => %s (rustc)\n' "4-tuple -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "quad Rust" "$got"; fail=1; fi; fi
...
if command -v node >/dev/null 2>&1; then ssc run bin/mira-js.ssc0 examples/hm-guard.hm > "${TMPDIR:-/tmp}/g.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/g.js" 2>/dev/null | tail -1); if [ "$got" = "0" ]; then printf 'ok   %-26s => %s (node)\n' "pattern guards -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "guard JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/mira-rust.ssc0 examples/hm-guard.hm > "${TMPDIR:-/tmp}/g.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/g.rs" -o "${TMPDIR:-/tmp}/g-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/g-bin"); else got="(rustc err)"; fi; if [ "$got" = "0" ]; then printf 'ok   %-26s => %s (rustc)\n' "pattern guards -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "guard Rust" "$got"; fail=1; fi; fi
...
if command -v node >/dev/null 2>&1; then ssc run bin/mira-js.ssc0 examples/hm-escapes.hm > "${TMPDIR:-/tmp}/se.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/se.js" 2>/dev/null | tail -1); if [ "$got" = "5" ]; then printf 'ok   %-26s => %s (node)\n' "string escapes -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "escapes JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/mira-rust.ssc0 examples/hm-escapes.hm > "${TMPDIR:-/tmp}/se.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/se.rs" -o "${TMPDIR:-/tmp}/se-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/se-bin"); else got="(rustc err)"; fi; if [ "$got" = "5" ]; then printf 'ok   %-26s => %s (rustc)\n' "string escapes -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "escapes Rust" "$got"; fail=1; fi; fi
...
if command -v node >/dev/null 2>&1; then ssc run bin/mira-js.ssc0 examples/hm-recupd.hm > "${TMPDIR:-/tmp}/ru.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/ru.js" 2>/dev/null | tail -1); if [ "$got" = "11" ]; then printf 'ok   %-26s => %s (node)\n' "record update -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "recupd JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/mira-rust.ssc0 examples/hm-recupd.hm > "${TMPDIR:-/tmp}/ru.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/ru.rs" -o "${TMPDIR:-/tmp}/ru-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/ru-bin"); else got="(rustc err)"; fi; if [ "$got" = "11" ]; then printf 'ok   %-26s => %s (rustc)\n' "record update -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "recupd Rust" "$got"; fail=1; fi; fi
...
if command -v node >/dev/null 2>&1; then ssc run bin/mira-js.ssc0 examples/hm-neg.hm > "${TMPDIR:-/tmp}/neg.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/neg.js" 2>/dev/null | tail -1); if [ "$got" = "14" ]; then printf 'ok   %-26s => %s (node)\n' "neg literal -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "neg literal JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/mira-rust.ssc0 examples/hm-neg.hm > "${TMPDIR:-/tmp}/neg.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/neg.rs" -o "${TMPDIR:-/tmp}/neg-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/neg-bin"); else got="(rustc err)"; fi; if [ "$got" = "14" ]; then printf 'ok   %-26s => %s (rustc)\n' "neg literal -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "neg literal Rust" "$got"; fail=1; fi; fi
...
if command -v node >/dev/null 2>&1; then ssc run bin/mira-js.ssc0 examples/hm-letann.hm > "${TMPDIR:-/tmp}/la.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/la.js" 2>/dev/null | tail -1); if [ "$got" = "$LANN" ]; then printf 'ok   %-26s => %s (node)\n' "let annot -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "let annot JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/mira-rust.ssc0 examples/hm-letann.hm > "${TMPDIR:-/tmp}/la.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/la.rs" -o "${TMPDIR:-/tmp}/la-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/la-bin"); else got="(rustc err)"; fi; if [ "$got" = "$LANN" ]; then printf 'ok   %-26s => %s (rustc)\n' "let annot -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "let annot Rust" "$got"; fail=1; fi; fi
...
if command -v node >/dev/null 2>&1; then ssc run bin/mira-js.ssc0 examples/hm-strord.hm > "${TMPDIR:-/tmp}/so.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/so.js" 2>/dev/null | tail -1); if [ "$got" = "101" ]; then printf 'ok   %-26s => %s (node)\n' "string ord -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "string ord JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/mira-rust.ssc0 examples/hm-strord.hm > "${TMPDIR:-/tmp}/so.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/so.rs" -o "${TMPDIR:-/tmp}/so-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/so-bin"); else got="(rustc err)"; fi; if [ "$got" = "101" ]; then printf 'ok   %-26s => %s (rustc)\n' "string ord -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "string ord Rust" "$got"; fail=1; fi; fi
...
if command -v node >/dev/null 2>&1; then ssc run bin/mira-js.ssc0 examples/hm-listconcat.hm > "${TMPDIR:-/tmp}/lc.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/lc.js" 2>/dev/null | tail -1); if [ "$got" = "15" ]; then printf 'ok   %-26s => %s (node)\n' "list ++ -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "list ++ JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/mira-rust.ssc0 examples/hm-listconcat.hm > "${TMPDIR:-/tmp}/lc.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/lc.rs" -o "${TMPDIR:-/tmp}/lc-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/lc-bin"); else got="(rustc err)"; fi; if [ "$got" = "15" ]; then printf 'ok   %-26s => %s (rustc)\n' "list ++ -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "list ++ Rust" "$got"; fail=1; fi; fi
echo -n "ok   list ++ shape + string ++ both => "; printf '[1, 2] ++ [3]' > "${TMPDIR:-/tmp}/lc2.hm"; ssc run bin/mirac.ssc0 "${TMPDIR:-/tmp}/lc2.hm" 2>/dev/null > "${TMPDIR:-/tmp}/lc2.coreir"; lc2=$(ssc run-ir "${TMPDIR:-/tmp}/lc2.coreir" | tail -1); printf '"foo" ++ "bar"' > "${TMPDIR:-/tmp}/lc3.hm"; ssc run bin/mirac.ssc0 "${TMPDIR:-/tmp}/lc3.hm" 2>/dev/null > "${TMPDIR:-/tmp}/lc3.coreir"; lc3=$(ssc run-ir "${TMPDIR:-/tmp}/lc3.coreir" | tail -1); if [ "$lc2" = "Cons(1, Cons(2, Cons(3, Nil)))" ] && [ "$lc3" = '"foobar"' ]; then echo "[1,2]++[3]=list ; \"foo\"++\"bar\"=foobar (one ++, both types)"; else echo "FAIL [$lc2 / $lc3]"; fail=1; fi
...
if command -v node >/dev/null 2>&1; then ssc run bin/mira-js.ssc0 examples/hm-letdestr.hm > "${TMPDIR:-/tmp}/ld.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/ld.js" 2>/dev/null | tail -1); if [ "$got" = "25" ]; then printf 'ok   %-26s => %s (node)\n' "let destructure -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "let destructure JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/mira-rust.ssc0 examples/hm-letdestr.hm > "${TMPDIR:-/tmp}/ld.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/ld.rs" -o "${TMPDIR:-/tmp}/ld-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/ld-bin"); else got="(rustc err)"; fi; if [ "$got" = "25" ]; then printf 'ok   %-26s => %s (rustc)\n' "let destructure -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "let destructure Rust" "$got"; fail=1; fi; fi
...
if command -v node >/dev/null 2>&1; then ssc run bin/mira-js.ssc0 examples/hm-negpat.hm > "${TMPDIR:-/tmp}/np.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/np.js" 2>/dev/null | tail -1); if [ "$got" = "601" ]; then printf 'ok   %-26s => %s (node)\n' "neg-lit pattern -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "neg-lit pattern JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/mira-rust.ssc0 examples/hm-negpat.hm > "${TMPDIR:-/tmp}/np.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/np.rs" -o "${TMPDIR:-/tmp}/np-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/np-bin"); else got="(rustc err)"; fi; if [ "$got" = "601" ]; then printf 'ok   %-26s => %s (rustc)\n' "neg-lit pattern -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "neg-lit pattern Rust" "$got"; fail=1; fi; fi
...
if command -v node >/dev/null 2>&1; then ssc run bin/mira-js.ssc0 examples/hm-listpat.hm > "${TMPDIR:-/tmp}/lpt.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/lpt.js" 2>/dev/null | tail -1); if [ "$got" = "21" ]; then printf 'ok   %-26s => %s (node)\n' "list pattern -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "list pattern JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/mira-rust.ssc0 examples/hm-listpat.hm > "${TMPDIR:-/tmp}/lpt.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/lpt.rs" -o "${TMPDIR:-/tmp}/lpt-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/lpt-bin"); else got="(rustc err)"; fi; if [ "$got" = "21" ]; then printf 'ok   %-26s => %s (rustc)\n' "list pattern -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "list pattern Rust" "$got"; fail=1; fi; fi
...
if command -v node >/dev/null 2>&1; then ssc run bin/mira-js.ssc0 examples/hm-recdestr.hm > "${TMPDIR:-/tmp}/rdc.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/rdc.js" 2>/dev/null | tail -1); if [ "$got" = "344" ]; then printf 'ok   %-26s => %s (node)\n' "rec destructure -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "rec destructure JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/mira-rust.ssc0 examples/hm-recdestr.hm > "${TMPDIR:-/tmp}/rdc.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/rdc.rs" -o "${TMPDIR:-/tmp}/rdc-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/rdc-bin"); else got="(rustc err)"; fi; if [ "$got" = "344" ]; then printf 'ok   %-26s => %s (rustc)\n' "rec destructure -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "rec destructure Rust" "$got"; fail=1; fi; fi
...
if command -v node >/dev/null 2>&1; then ssc run bin/mira-js.ssc0 examples/hm-charlit.hm > "${TMPDIR:-/tmp}/cl.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/cl.js" 2>/dev/null | tail -1); if [ "$got" = "1025" ]; then printf 'ok   %-26s => %s (node)\n' "char literal -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "char literal JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/mira-rust.ssc0 examples/hm-charlit.hm > "${TMPDIR:-/tmp}/cl.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/cl.rs" -o "${TMPDIR:-/tmp}/cl-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/cl-bin"); else got="(rustc err)"; fi; if [ "$got" = "1025" ]; then printf 'ok   %-26s => %s (rustc)\n' "char literal -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "char literal Rust" "$got"; fail=1; fi; fi
...
if command -v node >/dev/null 2>&1; then ssc run bin/mira-js.ssc0 examples/hm-floatmath.hm > "${TMPDIR:-/tmp}/fm.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/fm.js" 2>/dev/null | tail -1); if [ "$got" = "1111" ]; then printf 'ok   %-26s => %s (node)\n' "float math -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "float math JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/mira-rust.ssc0 examples/hm-floatmath.hm > "${TMPDIR:-/tmp}/fm.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/fm.rs" -o "${TMPDIR:-/tmp}/fm-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/fm-bin"); else got="(rustc err)"; fi; if [ "$got" = "1111" ]; then printf 'ok   %-26s => %s (rustc)\n' "float math -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "float math Rust" "$got"; fail=1; fi; fi
...
if command -v node >/dev/null 2>&1; then ssc run bin/mira-js.ssc0 examples/hm-mathx.hm > "${TMPDIR:-/tmp}/mx.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/mx.js" 2>/dev/null | tail -1); if [ "$got" = "1111111" ]; then printf 'ok   %-26s => %s (node)\n' "transcendentals -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "transcendentals JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/mira-rust.ssc0 examples/hm-mathx.hm > "${TMPDIR:-/tmp}/mx.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/mx.rs" -o "${TMPDIR:-/tmp}/mx-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/mx-bin"); else got="(rustc err)"; fi; if [ "$got" = "1111111" ]; then printf 'ok   %-26s => %s (rustc)\n' "transcendentals -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "transcendentals Rust" "$got"; fail=1; fi; fi
echo -n "ok   exp 1.0 bit-identical on 3 backends => "; printf 'exp 1.0' > "${TMPDIR:-/tmp}/mxe.hm"; ssc run bin/mirac.ssc0 "${TMPDIR:-/tmp}/mxe.hm" 2>/dev/null > "${TMPDIR:-/tmp}/mxe.coreir"; mxv=$(ssc run-ir "${TMPDIR:-/tmp}/mxe.coreir" | tail -1); mxok=1; if command -v node >/dev/null 2>&1; then ssc run bin/mira-js.ssc0 "${TMPDIR:-/tmp}/mxe.hm" > "${TMPDIR:-/tmp}/mxe.js" 2>/dev/null; [ "$(node "${TMPDIR:-/tmp}/mxe.js" 2>/dev/null | tail -1)" = "$mxv" ] || mxok=0; fi; if command -v rustc >/dev/null 2>&1; then ssc run bin/mira-rust.ssc0 "${TMPDIR:-/tmp}/mxe.hm" > "${TMPDIR:-/tmp}/mxe.rs" 2>/dev/null; rustc -O "${TMPDIR:-/tmp}/mxe.rs" -o "${TMPDIR:-/tmp}/mxe-bin" 2>/dev/null && [ "$("${TMPDIR:-/tmp}/mxe-bin")" = "$mxv" ] || mxok=0; fi; if [ "$mxok" = "1" ]; then echo "$mxv (run-ir == JS == Rust)"; else echo "FAIL diverged"; fail=1; fi
...
if command -v node >/dev/null 2>&1; then ssc run bin/mira-js.ssc0 examples/hm-rounding.hm > "${TMPDIR:-/tmp}/rd.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/rd.js" 2>/dev/null | tail -1); if [ "$got" = "1111111" ]; then printf 'ok   %-26s => %s (node)\n' "float rounding -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "float rounding JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/mira-rust.ssc0 examples/hm-rounding.hm > "${TMPDIR:-/tmp}/rd.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/rd.rs" -o "${TMPDIR:-/tmp}/rd-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/rd-bin"); else got="(rustc err)"; fi; if [ "$got" = "1111111" ]; then printf 'ok   %-26s => %s (rustc)\n' "float rounding -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "float rounding Rust" "$got"; fail=1; fi; fi
...
if command -v node >/dev/null 2>&1; then ssc run bin/mira-js.ssc0 examples/hm-mathx2.hm > "${TMPDIR:-/tmp}/m2.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/m2.js" 2>/dev/null | tail -1); if [ "$got" = "14" ]; then printf 'ok   %-26s => %s (node)\n' "math library -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "math library JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/mira-rust.ssc0 examples/hm-mathx2.hm > "${TMPDIR:-/tmp}/m2.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/m2.rs" -o "${TMPDIR:-/tmp}/m2-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/m2-bin"); else got="(rustc err)"; fi; if [ "$got" = "14" ]; then printf 'ok   %-26s => %s (rustc)\n' "math library -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "math library Rust" "$got"; fail=1; fi; fi
...
if command -v node >/dev/null 2>&1; then ssc run bin/mira-js.ssc0 examples/hm-recpat.hm > "${TMPDIR:-/tmp}/rpc.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/rpc.js" 2>/dev/null | tail -1); if [ "$got" = "6530" ]; then printf 'ok   %-26s => %s (node)\n' "record pattern -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "record pattern JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/mira-rust.ssc0 examples/hm-recpat.hm > "${TMPDIR:-/tmp}/rpc.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/rpc.rs" -o "${TMPDIR:-/tmp}/rpc-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/rpc-bin"); else got="(rustc err)"; fi; if [ "$got" = "6530" ]; then printf 'ok   %-26s => %s (rustc)\n' "record pattern -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "record pattern Rust" "$got"; fail=1; fi; fi
...
if command -v node >/dev/null 2>&1; then ssc run bin/mira-js.ssc0 examples/hm-letrecpoly.hm > "${TMPDIR:-/tmp}/lrp.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/lrp.js" 2>/dev/null | tail -1); if [ "$got" = "618" ]; then printf 'ok   %-26s => %s (node)\n' "letrec poly -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "letrec poly JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/mira-rust.ssc0 examples/hm-letrecpoly.hm > "${TMPDIR:-/tmp}/lrp.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/lrp.rs" -o "${TMPDIR:-/tmp}/lrp-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/lrp-bin"); else got="(rustc err)"; fi; if [ "$got" = "618" ]; then printf 'ok   %-26s => %s (rustc)\n' "letrec poly -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "letrec poly Rust" "$got"; fail=1; fi; fi
...
if command -v node >/dev/null 2>&1; then ssc run bin/mira-js.ssc0 examples/hm-dictpass.hm > "${TMPDIR:-/tmp}/dp.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/dp.js" 2>/dev/null | tail -1); if [ "$got" = "920" ]; then printf 'ok   %-26s => %s (node)\n' "dict-passing -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "dict-passing JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/mira-rust.ssc0 examples/hm-dictpass.hm > "${TMPDIR:-/tmp}/dp.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/dp.rs" -o "${TMPDIR:-/tmp}/dp-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/dp-bin"); else got="(rustc err)"; fi; if [ "$got" = "920" ]; then printf 'ok   %-26s => %s (rustc)\n' "dict-passing -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "dict-passing Rust" "$got"; fail=1; fi; fi
...
if command -v node >/dev/null 2>&1; then ssc run bin/mira-js.ssc0 examples/hm-dictord.hm > "${TMPDIR:-/tmp}/do.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/do.js" 2>/dev/null | tail -1); if [ "$got" = "511" ]; then printf 'ok   %-26s => %s (node)\n' "dict ord -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "dict ord JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/mira-rust.ssc0 examples/hm-dictord.hm > "${TMPDIR:-/tmp}/do.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/do.rs" -o "${TMPDIR:-/tmp}/do-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/do-bin"); else got="(rustc err)"; fi; if [ "$got" = "511" ]; then printf 'ok   %-26s => %s (rustc)\n' "dict ord -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "dict ord Rust" "$got"; fail=1; fi; fi
...
if command -v node >/dev/null 2>&1; then ssc run bin/mira-js.ssc0 examples/hm-scalarmatch.hm > "${TMPDIR:-/tmp}/sm.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/sm.js" 2>/dev/null | tail -1); if [ "$got" = "861" ]; then printf 'ok   %-26s => %s (node)\n' "scalar var-match -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "scalar var-match JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/mira-rust.ssc0 examples/hm-scalarmatch.hm > "${TMPDIR:-/tmp}/sm.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/sm.rs" -o "${TMPDIR:-/tmp}/sm-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/sm-bin"); else got="(rustc err)"; fi; if [ "$got" = "861" ]; then printf 'ok   %-26s => %s (rustc)\n' "scalar var-match -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "scalar var-match Rust" "$got"; fail=1; fi; fi
...
if command -v node >/dev/null 2>&1; then ssc run bin/mira-js.ssc0 examples/hm-userdict.hm > "${TMPDIR:-/tmp}/ud.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/ud.js" 2>/dev/null | tail -1); if [ "$got" = "1433" ]; then printf 'ok   %-26s => %s (node)\n' "user-class dict -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "user-class dict JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/mira-rust.ssc0 examples/hm-userdict.hm > "${TMPDIR:-/tmp}/ud.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/ud.rs" -o "${TMPDIR:-/tmp}/ud-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/ud-bin"); else got="(rustc err)"; fi; if [ "$got" = "1433" ]; then printf 'ok   %-26s => %s (rustc)\n' "user-class dict -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "user-class dict Rust" "$got"; fail=1; fi; fi
...
if command -v node >/dev/null 2>&1; then ssc run bin/mira-js.ssc0 examples/hm-prelude3.hm > "${TMPDIR:-/tmp}/p3.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/p3.js" 2>/dev/null | tail -1); if [ "$got" = "5329" ]; then printf 'ok   %-26s => %s (node)\n' "prelude2 -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "prelude2 JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/mira-rust.ssc0 examples/hm-prelude3.hm > "${TMPDIR:-/tmp}/p3.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/p3.rs" -o "${TMPDIR:-/tmp}/p3-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/p3-bin"); else got="(rustc err)"; fi; if [ "$got" = "5329" ]; then printf 'ok   %-26s => %s (rustc)\n' "prelude2 -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "prelude2 Rust" "$got"; fail=1; fi; fi
...
if command -v node >/dev/null 2>&1; then ssc run bin/mira-js.ssc0 examples/hm-strfns2.hm > "${TMPDIR:-/tmp}/sf.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/sf.js" 2>/dev/null | tail -1); if [ "$got" = "1112" ]; then printf 'ok   %-26s => %s (node)\n' "string fns 2 -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "string fns 2 JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/mira-rust.ssc0 examples/hm-strfns2.hm > "${TMPDIR:-/tmp}/sf.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/sf.rs" -o "${TMPDIR:-/tmp}/sf-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/sf-bin"); else got="(rustc err)"; fi; if [ "$got" = "1112" ]; then printf 'ok   %-26s => %s (rustc)\n' "string fns 2 -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "string fns 2 Rust" "$got"; fail=1; fi; fi
...
if command -v node >/dev/null 2>&1; then ssc run bin/mira-js.ssc0 examples/hm-strbuild.hm > "${TMPDIR:-/tmp}/sb.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/sb.js" 2>/dev/null | tail -1); if [ "$got" = "662" ]; then printf 'ok   %-26s => %s (node)\n' "string build -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "string build JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/mira-rust.ssc0 examples/hm-strbuild.hm > "${TMPDIR:-/tmp}/sb.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/sb.rs" -o "${TMPDIR:-/tmp}/sb-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/sb-bin"); else got="(rustc err)"; fi; if [ "$got" = "662" ]; then printf 'ok   %-26s => %s (rustc)\n' "string build -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "string build Rust" "$got"; fail=1; fi; fi
...
if command -v node >/dev/null 2>&1; then ssc run bin/mira-js.ssc0 examples/hm-preludecombi.hm > "${TMPDIR:-/tmp}/pc.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/pc.js" 2>/dev/null | tail -1); if [ "$got" = "11453" ]; then printf 'ok   %-26s => %s (node)\n' "prelude batch -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "prelude batch JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/mira-rust.ssc0 examples/hm-preludecombi.hm > "${TMPDIR:-/tmp}/pc.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/pc.rs" -o "${TMPDIR:-/tmp}/pc-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/pc-bin"); else got="(rustc err)"; fi; if [ "$got" = "11453" ]; then printf 'ok   %-26s => %s (rustc)\n' "prelude batch -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "prelude batch Rust" "$got"; fail=1; fi; fi
...
if command -v node >/dev/null 2>&1; then ssc run bin/mira-js.ssc0 examples/hm-aspat.hm > "${TMPDIR:-/tmp}/apc.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/apc.js" 2>/dev/null | tail -1); if [ "$got" = "4" ]; then printf 'ok   %-26s => %s (node)\n' "as-pattern -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "as-pattern JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/mira-rust.ssc0 examples/hm-aspat.hm > "${TMPDIR:-/tmp}/apc.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/apc.rs" -o "${TMPDIR:-/tmp}/apc-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/apc-bin"); else got="(rustc err)"; fi; if [ "$got" = "4" ]; then printf 'ok   %-26s => %s (rustc)\n' "as-pattern -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "as-pattern Rust" "$got"; fail=1; fi; fi
...
if command -v node >/dev/null 2>&1; then ssc run bin/mira-js.ssc0 examples/hm-opsections.hm > "${TMPDIR:-/tmp}/osc.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/osc.js" 2>/dev/null | tail -1); if [ "$got" = "21" ]; then printf 'ok   %-26s => %s (node)\n' "op section -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "op section JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/mira-rust.ssc0 examples/hm-opsections.hm > "${TMPDIR:-/tmp}/osc.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/osc.rs" -o "${TMPDIR:-/tmp}/osc-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/osc-bin"); else got="(rustc err)"; fi; if [ "$got" = "21" ]; then printf 'ok   %-26s => %s (rustc)\n' "op section -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "op section Rust" "$got"; fail=1; fi; fi
...
if command -v node >/dev/null 2>&1; then ssc run bin/mira-js.ssc0 examples/hm-lsections.hm > "${TMPDIR:-/tmp}/lsc.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/lsc.js" 2>/dev/null | tail -1); if [ "$got" = "247" ]; then printf 'ok   %-26s => %s (node)\n' "left section -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "left section JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/mira-rust.ssc0 examples/hm-lsections.hm > "${TMPDIR:-/tmp}/lsc.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/lsc.rs" -o "${TMPDIR:-/tmp}/lsc-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/lsc-bin"); else got="(rustc err)"; fi; if [ "$got" = "247" ]; then printf 'ok   %-26s => %s (rustc)\n' "left section -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "left section Rust" "$got"; fail=1; fi; fi
...
if command -v node >/dev/null 2>&1; then ssc run bin/mira-js.ssc0 examples/hm-stringfns.hm > "${TMPDIR:-/tmp}/sfc.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/sfc.js" 2>/dev/null | tail -1); if [ "$got" = "431" ]; then printf 'ok   %-26s => %s (node)\n' "string fns -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "string fns JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/mira-rust.ssc0 examples/hm-stringfns.hm > "${TMPDIR:-/tmp}/sfc.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/sfc.rs" -o "${TMPDIR:-/tmp}/sfc-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/sfc-bin"); else got="(rustc err)"; fi; if [ "$got" = "431" ]; then printf 'ok   %-26s => %s (rustc)\n' "string fns -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "string fns Rust" "$got"; fail=1; fi; fi
