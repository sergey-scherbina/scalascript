...
[`docs/specs/distributed-wire-protocol.md`](distributed-wire-protocol.md). This spec
...
| `DStream[A].toLocalSource` | `docs/specs/distributed-streams.md` | Specced, not implemented |
...
| Typed route clients | `docs/specs/typed-route-clients.md`, `JvmGen.scala` | Partial |
| `WireCodec[A]` | `docs/specs/distributed-wire-protocol.md`, `WORK_QUEUE.md` v1.62.1 | Planned |
...
startup code is tracked in v1.63.4/v1.63.5. Parser-time validation rejects registry entries
whose `function`, `source`, or `behavior` target is not defined locally, and
rejects missing request/response/args type declarations.
...
- Add `cluster_spawn` / `cluster_spawn_ack` wire messages. ✓ Landed
  ...
  profiles remain under `docs/specs/distributed-wire-protocol.md`.
...
- Add diagnostics for missing handlers/codecs/code mismatch. ✓ Landed
  2026-05-28 for missing front-matter registry definitions, missing registry
  types, and code identity mismatch.
...
| v1.63.3 | `clusterOf()` exposes local node, peers, auth token, seed resolver, and code identity; static/DNS/K8s seeds resolve; Consul fails clearly until its resolver backend lands; `assertCodeIdentity` reports expected/actual digest mismatch; `cluster:` and registry front matter parse into typed metadata and survive Normalize/Denormalize + `.sscc`; missing registry function/source/behavior targets and missing registry types are rejected; top-level `cluster Demo:` lowers into `ClusterDecl` |
...
- `docs/specs/distributed-wire-protocol.md`
