...
parity: 68/91 identical · 0 mismatch · 0 v2-error · 23 v1-only
        (26 both-fail not-a-gap · 36 true-server · 0 long-running ·
         33 backend-lane · 5 nondet · 4 v1-side · 195 total)
```
...
The single v2-error (`dataset-parallel-sum`) is closed by `44f3d4a24`: v2 crashed
with `StackOverflowError` in recursive list conversion, and `Prims.unlistPub` /
`listOf` are now iterative. The latest full sweep has **0 v2-error** cases:
**54/88 identical · 11 mismatch · 0 v2-error · 23 v1-only**. That run includes one
transient `invoice-email` generated byte-count mismatch; an immediate targeted rerun
of `invoice-email` plus `dataset-parallel-sum` was 2/2 MATCH.

Post-p3 update (2026-07-08): after `a0f032c15` (real v2 web server / rozum runner
work) and `d8e0ecee4` (stable invoice email output), the full production gate is:

```text
PARITY_TIMEOUT=45 SSC="bin/ssc" scripts/v2-output-parity --all
parity: 55/85 identical · 9 mismatch · 1 v2-error · 20 v1-only
        (40 both-fail not-a-gap · 36 true-server · 0 long-running ·
         32 backend-lane · 2 nondet · 195 total)
```
...
- quoted macro body evaluation: `quoted-macro-interpreter.ssc` still misses computed
  impl-body outputs;
- rozum schema-derived / streaming parity or scope decision: `rozum-agent` and
  `rozum-agent-pool` match, but `rozum-agent-schema-derived.ssc` and
  `rozum-agent-streaming.ssc` still mismatch.

The remaining actors/async/effects/os-env/parser mismatches are currently treated as
