...
- [x] Read the interoperability corpus byte-for-value equal to reference SQLite
  ...
  all four tiers reproduce the identical 629-line reference oracle.
...
- Twenty-four immutable valid databases pin official SQLite 3.53.3 source id,
  compile options, SHA-256, header metadata, `integrity_check = ok`, and 629
  ...
  table-leaf cells straddling the exact overflow threshold (`overflow-thresholds.db`:
  payloads `p = X-1/X/X+1`, the sharp `K > X` fall to the `m`-byte residue, the
  `K <= X` mid-range case, and a multi-page overflow chain).
...
- `tests/e2e/scljet-m2-corpus-smoke.sh` passes through the assembled JVM VFS:
  SclJet reproduces all 629 oracle lines, 25 pinned corrupt files produce
...
- `overflow-thresholds.db` adds exact table-leaf payload-threshold vectors on a
  512-byte page: total payloads `p = X-1/X/X+1` (the inclusive local boundary
  and the one-byte overflow that drops to the `m = 39`-byte residue when
  `K = m + ((p - m) % (u - 4)) > X`), a `K <= X` mid-range case that keeps `K`
  bytes local, and a multi-page overflow chain. All three tiers reproduce the
  reference rows byte-for-value; it is regenerated by the same pinned SQLite
  3.53.3 as the rest of the corpus (no external helper required).
...
an explicit open gate. Exact table-leaf payload thresholds are now pinned; deep
record/overflow/freeblock/schema corruptions and index-btree payload-threshold
vectors remain queued M2d hardening (`BACKLOG.md`).
