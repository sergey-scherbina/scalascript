...
| [Full-stack in-process transport](docs/fullstack-in-process-transport.md) | Planned, partially implemented: `BackendTransport` types, `ssc run --transport http|in-process` parsing/diagnostics, interpreter/test-harness `InProcessBackendTransport`, and same-process Swing runtime exist; generated frontend client selection is not implemented yet |
| [JVM Desktop Frontend](docs/jvm-desktop-frontend.md) | Partially implemented: `frontend-swing` backend, ServiceLoader registration, `ssc run-jvm --frontend swing`, static Swing toolkit subset emission, local signal action bridge, and a `swing-plugin` skeleton for future interpreter intrinsics; backend transport remains planned |
...
|---------|---------------|
| Full-stack in-process transport | `ssc run` and `ssc run-jvm` recognize `--transport http|in-process`; interpreter route tests can use `InProcessBackendTransport`; Swing now runs in the same JVM process, while generated backend client dispatch is still planned |
| JVM desktop frontend | `ssc run-jvm --frontend swing app.ssc` launches the JDK-only Swing runtime in the current JVM process; `ssc run-jvm --frontend swing --transport in-process` is accepted as the monolithic JVM mode foundation; `ssc run --frontend swing` remains the interpreter path and reports that Swing intrinsics are planned; backend transport dispatch and JavaFX/Compose adapters remain planned |
...
| [swing-hello.ssc](examples/frontend/swing-hello/swing-hello.ssc) | Minimal JDK-only Swing desktop window; frontend SPI `swing` backend support is still a skeleton |
