...
- [ ] **coremin-prelude-board-closeout** — docs-only closeout for `coremin-prelude-migrate` after UPDATE-3.
      Mark the `[~] coremin-prelude-migrate` entry actionable-scope done because the bundled-effect runner
      sub-thread is complete (16 names off core prelude; `runnerType2` removed). Keep future work explicit:
      non-bundled/advanced names need their own strict-opt-in preludeSymbols task, and Stream/Actors stay under
      their existing entangled entries. Add a concise CHANGELOG line; do not touch Typer/plugin code.
