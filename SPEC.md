...
| **C.3 — DataFrame ergonomics + schema bridge** | Five landed slices so far: (1) `>10` binds via `Map.ofEntries[String, Object]` once `SparkGen.MapOfMaxPairs = 10` is exceeded; (2) widen the emitted `sparkImports` to include `Row`, `DataFrame`, and `types._`; (3) `spark-config:` front-matter map → sorted `.config(k, v)` lines on `SparkSession.builder()`; (4) `spark-app-name:` front-matter overrides `.appName(...)` for the Spark UI / history server / log lines; (5) typed reader convenience shims `Dataset.fromParquet(path)`, `Dataset.fromJson(path)`, `Dataset.fromCsv(path)` — each emitted inline in `@main` returning Spark `DataFrame` so user blocks can do `val users = Dataset.fromParquet("/users")` without the `spark.read.X(path)` boilerplate; format-specific options (`header=true`, `inferSchema=true`, partition pruning, …) still require dropping to `spark.read.option(...).X(...)` directly.  Still open: map `std/parsing` schemas to Spark `StructType`. | C.1+C.2 ship the binding/runtime contract; C.3 polishes the typed surface. | partially landed (slices 1–5) |
