...
/** Cross-platform spec for [[RelayTransportBase]] — exercises the
 *  shared WC protocol logic (ApproveSession Type-1 X25519 handshake,
 *  EmitEvent Type-0, inbound demux, subscribe / unsubscribe framing,
 *  JWT-authenticated connect URL) against a synchronous mock
 *  [[WsChannel]].
 *
 *  Concrete platform sub-classes (`jvm/JvmRelayTransportTest` and
 *  `js/JsRelayTransportTest`) just provide a [[mkTransport]] factory
 *  that builds the platform's relay-transport subclass.  The
 *  shared spec then asserts the same protocol behaviour both ways. */
abstract class RelayTransportTestBase extends AnyFunSuite:
  given ExecutionContext = ExecutionContext.parasitic

  /** Build a relay transport.  Platform sub-classes wire this to
   *  `new JvmRelayTransport(...)` / `new JsRelayTransport(...)` — both
   *  delegate to [[RelayTransportBase]] so the protocol assertions
   *  below are platform-agnostic. */
  protected def mkTransport(
    ws:       WsChannel,
  ...
  protected final class MockWsChannel extends WsChannel:
  ...
  protected def newJwtSeed(): Array[Byte] =
    CryptoBackend.get().randomBytes(32)

  /** Extract the params.message field from the rendered irn_publish
   *  JSON the channel observed. */
  protected def parseIrnPublish(text: String): (String /*topic*/, String /*message-b64*/, Int /*tag*/) =
  ...
  test("ApproveSession on a pairing topic produces a Type-1 envelope decrypting to wc_sessionSettle") {
    ...
    val tx    = mkTransport(ws, store, newJwtSeed())
  ...
  test("connect builds a ws URL with projectId + auth=<jwt>, and the JWT verifies") {
    ...
    val ws     = new MockWsChannel
    val tx     = mkTransport(ws, new WcSessionStore, seed, relayUrl = "wss://relay.example.com")
    tx.connect("proj-123")
  ...
  test("subscribe / unsubscribe emit irn_subscribe / irn_unsubscribe JSON-RPC frames") {
    val ws = new MockWsChannel
    val tx = mkTransport(ws, new WcSessionStore, newJwtSeed())
