...
| **C.3 — DataFrame ergonomics + schema bridge** | Eight landed slices so far: (1) `>10` binds via `Map.ofEntries[String, Object]` once `SparkGen.MapOfMaxPairs = 10` is exceeded; (2) widen the emitted `sparkImports` to include `Row`, `DataFrame`, and `types._`; (3) `spark-config:` front-matter map → sorted `.config(k, v)` lines on `SparkSession.builder()`; (4) `spark-app-name:` front-matter overrides `.appName(...)`; (5)+(6) typed reader convenience shims `Dataset.{fromParquet,fromJson,fromCsv}(path, options: (String, String)*): DataFrame` with variadic option pairs; (7) symmetric writer extension methods on `Dataset[T]` — `ds.toParquet/toJson/toCsv(path, opts*): Unit`; (8) adaptive default configs — `spark.ui.enabled=false`, `spark.sql.shuffle.partitions=4`, and the log4j WARN override are emitted **only** when `sparkMaster.startsWith("local")`; cluster targets (`spark://`, `yarn`, `k8s://`) get Spark's own defaults instead, since the local-mode picks (UI off, parallelism capped at 4) are wrong-by-default on multi-node clusters.  User `spark-config:` overrides still arrive whichever the master shape.  Still open: map `std/parsing` schemas to Spark `StructType`. | C.1+C.2 ship the binding/runtime contract; C.3 polishes the typed surface. | partially landed (slices 1–8) |
