...
/** Cross-platform spec for [[WcSessionStore]] — pure Scala, no crypto
 *  backend needed.  Lives in `shared/`; the per-platform `*Test` sub-
 *  classes just inherit. */
abstract class WcSessionStoreTestBase extends AnyFunSuite:
