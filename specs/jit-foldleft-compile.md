...
- **Slice C goal (`typeclassFoldMacro` win) — ACHIEVED ~19%, but via a SAFE interpreter memo, not the VM.**
  ...
  keyed by BOTH resolved given identities. Repeat calls skip those sub-expressions. **DEFAULT-ON**
  (kill-switch `-Dssc.jit.foldtc=0`) — caching the evaluated `empty` assumes a lawful, referentially-
  transparent monoid. Verified: `JitFoldTcTest` 8 differential tests (memo-on == memo-off, incl. a
  polymorphic two-given soundness case) + the full interp suite green WITH IT ON (1839 tests, excluding only
  the infra-flaky `CrossBackendPropertyTest`, which checks interp==JVM==JS result-equivalence that the memo
  preserves); `typeclassFoldMacro` **1.794 → 1.453 ms/op (~19%, non-overlapping error bars)**. The full VM
  Slice C stays unbuilt (disproportionate); the memo captures most of the win without touching the JIT.
