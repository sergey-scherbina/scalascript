...
Remaining UX/distribution work (not blocking the SPI mechanism):
  ...
  - **Plugin test harness** — ✅ **LANDED (2026-05-26)**:
    ...
    and `jsonPlugin` now have isolated `src/test` suites via `testUtils % Test`.
...
- **Post-migration follow-ons** (not blocking; spec §11 of
  ...
  - **Plugin test harness** — ✅ **LANDED (2026-05-26)**:
    ...
    `jsonPlugin` have isolated `src/test` suites via `testUtils % Test`.
    Remaining: migrate the other plugin families one at a time.
...
## v1.43 — Electron JVM REST Backend ✓ Complete (2026-05-26)
...
- **Phase 2 — Client/server split commands.** Support backend-only server launch
  ...
  - **Still open:** packaged/static client-only builds and runtime-level JVM
    bind-host support.
- **Phase 3 ✓ Landed (2026-05-26) — Fetch routing and mixed content.**
  Same-origin API fetches route to the JVM backend via the `__sscBackendBaseUrl`
  + `_spaFetchPath` overlay already present in the JsGen core runtime. No new
  runtime code was needed — the fetch overlay was already wired. Extended
  `InProcessBackendTransport` test coverage for `Response.text`, `Response.json`,
  explicit status codes (204), non-2xx pass-through (400), and 500 from handler
  exceptions.
- **Phase 4 ✓ Landed (2026-05-26) — Security token.**
  Per-run UUID generated by CLI at JVM REST startup; passed as `SSC_DESKTOP_TOKEN`
  env var to the JVM backend process. `injectDesktopTokenMiddleware` appends a
  `use { (req, next) => ... }` guard to the generated JVM script that reads the
  env var and rejects requests with a wrong or missing
  `x-scalascript-desktop-token` header (returns 401). Token injected as
  `globalThis.__sscDesktopToken` into the Electron bundle; fetch overlay in
  JsGen adds the header to every backend request automatically. CORS for
  distributed browser clients remains a planned open question.
- **Phase 5 ✓ Evaluated (2026-05-26) — Desktop build packaging: deferred.**
  `ssc build --target desktop-jvm` needs three independent workstreams: (1)
  scala-cli fat-JAR/native-image packaging for the JVM backend, (2) Electron
  side-car process startup from inside a packaged app, (3) platform signing
  (macOS notarization, Windows Authenticode, Linux AppImage). Each is substantial
  enough to warrant its own milestone. Dev-mode is complete; packaging is deferred.
...
- **Phase 11 — Typed data mapping across stores.** Add a `derives`-based codec
  ...
  case-class/enum response shapes. Follow-up landed 2026-05-26:
...
- **Phase 2 ✓ Landed (2026-05-25)** — static toolkit subset:
  lowered text/labels, buttons, text fields, checkboxes, vstack/hstack,
  spacers, dividers, scroll views, fragments, show/for snapshots, styled
...
- **Phase 0 ✓ Landed (2026-05-25)** — spec and backlog:
  ...
  behavior changes.
- **Phase 1 ✓ Landed (2026-05-25)** — minimal API client metadata:
  `apiClients:` / `api-clients:` front-matter declarations now parse into
  `ApiClientDecl` / `ApiEndpointDecl` AST metadata and JVM codegen emits
...
- **Phase 6 ✓ Landed (2026-05-26)** — advanced shapes:
  ...
  `maxRetries` times. 4xx errors are never retried.
