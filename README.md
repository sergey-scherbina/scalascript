...
| Typed data mapping | `backend/typed-data` now provides the shared emitted typed JSON facade for generated route clients plus explicit `JsonValue`, `DecodeError`, `JsonCodec[A]` helpers, and `derives JsonCodec` for case classes and sealed ADTs. Shared derives-based codecs will map these shapes to SQL rows, JSON/IndexedDB/ObjectStore documents, property graph vertices/edges, RDF triples, `Dataset[T]` elements, and Spark schemas/encoders without forcing one universal ORM |
