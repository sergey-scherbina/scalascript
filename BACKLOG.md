...
- [~] **meta-extractor-decouple** — **INVESTIGATED 2026-06-14 → NOT WORTH EXECUTING (downgraded).** Scoping the 949 sites: **~650 are in codegen** (jvm 271 / js 296 / rust 83) = COLD (runs once per module, alloc irrelevant); **`DispatchRuntime` the hottest dispatch is already 0** (converted by prior perf work); EvalRuntime's 79 are mostly `Term.Apply`/`ApplyInfix` of which several are AST **construction** (can't field-access) + transform/special-form arms — the genuinely hot per-eval sites were already converted (`fastPrimitiveValue` etc.). scalameta is **4.17.0** and `After_4_x` extractors are forward-compat BY DESIGN — code using them is already the version-robust choice, so a field-access helper adds no decoupling. Net: no measurable perf win remains (interp sweep verified at floor 2026-06-14), no real version risk, and converting cold/nested sites HURTS readability. **Only do a one-off conversion if a FUTURE profile flags a specific NEW hot After_4_x site** (the established `dv.pats`-style type-test + field-access pattern). Original framing (below) over-stated the value before the hot/cold split was measured. ORIGINAL (Tier 1 — RECOMMENDED FIRST) — **949 `scala.meta` `After_4_x`
...
- [~] **codegen-megafile-deflation** (Tier 1) — **SLICE 1 ✓ LANDED 2026-06-15**: extracted the content-toolkit emission domain (~1000 LOC → `JvmGenContentEmit`, `self: JvmGen =>` mixin) → **JvmGen.scala 5050→4022 (−20%)**, behaviour-identical (63 tests green). **SLICE 2 ✓ LANDED 2026-06-15**: same for JsGen → `JsGenContentEmit`, **JsGen.scala 5136→4486 (−13%)**, 224 tests green. FOLLOW-ON: JvmGen route/registry helper cluster (~500 LOC); JsGen has further clusters too. ORIGINAL — `JsGen.scala` (5136 LOC) and `JvmGen.scala`
...
- [~] **intrinsic-registration-harmonise** ✓ INVESTIGATED→WON'T-DO 2026-06-15 (justified asymmetry). The 27 js-only core intrinsics (crypto/uuid/graphql/json) are NOT a defect: each is an `extern def` declared once in `runtime/std/*.ssc` (e.g. crypto.ssc) and implemented per-backend via the PLATFORM-NATURAL mechanism — interp via plugin `NativeImpl` (CryptoIntrinsics), JS via a codegen intrinsic (backend/js intrinsics/Crypto.scala), JVM via runtime-call/`@jvm` (crypto is a shipped, busi-used JVM feature → no gap). The parity gate's allowlist CORRECTLY documents this. Harmonising = forcing artificial uniform registration onto a working, widely-used crypto/uuid/graphql path — disproportionate risk for test-allowlist tidiness, and it fights the externs-implemented-per-backend design. CLOSED. ORIGINAL (Tier 3, low-pri — surfaced by build-time-cross-backend-parity 2026-06-15) — JS bundles crypto/uuid/graphql/json intrinsics into its CORE `JsIntrinsics` table, while JVM delegates the same to PLUGINS (crypto/uuid/graphql/json-plugin). Same capability, different registration location → 27 documented allowlist exceptions in `CrossBackendIntrinsicParityTest`. HOW: pick one convention (both delegate to plugins, OR both keep in core) and align the two backends; then the parity gate could compare true resolved capability sets (core + overlays) and the allowlist shrinks toward zero. Pure consistency; no user-facing change. Verify the gate + all backend output suites stay green.
...
- [x] **declarative-ui-rowspath-jvm** — ✓ Landed (2026-06-12). `rowsPath` (the dotted
      fetch-envelope drill for Remote DataTables, Scope B.3) now works on the
      **server-rendered native path**, not just the JS browser SPA. The earlier deferral
      premise ("JVM emits no DataTable client JS") was wrong: both the interpreter
      `serve(view)`/`emit(view, dir)` and emit-jvm `serve` render the View via
      `FrontendFrameworks.current().emit()` = `StaticJsEmitter` (custom/default frontend),
      which DOES emit client fetch JS. Fix: carry `rowsPath` on the shared model
      (`TableDataSource.Remote(signal, rowsPath = "")`), thread it through the interp +
      JVM-codegen `fetchRowsSource`, and drill it in StaticJsEmitter via a shared
      `__ssc_rowsOf` helper (mirrors the browser `_ssc_ui_rowsOf`). Verified end-to-end
      (`ssc run` + `emit` → drilled `app.js`). Native Swing/JavaFX/SwiftUI toolkit tables
      (no client JS fetch layer) remain envelope-drill-free by design. Spec
      `specs/declarative-ui-scope-b.md` §B.3 v2.
...
- [x] **interp-opt-effect-stream** — ✓ Landed 2026-06-04.
      ...
      allocations when caller only needs .length (1cb6d3f0).
...
- [x] **conf-fix-triple-quote-macro** — ✓ Landed 2026-06-03. Fixed `preprocessQuotedMacros`
      so `skipString` handles triple-quoted strings correctly.

- [x] **conf-fix-parsing-stdlib** — ✓ Landed 2026-06-03. Fixed `recovery.ssc` import
      so `Parser`, combinators and recovery exports are in scope.

- [x] **conf-fix-mcp-types-import** — ✓ Landed 2026-06-03. `SectionRuntime.runInlineImports`
      now exposes package-namespaced symbols locally.

- [x] **conf-fix-http-client-js-jvm** — ✓ Landed 2026-06-03. `httpGet`/`httpPost`/`httpClient`
      implemented on JS and JVM backends; gated with `requires: HttpClient`.
...
- [x] **jsgen-split-p3** _(landed 2026-05-30)_ - Phase 3 of the `JsGen.scala` preamble-string extraction.
      ...
      call-site changes. All 6 are operands of the eager top-level `val JsRuntime`
...
- [x] **jvmgen-split-p2** _(landed 2026-05-30)_ - Phase 2 of the `JvmGen.scala` split: extract the three
