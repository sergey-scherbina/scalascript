...
| [Markdown Content Introspection](specs/markdown-content-introspection.md) | Planned `std/content` API for reading non-code Markdown body content as typed metadata and lowering it through `std/ui` without hand-written markup |
...
| Markdown content introspection | `contentDocument()`, `contentCurrentSection()`, `contentSection(id)`, and `contentView(...)` will expose prose/list/link/image Markdown body nodes as typed metadata and renderable frontend input. Phase 0 landed the spec, docs, example sketch, and pending conformance fixture; compiler/runtime support remains planned |
