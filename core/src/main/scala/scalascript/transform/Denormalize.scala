...
object Denormalize:
  ...
  private def content(c: ir.Content): ast.Content = c match
    ...
    case ir.Content.SqlBlock(source, _, dbName, sp) =>
      ...
      // back unchanged" with the `sql` lang tag.  `dbName` is
      // surfaced through the fence-attribute slot so the
      // interpreter and JvmGen-emitted code can read it via the
      // same `attrs.get("db")` channel the parser uses.
