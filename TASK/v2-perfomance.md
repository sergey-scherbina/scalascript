...
*3 entries matching `kind=perf status=open`, generated by `scripts/task-views` — do not edit inside the markers.*
...
**Not in that table yet, and this is the honest gap:** the analysis below covers 12 items, of which
only these carry a header. The other nine are prose in `v2/BACKLOG.md` (`v2m-*`, `v2rt-*`), and
`BACKLOG.md` has no header schema at all — so no query can see them. `scripts/bugs-report` prints
that deficit rather than quietly answering 3 of 12. Fixing it means giving backlog entries the same
header; that is queued, not done.
