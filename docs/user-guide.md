...
| `signalButton(signal, value, label, disabled, variant)` | Button that sets `signal` to `value` on click. `variant` (default `"primary"`) picks the background colour from `Theme.colors`: `primary\|secondary\|danger\|success\|warning`; an unrecognized string falls back to `"primary"`, no crash (`specs/std-ui-button-variant.md`) |
| `actionButton(handler, label, disabled, variant)` | Button wired to an `EventHandler` (`fetchAction`, `incSignal`, …). Same `variant` param as `signalButton` |
