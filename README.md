...
| Graph storage | `backend/graph` now provides `GraphCapabilities`, `PropertyGraphBackend`, `RdfGraphBackend`, `GraphBackend`, `GraphRuntime.inMemory()`, `GraphRuntime.tinkerGraph()`, and `GraphRuntime.rdf4jMemory()` for portable JVM property/RDF storage through typed codecs. `graphs:` front matter survives AST/IR/.sscc; JVM codegen emits a typed `Graph.*` facade for declared in-memory, `embedded-tinkergraph`, and `rdf4j-memory` graph stores, and `runtime/std/graph-plugin` mirrors the portable in-memory facade for `ssc run`. Production adapters such as Neo4j, JanusGraph/TinkerPop providers, and RDF4J-compatible repositories remain planned |
...
| [graph-rdf4j-storage.ssc](examples/graph-rdf4j-storage.ssc) | JVM `graphs:` front matter plus generated `Graph.*` facade over RDF4J memory RDF storage |
