...
/** MCP server + client intrinsics for the JS (Node.js) backend.
 *
 *  All symbols are JavaScript function declarations emitted into the MCP
 *  runtime preamble that JsGen prepends when `blocksUseMcp` fires.
 *  `RuntimeCall` entries satisfy `CapabilityCheck` so programs importing
 *  `std/mcp/server` or `std/mcp/client` are accepted by the JS backend. */
val JsMcpIntrinsics: Map[QualifiedName, IntrinsicImpl] = Map(
  // Server — setup + launcher
  QualifiedName("mcpServer")               -> RuntimeCall("mcpServer"),
  QualifiedName("serveMcp")                -> RuntimeCall("serveMcp"),
  // McpServer builder methods (called as srv.tool / srv.resource / srv.prompt)
