...
| [JVM Desktop Frontend](docs/jvm-desktop-frontend.md) | Partially implemented: `frontend-swing` backend, ServiceLoader registration, CLI `--frontend swing` selection, static Swing toolkit subset emission, and local signal action bridge; run mode remains planned |
...
| JVM desktop frontend | `--frontend swing` is recognized and the `frontend-swing` SPI can emit a JDK-only `JFrame` source artifact for a toolkit subset with local signal actions; full `ssc run --frontend swing app.ssc` desktop launch, backend transport dispatch, and JavaFX/Compose adapters remain planned |
...
| JVM desktop sub-backend | `frontend-swing` | Partially implemented JDK-only JVM desktop frontend. It provides SPI discovery, Swing source emission for text/buttons/fields/toggles/stacks/spacers/dividers/scroll views, basic style hints, and local signal actions; backend transport dispatch and `ssc run` desktop launch are planned. See [`docs/jvm-desktop-frontend.md`](docs/jvm-desktop-frontend.md). |
