...
object RustCodeWalk:
  ...
  def walk(
    ...
    // DISPATCH TRAITS, before anything else touches `mapType` — see `_dynTraitTparams`'s own
    // comment. `mapType` reads `_dynTraitTparams` on every call from here on, so it has to be set
    // before the FIRST one (`_paramTypes`, a few lines below).
    val dispatchTraits = collectDispatchTraits(module)
    _dynTraitTparams = dispatchTraits.map(t => t.name -> t.tparams).toMap
    _dispatchTraits  = dispatchTraits.map(t => t.name -> t).toMap
    // Position of every DEFAULT-bodied dispatch-trait member (`DialectAdapter`'s `def aliases: …
    // = Set.empty`) — a real `Defn.Def`, so the deep `collectDefs` scan below would ALSO pick it up
    // as an ordinary top-level def and emit it a second time as a bodyless-receiver free function
    // (`pub fn aliases() -> Vec<String>`, no `&self`, wrong AND a duplicate of the one
    // `renderDispatchTrait` emits correctly inside the trait itself). Filtered out the same way an
    // effect-op marker or a given-instance method already is, a few lines below.
    val dispatchTraitMemberPos: Set[Int] =
      dispatchTraits.flatMap { t =>
    ...
    val topValNames       = collectTopValNames(module)
    _ambiguousMembers     = ownerOfMember.groupBy(_._1).collect {
                              case (mem, os) if os.size > 1 || topLevelDefNames.contains(mem) || topValNames.contains(mem) => mem
                            }.toSet
    ...
    _zeroArgDefNames      = defs.filter { d =>
      ...
      d.paramClauseGroups.flatMap(_.tparamClause.values).isEmpty
    }.map(_.name.value).toSet ++
      // A dyn-dispatch trait's OWN zero-arg members (`Processor.start`, `DialectAdapter.id`) never
      // reach `defs` at all: an ABSTRACT one is a scalameta `Decl.Def` (`collectDefs` only collects
      // `Defn.Def`), and a DEFAULT one was deliberately filtered OUT above
      // (`dispatchTraitMemberPos`) so it renders once, inside the trait, not twice. Both still need
    ...
    // for `role` was invisible to `namedCtorAsPositional`, which had nowhere to look it up and
    ...
    // start` (`UniML.scala`'s `parse`) needs to know "start" is a call on THIS SPECIFIC struct, not
    // merely that SOME def named "start" exists somewhere — `_zeroArgDefNames`'s own no-field-
    // anywhere guard already refuses "start" globally, correctly, because `SourceSpan.start` is a
    // genuine field of a DIFFERENT type. See `collectLocalRustTypes` for the other half (naming
    // `vm`'s type in the first place) and the `renderTerm` case that reads both.
    _structZeroArgMethods = standaloneCases.map(c =>
      c.name.value -> c.templ.body.stats.collect {
        case dd: m.Defn.Def if dd.paramClauseGroups.flatMap(_.paramClauses).flatMap(_.values).isEmpty &&
          dd.paramClauseGroups.flatMap(_.tparamClause.values).isEmpty => dd.name.value
      }.toSet).toMap
    ...
    val structTparams = structOk.map(s => s.structName -> s.tparams).toMap
    ...
    // `(enumName, ctorName) -> EnumCtor`, built from the SAME `(ctorName, EnumCtor)` pairs as
    // `ctorMap` above but BEFORE its lossy `.toMap` — two Scala-3 `enum`s in one module are free to
    // share a case NAME (`TokenChannel`'s comma-shorthand `case …, Error` and `Severity`'s `case …,
    // Error`, both in `uniml`'s `Diagnostics.scala`/`Tree.scala`), and `ctorMap`, keyed by the bare
    // ctor name alone, keeps only the LAST one written — the other's `Severity.Error` (a QUALIFIED
    // reference, naming exactly which enum it means) then resolved through the WRONG entry, failed
    // its own `enumName` check, and silently fell to the generic field-select path: `Severity.Error`
    // emitted VERBATIM, not `Severity::Error` — `error[E0423]: expected value, found enum Severity`,
    ...
    // Dyn-dispatch trait declarations (`DialectAdapter`, `Processor[S, I, O]` — see
    // `_dynTraitTparams`) — the trait items themselves, plus one THIN FORWARDING `impl Trait for
    // X` per concrete `class`/`object` extending one. Only CLASSES are covered: an `object`
    // extending a dyn trait (`object Literal extends DialectAdapter`, `dialect/Literal.scala`)
    // renders exactly as any other object does today — a set of free qualified functions, no
    // struct — so it cannot receive a self-taking `impl` block. Narrower than the general case, but
    // sound: nothing in this lane's own output ever needs to hand `Literal` itself around as a
    // `Rc<dyn DialectAdapter>` value (only a CONSTRUCTED implementor like `LiteralProcessor` is,
    // at `Literal.instructions`'s own return), so an object implementor simply is not reachable as
    // one yet — a real gap, left for whenever a module actually needs it, not papered over here.
    val dispatchTraitRendered: List[Either[List[Diagnostic], String]] =
      dispatchTraits.map(dt => renderDispatchTrait(dt, userTypeNames, intrinsics, userDefs, ctorMap, topVals, effectfulDefs))
    val dispatchImplRendered: List[Either[List[Diagnostic], String]] =
      standaloneCases.flatMap { c =>
        c.templ.inits.flatMap { init =>
          parentTypeName(init).flatMap(_dispatchTraits.get).map { dt =>
            renderDispatchTraitImpl(c.name.value, init, dt, userTypeNames ++ structTparams.getOrElse(c.name.value, Nil))
          }
        }
      }
    ...
    if allErrs.nonEmpty then Left(allErrs)
      ...
      val implBlock =
        methodOk.groupBy(_._1).toList.sortBy(_._1).map { case (owner, ms) =>
          ...
          // method this lane emits clones its own fields liberally (`let values = self.values.
          // clone();`, `renderStruct`'s established convention), so a generic method needs the
          // bound or rustc refuses the call outright: `the method clone exists … but its trait
          // bounds were not satisfied`.
          val implTpDecl = if tp.isEmpty then "" else tp.map(t => s"$t: Clone").mkString("<", ", ", ">")
  ...
  private def contentDefs(c: ast.Content): List[m.Defn.Def] = c match
    case ast.Content.CodeBlock(lang, _, Some(node), _, _, _, _)
        if isScalaLang(lang) =>
      topLevelDefs(node.tree)
    case _ => Nil

  /** Like `tree.collect { case d: m.Defn.Def => d }`, but does not descend into a `Defn.Def`'s OWN
   *  body looking for more — a `def` nested inside another `def`'s body is a LOCAL def
   *  (`TreeVm.scala`'s `record`/`addTop`/`attach`/`closeFrame`, lifted out by `liftLocalDefs`
   *  during that OWNING def's own rendering, with a Ctx that actually knows what it captures), not
   *  an independent top-level/class-member one. The plain `.collect` swept it in anyway — scalameta
   *  does not distinguish "nested in a Template" from "nested in a def body" — so a local def was
   *  ALSO rendered a second time here, through the ordinary top-level pipeline, with a freshly built
   *  Ctx that had never heard of its captures: `attach`'s `stack.nonEmpty` read `ctx.localSeqs =
   *  Set()` and refused, even though `liftLocalDefs`'s OWN render of the very same def, moments
   *  earlier, had it right. Two renders of one def is also just wrong on its own terms — a class
   *  member's own body genuinely can hold ANOTHER member (nothing here relies on that not
   *  happening), so descent still continues everywhere else: a `Template`'s stats, a `Pkg`'s stats,
   *  and so on — only a `Defn.Def`'s `body` field specifically is not walked into. */
  private def topLevelDefs(t: m.Tree): List[m.Defn.Def] = t match
    case d: m.Defn.Def => d :: Nil
    case _              => t.children.flatMap(topLevelDefs)
  ...
  /** Just the NAMES `collectTopVals` would find — no rendering, so it can run BEFORE `ctorMap`/
   *  `intrinsics` exist, early enough to feed `_ambiguousMembers` (see its own call site): an
   *  object member DEF sharing a bare name with SOME topval (`ProcessBatch.empty` the def, `Dialect
   *  Registry.empty` the val, `Dialect.scala`'s actual pair) renders both as the SAME bare Rust
   *  identifier — the def as a free function, the topval as a local variable in whatever def's
   *  preamble references it — and whichever ends up in scope shadows the other silently. Without
   *  this, `_ambiguousMembers` only knew about a name TWO OBJECTS shared, not one an object and a
   *  topval did, and `batch: ProcessBatch.empty` compiled to `batch: empty()`, calling a
   *  `DialectRegistry` value as a function. */
  private def collectTopValNames(module: ast.Module): Set[String] =
    def fromContent(c: ast.Content): List[String] = c match
      case ast.Content.CodeBlock(lang, _, Some(node), _, _, _, _) if isScalaLang(lang) =>
        ...
        descend(topStats0).collect { case v: m.Defn.Val => v }.flatMap { v =>
          v.pats.collect { case m.Pat.Var(m.Term.Name(n)) => n }
        }
      case _ => Nil
    def fromSection(s: ast.Section): List[String] =
      s.content.flatMap(fromContent) ++ s.subsections.flatMap(fromSection)
    module.sections.flatMap(fromSection).toSet
  ...
  private def contentTopVals(
    ...
    case ast.Content.CodeBlock(lang, _, Some(node), _, _, _, _)
      ...
      // recursively descend into top-level objects to reach their vals. The owner
      // NAME travels with each stat — `TreeVm.scala`'s `object TreeVm: private val
      // finishedDiagnostic` and `Dialect.scala`'s `object DialectRegistry: private
      // val empty` are two DIFFERENT companion objects that happen to reuse a topval
      // name (`empty`), and `_topValOwners` (set below) is what lets a QUALIFIED
      // reference (`TreeVm.finishedDiagnostic`) tell them apart — see
      // `selectOrNiladicCtor`'s own comment for the collision this fixes.
  ...
  /** `(enumName, ctorName) -> EnumCtor` — see the fix comment at the assignment site (`generate`,
   *  right after `ctorMap` itself is built). `ctorMap` is keyed by bare ctor name and silently keeps
   *  only the LAST of two same-named cases across DIFFERENT `enum`s in one module; a QUALIFIED
   *  reference (`Severity.Error`) names which one it means, and this is what lets that name survive
   ...
   *  TreeVm: private val finishedDiagnostic` and `Dialect.scala`'s `object DialectRegistry: private
   *  val empty` both did, coincidentally) — `ctx.topVals` itself carries no owner, only `(name,
   ...
   *  `()` at the use site, e.g. `vm.start`). A `Term.Select` of this shape reaches the SAME code
   *  path as a genuine field read, and the fallback rendered it as one: `vm.start.clone()`,
   *  `error[E0615]: attempted to take value of method 'start'`. Used ONLY alongside a
   *  no-struct-has-this-field check (`ctorMap.values.exists(_.fieldNames.contains(field))`) at the
   *  call site — a name that IS some other struct's real field never enters the rewrite, so this
   *  set only widens what compiles and never turns a working field read into a bad call. */
  private var _zeroArgDefNames: Set[String] = Set.empty

  /** struct name -> its own zero-arg method names. See the assignment site's own comment
   *  (`classMethodNames`'s neighbour in `walk()`) for what this is for. */
  private var _structZeroArgMethods: Map[String, Set[String]] = Map.empty

  /** A `trait` that is NOT sealed (so not an ADT-as-enum — see `isSealedTrait`) and whose name is
   *  used somewhere as an ordinary VALUE TYPE — a param, field, or return type — rather than only
   *  as a `given … : Trait[…] with` typeclass tag. `DialectAdapter`/`Processor` in
   *  scalascript-uniml's `Dialect.scala`/`Processor.scala` are the two: `DialectRegistry` stores a
   *  `Map[String, DialectAdapter]` for genuine RUNTIME dispatch (`Literal` is one adapter among
   *  others a caller could register), which Rust has no structural-typing answer for — only a
   *  trait object.
   *
   *  Deliberately narrow, not "every non-sealed trait": a `given … : Monoid[Int] with` typeclass
   *  instance references its trait ONLY inside the `given` template (`collectGivens` reads
   *  `templ.inits`, a separate AST location this collector's scan does not descend into — see
   *  `collectDynTraitNames`), and that machinery already has its own working answer (a plain
   *  struct + inherent impl, never `impl Trait for X`). Treating every std-library typeclass trait
   *  as a dyn-dispatch trait too would emit trait code nothing calls and risk a shape this narrow
   *  renderer cannot handle, on modules that compile today specifically because an unused trait
   *  declaration renders to nothing.
   *
   *  name -> the trait's own type parameter names, e.g. "Processor" -> List("S", "I", "O"). */
  private var _dynTraitTparams: Map[String, List[String]] = Map.empty

  /** The full collected shape of each dyn-dispatch trait (see `_dynTraitTparams`), keyed by name —
   *  populated once, alongside `_dynTraitTparams`, and read by `renderDispatchTrait` (the trait's
   *  own Rust `trait` item) and `renderDispatchTraitImpl` (a concrete implementor's forwarding
   *  `impl Trait for X` block). */
  private var _dispatchTraits: Map[String, DispatchTrait] = Map.empty

  // A STRING LITERAL matched against a struct FIELD (`QName(None, "xmlns", _)` /
  // `UniNode.Branch("xml.element", _, _, _)`, `uniml/xml`'s `Doc.scala`) — the field's Rust type is
  // an OWNED `String`, and only `&str`/`str` support a literal pattern at all, so `renderPattern`'s
  // OWN `Lit.String` case (correct where the MATCH SUBJECT itself is a string) is not valid syntax
  // here: `error[E0308]: expected String, found &str`. `renderPattern`'s signature carries only
  // pattern TEXT, with no channel back to the enclosing match ARM (which alone can add an `if`
  // guard) — so the with-fields-ctor Extract case binds such a field to a FRESH name instead and
  // pushes an equality guard here; every call site that renders ONE ARM's whole pattern resets this
  // list first, then reads it back to splice into that arm's own guard. Safe as a per-arm scratch
  // channel (not a structural risk despite being mutable state) because `renderPattern` for one
  // arm's pattern always completes, synchronously and to exhaustion, inside the ONE top-level call
  // each site makes before reading this back — nothing else runs between reset and read.
  private var _pendingPatternGuards: List[String] = Nil
  private var _pendingPatternGuardCounter: Int = 0
  ...
  private def renderStruct(
    ...
    // TYPE PARAMETERS, emitted rather than dropped — the same gap `renderEnumCase` had (see its own
    // fix comment): `final case class ProcessBatch[A](values: Vector[A], …)` rendered `pub struct
    // ProcessBatch { … }` with no `<A>` at all, so `values: Vec<A>` (correctly typed, since `A` now
    // joins `typeNames` below) referenced a type parameter the STRUCT ITSELF never declared — every
    // call site saying `ProcessBatch<i64>` then hit `error[E0107]: struct takes 0 generic arguments`,
    // in `uniml`'s `Processor.scala`/`TreeVm.scala`, whose whole subject is this shape.
    val tparams  = c.tparamClause.values.map(_.name.value).toList
    ...
    val fieldRendered = params.map { p =>
      p.decltpe match
        case Some(t) => mapType(t, s"struct $name", typeNames ++ tparams, inField = true).map(r => (p.name.value, r))
    ...
    if errs.nonEmpty then Left(errs.flatten)
      ...
      // A field typed by a bare type PARAMETER (`values: Vec<A>`) has no known Copy-ness — `A` might
      // be `i64` or might be `Diagnostic` — so a generic struct never claims `Copy`, the same
      // conservative default a truly-unknown field type would get.
      val allCopy = tparams.isEmpty && ok.forall((_, t) => Set("i64", "f64", "bool").contains(t))
      // `PartialEq` alongside the enum fix (see that arm's own comment): a user enum holding a
      // struct FIELD (`UniNode::Branch(edges: Vec<UniEdge>, span: SourceSpan, …)`) only derives its
      // own `PartialEq` correctly if EVERY field type does too — a struct without it here is what
      // turned `severity == Severity.Fatal`'s fix into a NEW `error[E0369]` one level down, on
      // `SourceSpan`/`SourceToken`/`Vec<UniEdge>`/`Vec<FrameSpec>` instead.
      val derives0 = if allCopy then "Debug, Clone, Copy, PartialEq" else "Debug, Clone, PartialEq"
      ...
      // representation — same walls a `Rc<dyn UserTrait>` field hits (`DialectRegistry.byName:
      // Map[String, DialectAdapter]`, `Dialect.scala`). Same answers, for the same reasons — see
      // `enumDebugImpl` and `enumLift`. `.contains`, not `.startsWith`: `DialectRegistry`'s field
      // isn't bare `Rc<dyn …>`, it is `HashMap<String, Rc<dyn DialectAdapter>>` — the problem type
      // NESTED inside a collection — and a prefix check missed it (`error[E0277]: (dyn
      // DialectAdapter + 'static) doesn't implement Debug`, from a struct that still tried to
      // derive one).
      val structHasClosure = ok.exists((_, t) => t.contains("std::rc::Rc<dyn "))
      val structDebug =
        if !structHasClosure then ""
                ...
                |}
                |""".stripMargin
      // The `Any`-boundary `From<T> for Value` conversion, SKIPPED for a generic struct
      // (`tparams.nonEmpty`): a field typed by a bare parameter (`x.values: Vec<A>`) needs `A:
      // Into<Value>` (or similar) as an impl bound this lane does not compute, and unlike the
      // struct HEADER's own `<A>` — needed just for the type to exist at all — nothing in `uniml`
      // (or any other reachable module) actually converts a `ProcessBatch`/`Stepped` into a
      // `Value`, so there is nothing to get right here yet, only a shape that would be wrong if
      // guessed.
      val lift =
        if tparams.nonEmpty then ""
        else
          val fieldLifts = ok.map { (n, t) =>
            if t.contains("std::rc::Rc<dyn ") then "crate::value::Value::Unit"
   ...
   */
  /** `"std::rc::Rc<dyn DialectAdapter>"` / `"std::rc::Rc<dyn Processor<String, SourceChunk,
   *  VmToken>>"` -> `Some("DialectAdapter")` / `Some("Processor")` — the trait name a `mapType`-
   *  rendered dyn-dispatch type names, read back off the Rust type STRING (there is no AST handy
   *  at the call site this feeds — see its own use). `None` for anything else. */
  private def dynTraitNameOfRustType(t: String): Option[String] =
    val prefix = "std::rc::Rc<dyn "
    if !t.startsWith(prefix) then None
    else
      val rest = t.stripPrefix(prefix)
      val name = rest.takeWhile(c => c != '<' && c != '>')
      if name.isEmpty then None else Some(name)

  /** Render a dyn-dispatch trait itself as a genuine Rust `pub trait` — see `_dynTraitTparams`'s
   *  own comment for what qualifies as one. The `impl` blocks that let a CONCRETE type satisfy it
   *  come separately, from `renderDispatchTraitImpl`; this only emits the trait declaration —
   *  `fn name(&self, …) -> T;` for an abstract member (a scalameta `Decl.Def`, no Scala body), or
   *  `fn name(&self, …) -> T { body }` for one with a default (`DialectAdapter`'s `def aliases:
   *  Set[String] = Set.empty`). */
  private def renderDispatchTrait(
      dt: DispatchTrait, enumNames: Set[String],
      intrinsics: Map[QualifiedName, IntrinsicImpl], userDefs: Set[String],
      ctorMap: Map[String, EnumCtor], topVals: List[TopVal], effectfulDefs: Set[String]
  ): Either[List[Diagnostic], String] =
    // The trait's OWN type parameters resolve as themselves — same trick `structTparams`/
    // `ownTparams` use for a generic struct/method: `mapType`'s `enumNames.contains(n) => Right(n)`
    // case (a plain name-set membership test) sees "S" here exactly as it would see a real
    // user-type name, rather than falling to the generic-type-parameter `i64` default.
    val ownNames = enumNames ++ dt.tparams.toSet
    val memberResults = dt.members.map { mem =>
      val defLabel = s"${dt.name}.${mem.name}"
      for
        paramsRs <- traverseTypes(mem.params.map(_._2), defLabel, ownNames)
        retRs    <- mapType(mem.ret, defLabel, ownNames)
    ...
    val (errs, ok) = memberResults.partitionMap(identity)
    if errs.nonEmpty then Left(errs.flatten)
    else
      val tparamDecl = if dt.tparams.isEmpty then "" else dt.tparams.mkString("<", ", ", ">")
      Right(
        s"""#[allow(dead_code)]
           |pub trait ${dt.name}$tparamDecl {
           |${ok.mkString("\n")}
           |}
           |""".stripMargin
      )

  /** Render a THIN FORWARDING `impl Trait for Concrete` for one class/object `extends` relationship
   *  to a dyn-dispatch trait — every method just calls the SAME-NAMED inherent method the ordinary
   *  struct/method pipeline already rendered (`renderStruct` + the per-owner `impl` block built in
   *  `walk()`). Safe without any duplicate-body machinery: Rust's method resolution always prefers
   *  an INHERENT method over a trait method, including from inside that very trait's own `impl`
   *  block, so `self.start()` here calls the real inherent `TreeVm::start`/`LiteralProcessor::
   *  start`, never itself — no infinite recursion, and the body is written once. */
  private def renderDispatchTraitImpl(
      implTypeName: String, init: m.Init, dt: DispatchTrait, enumNames: Set[String]
  ): Either[List[Diagnostic], String] =
    val concreteArgs: List[m.Type] = init.tpe match
      case m.Type.Apply.After_4_6_0(_, argClause) => argClause.values.toList
      case _                                       => Nil
    val subst = dt.tparams.zip(concreteArgs).toMap
    val methodResults = dt.members.map { mem =>
      val substParams = mem.params.map((n, t) => n -> substType(t, subst))
      val substRet     = substType(mem.ret, subst)
      for
        paramsRs <- traverseTypes(substParams.map(_._2), implTypeName, enumNames)
        retRs    <- mapType(substRet, implTypeName, enumNames)
      yield
        val plist = substParams.map(_._1).zip(paramsRs).map((n, t) => s"$n: ${paramType(t)}").mkString(", ")
        val args  = substParams.map(_._1).mkString(", ")
        val sig   = if retRs.isEmpty then s"fn ${rustIdent(mem.name)}(&self${if plist.isEmpty then "" else ", " + plist})"
                    else               s"fn ${rustIdent(mem.name)}(&self${if plist.isEmpty then "" else ", " + plist}) -> $retRs"
        s"    $sig { self.${rustIdent(mem.name)}($args) }"
    }
    for
      argsRs  <- traverseTypes(concreteArgs, implTypeName, enumNames)
      methods <- { val (errs, ok) = methodResults.partitionMap(identity); if errs.nonEmpty then Left(errs.flatten) else Right(ok) }
    yield
      val header = if argsRs.isEmpty then dt.name else s"${dt.name}<${argsRs.mkString(", ")}>"
      s"""impl $header for $implTypeName {
         |${methods.mkString("\n")}
         |}
         |""".stripMargin
    ...
    val arms = ctors.map { (ctor, ec) =>
      ...
      if ec.fieldNames.isEmpty then
        ...
        val lifts = ec.fieldNames.zipWithIndex.map { (n, i) =>
          ...
          if ec.fieldTypes.lift(i).exists(_.contains("std::rc::Rc<dyn ")) then "crate::value::Value::Unit"
  ...
  private def ctorNameOfExpr(t: m.Term, ctx: Ctx): Option[String] = t match
    ...
    case m.Term.Select(qual, m.Term.Name(field)) =>
      for
        ...
        if idx >= 0
        // `document.root.name` (`uniml/xml`'s `Doc.scala`'s `parse`) — `root: Element`, a field
        // whose OWN type is ANOTHER variant of the same enum `Doc` belongs to; `ec.fieldTypes`
        // already collapsed it to `Node` (the ENUM name — `mapType`'s own deliberate rule, since
        // there is no Rust type for "just the Element case"), so the OLD single-source lookup below
        // could never recover "Element" specifically and this WHOLE chain stopped resolving one
        // field short of where it needed to. `fieldCtorNames` is the RAW, pre-collapse answer for
        // exactly this: tried FIRST, since it is strictly more precise wherever it has an entry.
        result <- ec.fieldCtorNames.get(field).filter(ctx.ctorMap.contains)
  ...
  private def contentAllTraits(c: ast.Content): List[m.Defn.Trait] = c match
    case ast.Content.CodeBlock(lang, _, Some(node), _, _, _, _) if isScalaLang(lang) =>
      node.tree.collect { case d: m.Defn.Trait => d }.toList
    case _ => Nil

  /** Every `Type.Name`/`Type.Apply` occurrence anywhere in the module whose name is one of
   *  `traitNames`, EXCEPT inside a `given … with` template — see `_dynTraitTparams`'s own comment
   *  for why that exclusion is what keeps a typeclass trait (`Monoid`, `Eq`, …) from being mistaken
   *  for a genuine dynamic-dispatch one; a typeclass names its trait ONLY inside the `given`
   *  template (`collectGivens` reads `templ.inits`), so pruning that subtree here is enough to
   *  leave it alone without having to special-case it by name. */
  private def collectDynTraitNames(module: ast.Module, traitNames: Set[String]): Set[String] =
    if traitNames.isEmpty then Set.empty
    else
      val used = scala.collection.mutable.Set.empty[String]
      def noteType(t: m.Type): Unit = t match
        case m.Type.Name(n) if traitNames.contains(n) => used += n
        case m.Type.Apply.After_4_6_0(m.Type.Name(n), _) if traitNames.contains(n) => used += n
        case _ => ()
      def scan(t: m.Tree): Unit = t match
        case _: m.Defn.Given => ()
        case tp: m.Type =>
          noteType(tp)
          tp.children.foreach(scan)
        case other => other.children.foreach(scan)
      def content(c: ast.Content): Unit = c match
        case ast.Content.CodeBlock(lang, _, Some(node), _, _, _, _) if isScalaLang(lang) => scan(node.tree)
        case _ => ()
  ...
  /** Collect every genuine dynamic-dispatch trait — see `_dynTraitTparams` for the full rationale
   *  and `_dispatchTraits` for how the result is used. A trait with any `@rust(...)` extern member
   *  is left to the EXISTING extern-class-trait mechanism (`collectExternClasses`'s own `Defn.Trait`
   *  case) rather than double-handled here. */
  private def collectDispatchTraits(module: ast.Module): List[DispatchTrait] =
    val allTraits = module.sections.flatMap(sectionAllTraits).filterNot(isSealedTrait)
      .filterNot(t => t.templ.body.stats.exists { case dd: m.Defn.Def => isExternBody(dd.body); case _ => false })
    if allTraits.isEmpty then Nil
    else
      val dynNames = collectDynTraitNames(module, allTraits.map(_.name.value).toSet)
      allTraits.filter(t => dynNames.contains(t.name.value)).map { t =>
        val tparams = t.tparamClause.values.map(_.name.value).toList
        val members = t.templ.body.stats.toList.flatMap {
          case dd: m.Decl.Def =>
            val params = dd.paramClauseGroups.flatMap(_.paramClauses).flatMap(_.values)
              .flatMap(p => p.decltpe.map(p.name.value -> _))
            Some(DispatchTraitMember(dd.name.value, params, dd.decltpe, None))
          case dd: m.Defn.Def =>
            val params = dd.paramClauseGroups.flatMap(_.paramClauses).flatMap(_.values)
              .flatMap(p => p.decltpe.map(p.name.value -> _))
            dd.decltpe.map(ret => DispatchTraitMember(dd.name.value, params, ret, Some(dd.body)))
          case _ => None
        }
        DispatchTrait(t.name.value, tparams, members)
      }
  ...
  private def renderEnum(e: m.Defn.Enum, userTypeNames: Set[String]): Either[List[Diagnostic], GeneratedEnum] =
    val enumName = e.name.value
    val cases    = e.templ.body.stats.collect { case c: m.Defn.EnumCase => c }
    // `case Syntax, Trivia, Comment, ...` (Scala 3's shorthand for several NULLARY cases under one
    // `case`) does not parse as `Defn.EnumCase` at all — scalameta gives it its own node,
    // `Defn.RepeatedEnumCase(mods, cases: List[Name])`, so `.collect[Defn.EnumCase]` above silently
    // saw zero cases for an enum that plainly has some, and reported "no case variants" — a parse
    // success mis-diagnosed as an empty enum. `case A` / `case B` one per line already worked; only
    // the comma-shorthand was missing this second case-node type entirely.
    val repeated = e.templ.body.stats.collect { case c: m.Defn.RepeatedEnumCase => c.cases.map(_.value) }.flatten
    ...
    if cases.isEmpty && repeated.isEmpty then
      Left(List(unsupported(s"enum `$enumName` has no `case` variants")))
    else
      // Each name from the comma-shorthand is a NULLARY case by construction (Scala 3 does not
      // allow constructor args on `case A, B, C` — there is nowhere to attach them), so its
      // rendering is exactly `renderEnumCase`'s own empty-fields branch, reproduced directly rather
      // than synthesizing a fake `Defn.EnumCase` tree node just to hand it back to that function.
      val repeatedRendered: List[Either[List[Diagnostic], (String, (String, EnumCtor))]] =
        repeated.map(name => Right((name, (name, EnumCtor(enumName, Nil, fieldTypes = Nil)))))
      val rendered = cases.map(c => renderEnumCase(enumName, c, tparams.toSet ++ userTypeNames)) ++ repeatedRendered
      ...
      if errs.nonEmpty then Left(errs.flatten)
        ...
        val hasClosure = ctors.exists(_._2.fieldTypes.exists(_.contains("std::rc::Rc<dyn ")))
        // `PartialEq`, so `severity == Severity.Fatal` (`TreeVm.scala`, several places) actually
        // compiles — `error[E0369]: binary operation == cannot be applied`. Every OTHER field type
        // this lane emits already derives it transitively (String, Vec<T>, Option<T>, another user
        // enum/struct — all standard-library or this-lane-generated, and this lane's OWN structs/
        // enums derive PartialEq the same unconditional way structs do below), so it costs the same
        // nothing `Debug`/`Clone` already cost; only a closure field, which has none of the three,
        // is excluded, same set as the other two.
        val enumDerives = if hasClosure then "Clone" else "Debug, Clone, PartialEq"
  ...
  private def renderClassCtor(
    ...
    if errs.nonEmpty then Left(errs.flatten)
      ...
      val fieldCtorNames = params.flatMap { p =>
        p.decltpe.collect {
          case m.Type.Name(tn)                   => p.name.value -> tn
          case m.Type.Select(_, m.Type.Name(tn)) => p.name.value -> tn
        }
      }.toMap
      Right((variant, (ctor, EnumCtor(enumName, ok.map(_._1), boxed, fieldTypes = ok.map(_._2), fieldCtorNames = fieldCtorNames))))
  ...
  private case class Ctx(
      ...
      // Local val/var names bound directly to a `.charAt(i)` call, so they hold `SscChar` — see
      // `yieldsSscChar`, which this set extends to a NAME, not just the syntactic call itself
      // (`val c = s.charAt(i); f(c)` needs the same `.0` coercion `f(s.charAt(i))` already gets;
      // before this the binding erased the fact, the same failure mode `localStrings` exists to
      ...
      // The owning case class's OWN field names — `TreeVm(limits: Limits)`'s `limits`, read inside
      // `step`/`stop` with no `self.` prefix (Scala's implicit-receiver access). `selfMethod` binds
      // these as `let limits = self.limits.clone();` in the RENDERED TEXT, after this Ctx is built —
      // a local-def lift (below) needs to know they exist at the AST-analysis stage too, since a
      // lifted def is a Rust nested `fn` that cannot see that `let` any more than it can see any
      // other enclosing local. Read-only (`selfMethod` always clones), so it joins a lift's capture
      // pool the same way a `val` does.
      selfFields: Set[String] = Set.empty,
      // Local-def lambda-lifting (below): names that are, IN THIS Ctx, already a Rust `&mut`
      // reference — either a lifted def's own captured-`var` parameter, or (not yet used) some
      // future by-ref parameter. `Term.Name` derefs these on every read AND on assignment
      // (`(*n) = …`, `(*n) += …`), because unlike a method call (which autoderefs through `&mut`
      ...
      // its OWN signature, and that T has to come from somewhere when the `var` itself carries no
      // annotation (`var stack = state.stack`, not `var stack: Vector[VmFrame] = …` — see
      // `TreeVm.scala`). Only the def's OWN params are here; a capture initialized from a chain of
      // locals rather than directly off a param is out of scope for this narrow inference.
      paramTypes: Map[String, String] = Map.empty,
      // param name -> the SPECIFIC enum-case/ctor name, for a param declared with a QUALIFIED
      // variant type (`instruction: VmInstruction.Reframe`, `reframeProblem`'s own signature).
      // `paramTypes` holds the RUST TYPE of such a param — the owning ENUM's name ("VmInstruction",
      // via `mapType`'s `_variantOwner` case), which is right for the signature but wrong as a
      // `ctorMap` key: `ctorMap` is keyed by CTOR name ("Reframe"), and only the Reframe variant
      // actually has a `closeBefore` field. Without this, `instruction.closeBefore`'s type could
      // not be resolved at all — `ctorMap.get("VmInstruction")` is simply absent.
      paramCtorNames: Map[String, String] = Map.empty,
      // The enclosing def's OWN top-level body statements (`Nil` for a non-block body). Lets
      // `elementTypeOf` place a local's declaration (`val closeBefore = instruction.closeBefore`)
      // from ANYWHERE inside the def — a condition, a nested `if` — without threading a fresh
      // `stats` list through every call. Whole-def-body scoped, same as `paramTypes`/`selfFields`;
      // a local declared inside a DEEPER nested block is out of reach, same limit `inferCaptureType`
      // already has for `liftLocalDefs`.
      bodyStats: List[m.Stat] = Nil,
      // Match-arm binder name -> the SPECIFIC variant it narrowed to — but ONLY when `renderPattern`
      // ACTUALLY DESTRUCTURED the fields as bare local bindings (`r @ VmInstruction::Reframe {
      // open, closeBefore, … }`), unlike `paramCtorNames` above, which also covers a def PARAMETER
      // declared with a qualified variant type (`instruction: VmInstruction.Reframe`) where NO
      // destructuring happens — Rust has no per-variant subtype for a plain parameter to narrow
      // to, so `instruction.closeBefore` there is still just a (broken) field access, and rewriting
      // it to a bare `closeBefore` would reference a binding that was never created:
      // `error[E0425]: cannot find value closeBefore` — reached and reverted by testing this exact
      // rule against `reframeProblem`'s OWN body, which shares `instruction`'s ctor name with the
      // match arms it calls into but shares NONE of their bindings. Kept separate from
      ...
      // (`Dialect.scala`'s `DialectRegistry.apply`): `isEitherExpr` recognises `Right(empty)` (the
      // zero value) but `result` inside the closure body is a bare name with no syntax of its own
  ...
  private case class EnumCtor(
      ...
      // field name -> the RAW ctor name its decltpe names, BEFORE `mapType` collapses a variant
      // reference to its owning ENUM name (`Doc.root: Element` maps to `fieldTypes`' "Node", losing
      // "Element" specifically — see `ctorNameOfExpr`'s own comment for why that loss matters).
      // Populated whenever a field's declared type is a bare or qualified NAME (never for
      // List[...]/Option[...]/... — those never name a single ctor at all); the consumer still
      // checks `ctx.ctorMap.contains(...)` at USE time, since ctorMap is not yet complete while
      // this very entry is being built.
      fieldCtorNames: Map[String, String] = Map.empty
  )
  ...
  private case class GeneratedStruct(
      ...
      fieldNames: List[String],
      fieldTypes: List[String] = Nil,
      // The struct's OWN type parameters, so the `impl` block assembled later (companion-object
      // methods like `ProcessBatch.empty[A]`) can open with `impl<A> ProcessBatch<A>` instead of
      // bare `impl ProcessBatch` — see that assembly site's own comment for the error this fixes.
      tparams:    List[String] = Nil
  )

  /** One member of a dyn-dispatch trait (see `_dynTraitTparams`) — either abstract (a scalameta
   *  `Decl.Def`, `default = None`) or carrying a default body (a `Defn.Def`, `default = Some(…)`,
   *  e.g. `DialectAdapter`'s `def aliases: Set[String] = Set.empty`). Params/return are the RAW
   *  (unsubstituted) scalameta types, still naming the trait's OWN type parameters — substituted
   *  per-implementor by `renderDispatchTraitImpl` via `substType`. */
  private case class DispatchTraitMember(
      name:    String,
      params:  List[(String, m.Type)],
      ret:     m.Type,
      default: Option[m.Term]
  )

  private case class DispatchTrait(
      name:    String,
      tparams: List[String],
      members: List[DispatchTraitMember]
  )
  ...
  private def renderDef(
      ...
      // The owning case class's OWN field name -> Rust type, for a METHOD def only (empty for a free
      // def — the default, so the one free-def call site is unchanged). `selfMethod` binds these
      // later as `let f = self.f.clone();`, over the RENDERED TEXT this call produces; a local-def
      // lift (`liftLocalDefs`, inside `renderBody`) needs the same names — and, unlike `selfMethod`,
      // their TYPES too, since a lifted def is a Rust nested `fn` that cannot see that `let` any
      // more than any other enclosing local, and its own signature needs an explicit type for
      // whatever it captures. Threaded into `Ctx.selfFields`/`Ctx.paramTypes` here, one level up.
      ownFieldTypes: Map[String, String] = Map.empty,
      // The owning GENERIC struct's OWN type parameters (`ProcessBatch[A]`'s `A`, for a method of
      // `ProcessBatch` — `Processor.scala`'s `def map[B](f: A => B): ProcessBatch[B] = …`). Folded
      // into a LOCAL ctorMap extension below (self-mapping `EnumCtor` entries, so `mapType`'s own
      // `enumNames.contains(n) => Right(n)` case sees them as known type names) rather than into
      // `Ctx` directly — `Ctx.enumNames` is DERIVED from `ctorMap`, not a stored field, so this is
      // the one place that can widen it without restructuring every other reader of it. Without
    ...
    // The def's OWN type params (`def map[B](...)`'s `B`) join `ownTparams` (the OWNING struct's
    // `A`) — see this function's own `ownTparams` doc for why a self-mapping `EnumCtor` entry per
    // name is what lets `mapType` treat them as known types instead of falling to `i64`.
    val allTparams = (ownTparams ++ d.paramClauseGroups.flatMap(_.tparamClause.values).map(_.name.value)).distinct
    val ctorMap: Map[String, EnumCtor] =
      ctorMap0 ++ allTparams.map(t => t -> EnumCtor(t, Nil, fieldTypes = Nil)).toMap
    ...
    if isExternBody(d.body) then
      ...
      // level, never touching `collectLocalMaps`/`collectLocalSeqs`, which only ever scan the def's
      // OWN local var/val statements). Without this, a self-field typed `Map`/`Vec` read the SAME
      // way as any other unplaced name and `byName.contains(id)` took the STRING lowering —
      // `error[E0599]: no method named contains found for struct HashMap` — the map-specific
      ...
      val ctx     = Ctx(intrinsics, userDefs, ctorMap, topVals, name, effectfulDefs, lseqs ++ paramSeqs ++ selfSeqFields, larrays, lstrings,
                        ...
                        paramTypes = d.paramClauseGroups.flatMap(_.paramClauses).flatMap(_.values).flatMap { p =>
                          p.decltpe.flatMap(t => mapType(t, name, ctorMap.values.map(_.enumName).toSet).toOption)
                            .map(p.name.value -> _)
                        }.toMap ++ ownFieldTypes ++ collectLocalRustTypes(d, ctorMap),
                        paramCtorNames = {
                          val fromParams = d.paramClauseGroups.flatMap(_.paramClauses).flatMap(_.values).flatMap { p =>
                            p.decltpe.collect {
                          ...
                          // def parameter's own ctor name (`stack`'s tuple literal is built from
                          // `root`, itself a parameter) — seeded with `fromParams` alone, not the
                          // still-being-built `ctx` this whole block is inside of.
                          val seedCtx = Ctx(intrinsics, userDefs, ctorMap, topVals, name, effectfulDefs,
                            paramCtorNames = fromParams)
                          fromParams ++ collectTupleDestructureCtorNames(d.body, seedCtx)
                        },
                        // A PARAMETER declared with a qualified WITH-FIELDS variant type
                        // (`reframeProblem(instruction: VmInstruction.Reframe, …)`, `TreeVm.scala`)
                        // — the exact match-arm gap `destructuredCtorNames` exists to close (see
      ...
      yield
        ...
        // struct's `A`, already in scope via the `impl<A: Clone>` header this method sits inside —
        // would double-declare and shadow it as a SEPARATE, unrelated `A` if repeated here).
        val ownTp = d.paramClauseGroups.flatMap(_.tparamClause.values).map(_.name.value).toList
        val ownTpDecl = if ownTp.isEmpty then "" else ownTp.mkString("<", ", ", ">")
        val signature = if ret.isEmpty then s"pub fn ${rustIdent(name)}$ownTpDecl($params)"
                        else                s"pub fn ${rustIdent(name)}$ownTpDecl($params) -> $ret"
        ...
        // `reframeProblem(instruction: VmInstruction.Reframe, …)` — the parameter's RUST type is
        // still the whole enum (`VmInstruction`; Rust has no per-variant subtype a parameter could
        // narrow to), so `instruction.closeBefore` needs a REAL destructure to exist at all — this
        // is that destructure, a preamble `let-else` binding each field under its own name (`else
        // { unreachable!() }` because Rust's pattern IS refutable against the full enum type, even
        // though the CALLER — by Scala's own typing — never passes anything but a Reframe). Mirrors
        // `renderPattern`'s identical rewrite for a match-arm binder of this same shape; see its
        // comment for why a bare `.field` access cannot be expressed any other way.
        val paramDestructurePreamble =
          paramVariantDestructures.toList.sortBy(_._1).map { (p, ctorName) =>
            val ec = ctx.ctorMap(ctorName)
            // `doc: Markup.Doc`'s own `root: Elem` field (`uniml/xml`'s `Doc.scala`'s `serialize`)
            // is DIRECTLY RECURSIVE (`Elem` is a sibling variant of the SAME `Node` enum `Doc`
            // itself belongs to), so the struct boxes it (`ec.boxedFields`) — binding it straight
            // into a pattern gives `root: Box<Node>`, and every USE afterward expected the plain
            // `Node` the Scala field declares: `error[E0308]: expected Node, found Box<Node>`. Every
          ...
          }.mkString("\n") + (if paramVariantDestructures.isEmpty then "" else "\n")
        ...
        // A def DECLARED to return a dyn-dispatch trait (`Literal.instructions`'s `Processor[
        // String, SourceChunk, VmToken]`, `dialect/Literal.scala`) still just CONSTRUCTS the
        // concrete implementor in its body (`LiteralProcessor(source.source)`) — Scala's implicit
        // upcast to the trait type has no Rust equivalent, `Rc::new` is the explicit spelling of
        // it, and a Rust block expression's own value is its last expression, so wrapping the
        // WHOLE rendered body (single expression or a multi-statement `{ …; expr }` block, either
        // way) is correct regardless of shape. Unconditional on `ret` alone rather than checked
        // against the body's own shape: nothing in this corpus returns an ALREADY-`Rc`-wrapped
  ...
  private def collectLocalSeqs(
      body: m.Term, seed: Set[String], seqFields: Map[String, Set[String]]
    ...
    def seqCtor(rhs: m.Term): Option[Boolean] = rhs match  // Some(isArray) iff a seq ctor
      ...
      // `step`/`stop` fell to the field path and rustc said `no field 'nonEmpty' on type
      // Vec<VmFrame>` — this codegen's OWN "unlowered no-paren member" refusal, reported on a
    ...
    // `map`/`flatMap` change the ELEMENT type, not the "is this indexable" fact this set answers —
    // `var kinds = stack.map(_.kind)` (`TreeVm.scala`'s `reframeProblem`) left `kinds` untracked, so
    // `kinds.isEmpty`/`.last` fell to the same no-paren-becomes-a-field refusal `dropRight`'s own gap
    // did. `isKnownVecReceiver` already trusts `map`/`flatMap` for the INLINE-chain shape (`xs.map(f).
    // headOption`); this is the same trust for the BOUND-TO-A-LOCAL spelling.
    val SeqPreserving = Set("filter", "filterNot", "drop", "take", "dropRight", "takeRight",
                            "sortBy", "sortWith", "tail", "init", "map", "flatMap")
   ...
   *  wider "might be char-shaped" heuristic. */
  private def collectLocalSscChars(body: m.Term): Set[String] =
    val names = scala.collection.mutable.Set.empty[String]
  ...
  /** `val (element, inherited) = stack.remove(stack.size - 1)` (`uniml/xml`'s `Doc.scala`'s
   *  `validateNamespaces`) — a TUPLE-DESTRUCTURING local bind whose source is a COLLECTION of
   *  tuples (`ArrayBuffer((root, Map(...)))`), not a bare Option/pair literal
   *  `collectLocalTupleStringPositions` already covers. `element`'s own ctor name (`root: Markup.
   *  Element`, threaded through the `ArrayBuffer` literal) was invisible to `ctorNameOfExpr`
   *  entirely — no def parameter, no field, just a local bound from a collection method call — so
   *  `element.attrs`/`.name`/`.children` (variant fields, needing the enum-field-read match
   *  `selectOrNiladicCtor` already knows how to build) fell through to plain field access instead:
   *  `error[E0609]: no field attrs on type Node`.
   *
   *  Two passes: first, which LOCALS hold a collection of tuples, and each position's ctor name
   *  (read straight off the tuple LITERAL's own elements via `ctorNameOfExpr`, using a ctx that
   *  already knows every DEF PARAMETER's ctor name — `root` above); second, a tuple-pattern
   *  destructure whose RHS calls a method ON one of those locals inherits that position's ctor name
   *  for each bound name. Deliberately narrow — ANY method call qualifies, not just `remove`/`head`/
   *  `apply`, because Every method this shape could realistically call (`remove`, `apply`, `head`,
   *  `last`, …) returns exactly one element of the same tuple type, and naming them all invites the
   *  list going stale the moment a fifth one is used. */
  private def collectTupleDestructureCtorNames(body: m.Term, ctx: Ctx): Map[String, String] =
    val seqTuplePos = scala.collection.mutable.Map.empty[String, Map[Int, String]]
    def walkSeq(t: m.Tree): Unit =
      t match
            ...
            case (List(m.Pat.Var(m.Term.Name(n))), Some(m.Term.Apply.After_4_6_0(_, args))) =>
              args.values.toList match
                case List(m.Term.Tuple(elems)) =>
                  val posCtors = elems.toList.zipWithIndex.flatMap((e, i) => ctorNameOfExpr(e, ctx).map(i -> _)).toMap
                  if posCtors.nonEmpty then seqTuplePos(n) = posCtors
                case _ => ()
            case _ => ()
        case _ => ()
      t.children.foreach(walkSeq)
    walkSeq(body)
    val result = scala.collection.mutable.Map.empty[String, String]
    def walkDestructure(t: m.Tree): Unit =
      t match
        case v: m.Defn.Val =>
          v.pats match
            case List(m.Pat.Tuple(pats)) =>
              v.rhs match
                case m.Term.Apply.After_4_6_0(m.Term.Select(m.Term.Name(recv), _), _) =>
                  seqTuplePos.get(recv).foreach { posCtors =>
                    pats.toList.zipWithIndex.foreach {
                      case (m.Pat.Var(m.Term.Name(n)), i) => posCtors.get(i).foreach(cn => result(n) = cn)
                      case _ => ()
                    }
                  }
                case _ => ()
            case _ => ()
        case _ => ()
      t.children.foreach(walkDestructure)
    walkDestructure(body)
    result.toMap
  ...
  private def mapType(
    ...
    // A DYN-DISPATCH TRAIT (`DialectAdapter`, `Processor[S, I, O]` — see `_dynTraitTparams`'s own
    // comment for what qualifies) has no Rust structural-typing answer, only a trait object.
    // `Rc`, not `Box`: the same reason a closure field is `Rc<dyn Fn…>` (see the `Type.Function`
    // arm below) — a struct/enum holding one still needs to `#[derive(Clone)]`, and `Rc<dyn Trait>`
    // is Clone unconditionally (the ref count bumps; `T: Clone` is not required), while `Box<dyn
    // Trait>` is not Clone at all. Used uniformly at every position (param, field, return, local) —
    // unlike the closure case, which only boxes in a FIELD — since a dyn-dispatch trait value in
    // this corpus (`DialectRegistry.byName: Map[String, DialectAdapter]`) is exactly the case that
    // must be stored, so there is no plain-parameter shape worth keeping separate.
    case m.Type.Name(n) if _dynTraitTparams.get(n).exists(_.isEmpty) =>
      Right(s"std::rc::Rc<dyn $n>")
    case m.Type.Apply.After_4_6_0(m.Type.Name(n), argClause) if _dynTraitTparams.contains(n) =>
      traverseTypes(argClause.values.toList, defName, enumNames)
        .map(args => s"std::rc::Rc<dyn $n<${args.mkString(", ")}>>")
    ...
    // `Set[T]` joins the family here for the same reason `Array` did above (see that comment):
    // R.2's own docstring already promises "primitives, enums, function types, tuple, and
    // List/Vec" and a caller reading `def aliases: Set[String]` has no reason to expect it isn't
    // covered. `Vec<T>` rather than a real `HashSet`/`BTreeSet`: this lane does not enforce
    // Scala's uniqueness/unordered-ness invariants for ANY collection type it lowers (a `List`
    // with a duplicate lowers fine too), so a `Set` gets the same "eager sequence" treatment as
    // its List-family siblings rather than a richer type nothing here actually needs yet.
    case m.Type.Apply.After_4_6_0(m.Type.Name("List" | "Vec" | "Vector" | "Seq" | "IndexedSeq" | "Iterable" | "Array" | "Set"), argClause) =>
      argClause.values.toList match
    ...
    // Same variant-as-type case, spelled QUALIFIED (`UniNode.Branch` rather than bare `Branch`) —
    // scalascript-uniml's `Tree.scala`/`TreeVm.scala` write it this way throughout (`def attach(...):
    // UniNode.Branch`). Scalameta parses the qualifier as a `Type.Select`; this backend has no
  ...
  private def renderParams(
    ...
    val rendered = recvParam ++ params.map { p =>
      p.decltpe match
        ...
        // `sb: StringBuilder` used as a PARAMETER, not a local (`uniml/xml`'s `Doc.scala`'s
        // `serializeNode`, threaded through a recursive call chain rather than declared once with
        // `let mut`) — `mapType`'s own `StringBuilder -> String` case gives the right Rust TYPE, but
        // every `.push`/`.push_str` needs the BINDING itself to be `mut`, and nothing here ever
        // added it: `error[E0596]: cannot borrow sb as mutable, as it is not declared as mutable`.
        // A LOCAL `StringBuilder` never hit this gap because `renderLetBinding`'s own `kw` already
          ...
          tailRendered match
            ...
            case Right(tailRs) =>
              if errs1.nonEmpty then Left(errs1.flatten)
                ...
                val parts = (liftedFns ++ initOk :+ tailRs).filter(_.nonEmpty)
   ...
   *  an explicit `&mut T`, and un-annotated locals (`var stack = state.stack`, `TreeVm.scala`'s
   *  actual spelling) carry no type in the source at all. Two sources, matching the two shapes
   *  that spelling actually takes: an explicit `decltpe` on the local itself, or — the un-annotated
   *  case — the local's initializer being `param.field` where `param`'s declared type is a known
   *  case class, resolved through `ctorMap` the same way `collectSeqFields`/`collectMapFields`
   *  already resolve a field's type for their own purposes. A self field or the enclosing def's own
   *  parameter is simpler still: both are already in `ctx.paramTypes` (`ownFieldTypes` folds in at
   *  `Ctx` construction). Anything else returns `None` and the caller refuses the lift by name,
   *  rather than emit a `Vec<i64>`-style silent wrong guess `mapType`'s OWN fallback would produce
   *  for an ordinary unresolved type. */
  private def inferCaptureType(name: String, stats: List[m.Stat], ctx: Ctx): Option[String] =
    def matches(pats: List[m.Pat]): Boolean =
      pats.exists { case m.Pat.Var(m.Term.Name(n)) => n == name; case _ => false }
    def fromInit(t: m.Term): Option[String] = t match
      case m.Term.Select(m.Term.Name(r), m.Term.Name(f)) if ctx.paramTypes.contains(r) =>
        val ctorName = ctx.paramCtorNames.getOrElse(r, ctx.paramTypes(r))
        ctx.ctorMap.get(ctorName).flatMap { ec =>
          ec.fieldNames.indexOf(f) match
            case -1 => None
            case i  => ec.fieldTypes.lift(i)
        }
      // `var position = SourcePosition.Start` — a companion-object niladic constant
      // (`SourcePosition.Start`, `Limits.default`, `VmState.initial`…). Its type is the type it is
      // a constant OF, which this codegen already trusts by NAME for the same shape everywhere
      // else it appears (`mapType`'s `_variantOwner`/enum-name fallback below is the same idea one
      // level up: a bare known type name IS its own Rust type).
      case m.Term.Select(m.Term.Name(tn), _) if ctx.enumNames.contains(tn) => Some(tn)
      // A literal initializer (`var nextTokenId = 0L`, `dialect/Literal.scala`'s actual spelling) —
      // no declared type, no field to resolve through, just the literal's own Rust type.
      case _: m.Lit.Int | _: m.Lit.Long              => Some("i64")
      case _: m.Lit.Double | _: m.Lit.Float          => Some("f64")
      case _: m.Lit.String                           => Some("String")
      case _: m.Lit.Boolean                          => Some("bool")
      // `var kinds = stack.map(_.kind)` (`TreeVm.scala`'s `reframeProblem`) — a `Vec<T>` built by
      // projecting one field off each element. `stack`'s OWN element type (`elementTypeOf`, the
      // same inference this function feeds) plus the projected field's type off THAT type's
      // `ctorMap` entry gives the result's element type; mutual with `elementTypeOf` but
      // terminating, since the recursive call bottoms out on `qual`, never on `name` again.
      case m.Term.Apply.After_4_6_0(m.Term.Select(qual, m.Term.Name("map")), args)
          if args.values.size == 1 =>
        val fieldName = args.values.head match
          case af: m.Term.AnonymousFunction =>
            af.body match
              case m.Term.Select(_: m.Term.Placeholder, m.Term.Name(f)) => Some(f)
              case _                                                     => None
          case _ => None
        for
          elemT     <- elementTypeOf(qual, ctx)
          f         <- fieldName
          ec        <- ctx.ctorMap.get(elemT)
          fieldType <- ec.fieldNames.indexOf(f) match { case -1 => None; case i => ec.fieldTypes.lift(i) }
        yield s"Vec<$fieldType>"
      // `val names = adapter.aliases + adapter.id` (`Dialect.scala`'s `register`) — a `Set`'s
      // single-element `+` (see the `renderTerm` case of the same name for why it lowers to a
      // Vec-concat) over a ZERO-ARG method call, whose element type this reads straight off
      // `_returnTypes` the same way `isKnownVecReceiver`'s own new case does — `adapter.aliases`
    ...
    val fromLocalDecl: Option[String] = stats.collectFirst {
      case v: m.Defn.Var if matches(v.pats) =>
        v.decltpe.flatMap(t => mapType(t, ctx.defName, ctx.enumNames).toOption).orElse(fromInit(v.body))
   ...
   *  `closeBefore`'s element is `String` before the anonymous lambda parameter's own `.isEmpty` can
   ...
   *  element type into `paramTypes` at the closure body) an anonymous lambda placeholder:
   *  `open.exists(_.kind.isEmpty)` — `_`'s element type is `FrameSpec`, and `kind` is `FrameSpec`'s
   *  own `String` field. `isStringExpr` cannot see this on its own: it is a pure syntactic check
  ...
  private def isKnownStringField(t: m.Term, ctx: Ctx): Boolean =
    ...
    t match
      case m.Term.Select(m.Term.Name(r), m.Term.Name(f)) => check(r, f)
   ...
   *  `step`/`stop`, where `record`/`addTop`/`attach`/`closeFrame` each close over the same shared
   *  mutable local state and call one another. Rust nested `fn` ITEMS cannot capture their
   *  environment at all (unlike a closure); and sibling CLOSURES that would alias the same mutable
   *  local are exactly the shape rustc's borrow checker refuses even when they are never called
   *  concurrently — two `FnMut` closures both wanting `&mut stack` alive at once is `error: cannot
   *  borrow `stack` as mutable more than once`, regardless of whether they ever actually run at the
   *  same instant. So each local `def` becomes an ordinary Rust nested `fn`, with everything it —
   *  transitively, through the OTHER lifted defs it calls — reads from the enclosing scope appended
   *  as an explicit parameter: `&mut T` for a captured `var`, an owned clone for a captured `val` /
   *  self field / enclosing def parameter. Every call to a lifted name, both among the remaining
   *  statements and inside another lifted def's own body, gets those extra arguments spliced in by
   *  the early `Term.Apply` case in `renderTerm`, driven by the Ctx this returns.
   *
   *  A no-op (the input echoed back, `ctx` untouched) when the block has no local defs — the
   *  overwhelmingly common case, so ordinary bodies pay nothing for this existing. */
  private def liftLocalDefs(
    ...
    if localDefs.isEmpty then Right((Nil, stats, propagatedCtx))
      ...
      // `intersect pool` below, not by naming it here.
      val pool = varNames ++ valNames ++ ctx.defParams ++ ctx.selfFields

      def freeNames(t: m.Tree): Set[String] = t.collect { case m.Term.Name(n) => n }.toSet
      def ownBound(d: m.Defn.Def): Set[String] =
      ...
      // Fixed point over the (small, per-block) local-def call graph: a def that calls another
      // lifted def must ALSO receive whatever that callee needs, because a Rust nested `fn` cannot
      // see even ANOTHER nested `fn`'s captures — only what is passed explicitly. `closeFrame` never
      // touches `roots` itself, but calls `attach`, which does; `closeFrame`'s own signature must
      // carry `roots` too, purely to hand it onward.
      var captures = directCaptures
      var changed  = true
      while changed do
        changed = false
        for name <- defNames do
          val merged = captures(name) ++ callsOf(name).flatMap(callee => captures.getOrElse(callee, Set.empty))
          if merged != captures(name) then
            captures = captures.updated(name, merged)
            changed = true
      // A FIXED canonical order (alphabetical), used identically at a lifted def's own parameter
      // list and at every call site that appends them — the two are never computed independently.
      ...
      val renderedDefs: List[Either[List[Diagnostic], String]] = localDefs.map { d =>
        ...
        val ownParamNames = d.paramClauseGroups.flatMap(_.paramClauses).flatMap(_.values).map(_.name.value).toSet
        val resolved      = myCaptures.map(c => c -> inferCaptureType(c, stats, ctx))
        val missing       = resolved.collect { case (c, None) => c }
        if missing.nonEmpty then
          Left(List(unsupported(
            s"def `${d.name.value}` (a local def lifted out of `${ctx.defName}`) captures " +
            s"`${missing.mkString(", ")}` and this lane cannot infer its type — give it an " +
          ...
          val childCtx = baseCtx.copy(
            ...
            // out — and that collection, like most of this file's own pre-passes, does not descend
            // into a NESTED `Defn.Def`'s own body (`collectDefs`'s own documented convention), so a
            // `.charAt`-bound local declared entirely inside the lifted def was invisible to it:
            // `quote = char` (both `i64`) had no `.0` unwrap and reached rustc as `error[E0308]:
            // expected i64, found SscChar`. Recomputed here, from `d.body` (the lifted def's OWN
      ...
      val (defErrs, defOk) = renderedDefs.partitionMap(identity)
  ...
  private def renderTerm(
    ...
    // `Vector.empty` / `List.empty` / `Array.empty` / `Set.empty` / `Map.empty` — the companion
    // factory for an EMPTY collection, `TreeVm.scala`'s actual spelling for `var roots: Vector[…] =
    // Vector.empty`. Nothing lowered this: it fell through to the generic field-select path and
    // emitted the Scala name VERBATIM (`Vector.empty`), which is not a Rust value at all —
    // `error[E0425]: cannot find value 'Vector' in this scope`, on a receiver every OTHER collection
    // construction on this lane already recognises by name (`Vector(…)`, `List(…)`, `Map(k -> v,
    // …)`). Found only by a real `cargo build`; `emit-rust --print-only` has no line for a name
    // that resolves to nothing.
    case m.Term.Select(m.Term.Name("Vector" | "List" | "Array" | "Set"), m.Term.Name("empty")) =>
      Right("Vec::new()")
    ...
    case m.Term.Apply.After_4_6_0(m.Term.Select(qual, m.Term.Name("append")), args)
      ...
      def pushStmt(q: String, arg: m.Term): Either[List[Diagnostic], String] =
        renderTerm(arg, ctx).map { a =>
          if arg.isInstanceOf[m.Lit.Char] || isConceptuallyChar(arg, ctx) then
    ...
    // destructuring every field as a BARE local — Rust has no `.field` accessor on `r` itself for
    // a per-variant field, only on the destructured bindings the pattern produced. `ctx.
    ...
    // A local-def lift's own captured-`var` parameter (`ctx.byRefMut`) is a Rust `&mut T` — every
    // READ derefs, and so does an assignment target (`Term.Assign`/compound-infix both render
    // their LHS through THIS case, so `(*n) = …` / `(*n) += …` fall out for free, with no separate
    // case needed at either site). A method call on it (`.is_empty()`, `.clone()`, `[i]`) does not
    // need this — Rust autoderefs `&mut` for method/index syntax — but `<`, `+`, `==`… do not, and
    ...
    // ordinary (coercion/intrinsic-aware) call machinery below, which knows nothing about them.
    // Per capture: already a reference in THIS Ctx (`ctx.byRefMut`, i.e. a lift calling another
    // lift with the same captured `var`) forwards bare — Rust reborrows a `&mut` passed to a
    // `&mut` parameter; still owned here but backed by a `var` takes a fresh `&mut`; anything else
    // (a `val`, a def param, a self field) is read-only and is cloned, at the owning scope and at
    // every forward alike, matching this codegen's clone-liberally convention throughout.
    case m.Term.Apply.After_4_6_0(m.Term.Name(n), args) if ctx.liftedDefExtraArgs.contains(n) =>
      // A NAMED call argument (`emit(lexeme, valid = true)`, `dialect/Literal.scala`'s actual
      // spelling) parses as `Term.Assign(Term.Name(p), v)` — Rust calls are positional, so only the
      // VALUE renders; the ordinary call machinery this arm bypasses already does this (see its own
      // comment two cases down), but this arm builds its own argument list and has to repeat it.
      val origArgs = args.values.toList.map {
          ...
          asMapIndexTarget(a.lhs, ctx) match
            ...
            case None =>
              for
                ...
                r0 <- renderTerm(a.rhs, ctx)
              yield
                // `quote = char` (`uniml/xml`'s `Doc.scala`'s `scanDoctype`: `var quote: Char = ' '`,
                // `val char = input.charAt(cursor)`) — the RHS is a genuine `SscChar` newtype
                // (`val char = …charAt(…)`), the LHS a plain `i64` var (this lane's `Char`
                // convention) — assignment is ANOTHER position (alongside a call argument, an
                // if/else branch) needing the SAME `.0` unwrap, applied here whenever the RHS
    ...
    case m.Term.Apply.After_4_6_0(m.Term.Select(qual, m.Term.Name(tr @ ("takeRight" | "dropRight"))), a)
      ...
      yield
        ...
        // CLONE, like `reverse`/`sorted`/`distinct` two arms below already do for the same reason
        // (see their comment): `stack = stack.dropRight(1) :+ x` is exactly `std/fs.ssc`'s
        // self-reassignment shape, and a bare `let __v = $q;` MOVES the receiver — `error[E0382]:
        // borrow of moved value` on a plain top-level var, `error[E0507]: cannot move out of` a
        // captured `&mut` one (a local-def lift's own parameter). Found via a real `cargo build` on
        // BOTH shapes, not by reading — `emit-rust --print-only` has nothing to say about a move.
        s"{ let __v = ($q).clone(); let __k = ($k) as usize; $slice }"
    ...
    // `_` is resolved to its `__p0` name (that substitution happens deeper, inside the generic
    // `Term.AnonymousFunction` rendering this qualifier is itself an argument of), so `isStringExpr`
    // — a pure syntax check — cannot recognise a bare `Term.Placeholder` as a `String` the way it
    // recognises `Term.Name("__p0")` once rendered. `renderVecIterBody`'s `AnonymousFunction` case
    // registers the element type under the name `__p0` WILL become (`_phCounters` assigns
    // placeholder names in encounter order from 0, and a one-arg predicate has exactly one), so
    // `ctx.localStrings` already has the answer here — just not under a name this term carries yet.
    case m.Term.Select(ph: m.Term.Placeholder, m.Term.Name(meth))
        if ctx.localStrings.contains("__p0") && Set("nonEmpty", "isEmpty").contains(meth) =>
      renderTerm(ph, ctx).map(q => if meth == "isEmpty" then s"$q.is_empty()" else s"!$q.is_empty()")

    // `problem.isEmpty` / `.nonEmpty` on a known `Option` (`TreeVm.scala`'s `reframeProblem`: `var
    // problem: Option[Diagnostic] = None; … if problem.isEmpty then …`) — Rust's `Option` has no
    ...
    // DialectAdapter`) is a zero-arg TRAIT method too, but `id` also happens to be a genuine field
    // name elsewhere in this module (`SourceToken.id: Long`, `Source.scala`) — so the name-only
    // guard just below refuses to touch it at all (by design: it would rather miss a call than
    // wrongly rewrite someone else's field). Here the receiver's OWN declared type is known
    // PRECISELY — a def parameter's Rust type, read off `ctx.paramTypes` — so this checks the
    // SPECIFIC trait's own member list instead of every field in the module, and catches exactly
    // the case the name-only guard has to let go.
    case m.Term.Select(qual @ m.Term.Name(n), m.Term.Name(field))
        if ctx.paramTypes.get(n).flatMap(dynTraitNameOfRustType).flatMap(_dispatchTraits.get)
    ...
    // to a genuine field access, and the fallback below renders it as one: `vm.start.clone()`,
    // `error[E0615]: attempted to take value of method 'start'`. Guarded on POSITIVE evidence this
    // name can only mean a call — a real zero-param, zero-tparam def AND never a struct field
    ...
    case m.Term.Apply.After_4_6_0(
        ...
        rArgs
    ) if isEitherExpr(qual, ctx) && lArgs.values.size == 1 && rArgs.values.size == 1 =>
      for
        q <- renderTerm(qual, ctx)
    ...
    case m.Term.Apply.After_4_6_0(
      m.Term.Select(qual, m.Term.Name("fold")),
      args
    ) if isEitherExpr(qual, ctx) && args.values.size == 2 =>
      for
        q <- renderTerm(qual, ctx)
    ...
    case pf: m.Term.PartialFunction =>
      val arms = pf.cases.map { c =>
        ...
        // closure-locals; everything else it touches is captured. `variantBodyCtxExtra`
        // additionally covers `case child: Markup.Element => …` — a WITH-FIELDS-VARIANT typed
        // bind, the SAME shape `renderMatch`'s own per-arm bodyCtx already enriches (byRefMut,
        // paramTypes, destructuredCtorNames, seqFields/mapFields) for a STANDALONE `match`
        // statement, but this SEPARATE PartialFunction-in-`.map`/`.filter`/… renderer never
        // applied at all: `.map { case child: Markup.Element => resolveElement(child, …) }`
        // (`uniml/xml`'s `Doc.scala`'s `resolveElement`) left `child` looking like a plain owned
        // name here, when `renderPattern`'s OWN twin case renders it `ref child @ …` — a BORROW —
        // so `resolveElement(child, …)`, wanting an OWNED `Node`, got `error[E0308]: expected
        // Node, found &Node`.
        val bodyCtx = variantBodyCtxExtra(c.pat, enteringClosure(ctx, patBoundNames(c.pat) + "__pf"))
    ...
    case m.Term.Apply.After_4_6_0(m.Term.Select(qual, m.Term.Name("orElse")), a)
        if a.values.size == 1 && isOptionExpr(qual, ctx) =>
    ...
    case m.Term.Apply.After_4_6_0(
      ...
      for
        q <- renderTerm(qual, ctx)
        // `names.find(byName.contains)` (`Dialect.scala`'s `register`) needs its element type here:
        // the METHOD-REFERENCE shape lowers to a doubly-nested closure (`renderVecIterBody`'s own
        // comment on that case explains why), and rustc's inference does not reach through that far
        // on its own — `error[E0282]: type annotations needed`. `elementTypeOf` is a best-effort
        // read of a local's OWN declaration; `None` here just means the annotation is skipped,
    ...
    // Rust's `Iterator` has no `.count(predicate)` at all, only `.count()` (every element) — the
    // direct equivalent is `.filter(p).count()`, cast back to `i64` since Rust's own `.count()`
    // returns `usize` and Scala's `count` returns `Int`.
    case m.Term.Apply.After_4_6_0(
    ...
    case m.Term.Apply.After_4_6_0(m.Term.Select(qual, m.Term.Name("indexOf")), args)
      ...
      for
        ...
        needle <-
          if needleArg.isInstanceOf[m.Lit.Char] || isConceptuallyChar(needleArg, ctx) then
    ...
    //
    // `Some(frame.openingSpan)` (`TreeVm.scala`'s `stop`) needs the SAME move-vs-clone decision
    // every ordinary call argument gets — `frame` is read again two lines later
    // (`TreeVm.buildBranch(frame, …)`) — but `renderTerm`'s recursive call on the WRAPPED term
    // bypassed `cloneIfMoved` entirely: that helper is only invoked at a call's OWN argument
    // positions, and the argument HERE is the whole `Some(frame.openingSpan)` `Term.Apply`, not
    // the field projection inside it. `error[E0382]: borrow of partially moved value: frame`, and
    ...
    // `Emit()` — a ctor call with EMPTY parens where every field carries a default
    // (`VmInstruction.Emit(role: Option[String] = None)`, `dialect/Literal.scala`'s actual
    // spelling). `namedCtorAsPositional` already fills a field the caller OMITTED from its default
    // — that machinery is exactly what an all-omitted, zero-arg call needs too — but the call site
    // below requires `args.values.nonEmpty`, so a fully-defaulted ctor never reached it at all and
    // fell to the generic application path, which had no ctor arity of 0 to compare against and
    // rendered the bare enum-case spelling (`VmInstruction::Emit`) as if the variant carried no
    // fields — `error[E0533]: expected value, found struct variant`. Guarded on `fieldNames.
    // nonEmpty` so a GENUINELY field-less ctor (already correct via the niladic-value path) does
    // not also take this one — `namedCtorAsPositional` on an empty field list reproduces the exact
    // same zero-arg call it started from and would recurse forever.
    case m.Term.Apply.After_4_6_0(m.Term.Name(n), args)
        if args.values.isEmpty && ctx.ctorMap.get(n).exists(_.fieldNames.nonEmpty) =>
      namedCtorAsPositional(n, Nil, ctx) match
        case Some(desugared) => renderTerm(desugared, ctx)
        case None            => Left(List(unsupported(
          s"def `${ctx.defName}` calls `$n()` but not every field of `$n` has a default"
        )))
    ...
    // REPEATING the string `depth` times (indentation), not numeric multiplication. Reached rustc as
    // a literal `*`: `error[E0369]: cannot multiply String by i64` (`String` has no `Mul` at all).
    // `.repeat(n as usize)` is the direct Rust equivalent; guarded on the LEFT side being a string so
    // an ordinary `i64 * i64` a few arms below keeps rendering as plain `*`, unaffected.
    case m.Term.ApplyInfix.After_4_6_0(lhs, m.Term.Name("*"), _, args)
        if args.values.size == 1 &&
           (isStringExpr(lhs) || isKnownStringField(lhs, ctx) ||
            (lhs match { case m.Term.Name(n) => ctx.localStrings.contains(n); case _ => false })) =>
      for
        l <- renderTerm(lhs, ctx)
        r <- renderTerm(args.values.head, ctx)
      yield s"($l).repeat(($r) as usize)"
    ...
    case m.Term.ApplyInfix.After_4_6_0(lhs, m.Term.Name("->"), _, args)
        if args.values.size == 1 =>
      // `names.map(_ -> adapter)` (`Dialect.scala`'s `register`) reads `adapter` — a captured
      // OUTER parameter, non-`Copy` — from inside a closure that runs once per element: the FIRST
      // iteration moves it into the tuple, and the SECOND has nothing left — `error[E0507]: cannot
      // move out of adapter, a captured variable in an FnMut closure`. `cloneIfMoved` is the SAME
      // move-vs-clone decision every other call argument already gets; a tuple literal's two
      // operands just never routed through it before.
      for
        l <- renderTerm(lhs, ctx)
        r <- renderTerm(args.values.head, ctx)
      yield s"(${cloneIfMoved(lhs, l, ctx)}, ${cloneIfMoved(args.values.head, r, ctx)})"

    // `byName ++ names.map(_ -> adapter)` (`Dialect.scala`'s `DialectRegistry.register`) — a Map
    // UNION, not a sequence concat: the general `++` case just below builds `[&(a)[..],
    // &(b)[..]].concat()`, the borrowed-SLICE idiom, and a `HashMap` cannot be sliced at all
    // (`error[E0308]: expected &_, found RangeFull` — `&(byName)[..]` parses as indexing `byName`
    // with a full range, which only a slice/Vec supports). `HashMap::extend` is the direct Rust
    // equivalent of Scala's `Map ++ Map` (later pairs overwrite earlier ones on a key collision,
    // the same rule `Map1 ++ Map2` has) — cloning the LEFT map first keeps this an expression
    // rather than a statement, matching every other `++` site's shape.
    case m.Term.ApplyInfix.After_4_6_0(lhs, m.Term.Name("++"), _, rargs)
        if rargs.values.size == 1 && isKnownMapReceiver(lhs, ctx) =>
      for
        l <- renderTerm(lhs, ctx)
    ...
    // `xs ::: ys` — Scala's List-specific concat, same semantics as `++` restricted to List
    // operands (`scalascript-uniml`'s `Tree.scala`: `edges.map(_.child).toList ::: pending.tail`).
    // Same borrowed-slice-concat lowering as the `++` case above, for the same reason (owned
    // `[a, b]` would move both operands, breaking any later reuse in the same scope).
    case m.Term.ApplyInfix.After_4_6_0(lhs, m.Term.Name(":::"), _, rargs)
    ...
    // `aliases: Set[String]` and `id: String`). This lane treats `Set[T]` as `Vec<T>` throughout
    // (`renderStruct`'s own comment on why: no uniqueness enforced for ANY collection type here),
    // so the answer is the SAME borrowed-slice-concat shape as `:+` just above — the only new part
    // is recognising the RECEIVER as a sequence at all, since `isKnownVecReceiver` otherwise has no
    // case for "the return value of a known zero-arg method/trait member" (`aliases()` is neither a
    // local, a field, nor a sequence-combinator chain). Guarded on the RIGHT operand NOT also being
    // a known sequence, so ordinary numeric/string `+` — including a `Vec ++ Vec`-shaped call this
    // lane would otherwise mis-detect as "vec plus scalar" — keeps taking `renderInfix` exactly as
    // before.
    case m.Term.ApplyInfix.After_4_6_0(lhs, m.Term.Name("+"), _, rargs)
    ...
    // A Scala `Char` is a UTF-16 CODE UNIT, and this backend already represents a `Char`-typed
    // def PARAMETER/local as plain `i64` everywhere (confirmed by compiling a real caller:
    // `isHighSurrogate(char: Char)` renders as `fn isHighSurrogate(char: i64)`, not `SscChar`).
    // `SscChar` is a NARROWER thing — the return type of `.charAt()` specifically, which callers
    // convert to `i64` themselves where needed (`yieldsSscChar`). A literal has to match the
    // representation of the values it gets compared/passed against, not invent a third one:
    // `'\uD800'` first went out as `SscChar(...)`, which does not typecheck against the `i64`
    // parameter `isHighSurrogate` actually has (`error[E0308]: expected i64, found SscChar`,
    // caught by an actual `cargo build` of the generated crate, not just codegen succeeding).
    // Plain `i64` matches the dominant representation with no further change needed anywhere.
  ...
  private def renderStrPatternArg(t: m.Term, ctx: Ctx): Either[List[Diagnostic], String] = t match
    ...
    // `content.contains('<')` (`uniml/xml`'s `Doc.scala`'s `validateContent`) — a Rust `str`
    // method taking a `Pattern`, which `&i64` does not implement at all (`error[E0277]`): `'<'`
    // renders as its `i64` code point (`renderTerm`'s own `Lit.Char` case, this lane's SscChar
    // convention), same as `.charAt` and everywhere else — but a `Pattern` needs an actual `char`
    // here, not the borrowed number the fallback below gives every OTHER argument type.
    case lit: m.Lit.Char =>
  ...
  private def isEitherExpr(term: m.Term, ctx: Ctx): Boolean = term match
    ...
    case m.Term.Apply.After_4_6_0(
      ...
      _
    ) if isEitherExpr(inner, ctx) => true
    // A bare NAME can be Either-typed too, when nothing about ITS OWN syntax says so — see
    // `Ctx.eitherLocals`'s own comment for the one case this covers today (a `foldLeft`
    // accumulator whose zero value is itself Either-shaped).
    case m.Term.Name(n) if ctx.eitherLocals.contains(n) => true
  ...
  /** Is `term` a bare name KNOWN (by declaration, never inference) to hold a `Map`? Narrow on
   *  purpose, matching `isKnownVecReceiver` one type over: a receiver this cannot place keeps
   *  whatever lowering it already had, so a wrong `false` costs a missed rewrite, never a wrong one. */
  private def isKnownMapReceiver(term: m.Term, ctx: Ctx): Boolean = term match
  ...
  private def isKnownVecReceiver(term: m.Term, ctx: Ctx): Boolean = term match
    ...
    // Scala's own spelling for a parameterless method, so this reaches here as the SAME
    // `Term.Select(Name, Name)` shape a field read does, not a `Term.Apply`. `_returnTypes` is the
    // same table `renderDef`'s own return-type mapping already builds, keyed by bare def name (this
    // backend has no per-receiver overload resolution, so that is as precise as it gets — good
  ...
  private def isOptionExpr(term: m.Term, ctx: Ctx): Boolean = term match
    ...
    // `attribute.name.prefix` (`uniml/xml`'s `Doc.scala`'s `resolveElement`) — a MULTI-LEVEL field
    // projection chain (`attribute` a `.map` closure param, `.name` a `QName`, `.prefix` the actual
    // `Option[String]`), which every case above misses: they only ever look at a BARE name's own
    // type, never a chain more than one `.field` deep. `ctorNameOfExpr` already resolves a chain
    // like this (the SAME lookup `isKnownStringField`/`isKnownVecReceiver` use for their own
    // multi-level cases) — read its target ctor's OWN field type here instead of re-deriving it.
    case m.Term.Select(qual, m.Term.Name(f)) =>
      ctorNameOfExpr(qual, ctx).flatMap(ctx.ctorMap.get).exists { ec =>
        val i = ec.fieldNames.indexOf(f)
        i >= 0 && ec.fieldTypes.lift(i).exists(_.startsWith("Option<"))
      }
              ...
              ctx.ctorMap.get(n).map { ec =>
                ...
                val body   =
                  if fields.isEmpty then ""
                    ...
                    // Same for a dyn-dispatch-trait FIELD (`Rc<dyn DialectAdapter>`). `.startsWith`,
                    // not `.contains`: unlike the Debug/PartialEq/Value-lift checks above (which ask
                    // "does this field's type mention an Rc<dyn …> ANYWHERE"), this one wraps the
                    // WHOLE argument, so it must fire ONLY when the field type IS directly `Rc<dyn
                    // …>` — a field typed `Map[String, DialectAdapter]` (`DialectRegistry.byName`)
  ...
  private def renderVecIterBody(
      ...
      // The receiver's ELEMENT type, when the caller could place it (see `elementTypeOf`) — fed
      // into the closure body's OWN Ctx as its single param's type, so `_.isEmpty` / `_.kind.isEmpty`
      // inside `closeBefore.exists(_.isEmpty)` (`TreeVm.scala`'s `reframeProblem`) can resolve at
      // all: an anonymous `_` parameter carries no type of its own for `isStringExpr`/
      // `isKnownStringField` to read.
      elemType: Option[String] = None,
      // `foldLeft`-only: the ZERO value handed to this fold is itself Either-shaped (`Right(empty)`
    ...
    case fn2: m.Term.Function =>
      ...
      val bodyCtx0 = enteringClosure(ctx, Set(p0, p1))
      val bodyCtx1 = elemType match
        case Some(t) if params.sizeIs <= 1 =>
          bodyCtx0.copy(
    ...
    case m.Term.Select(qual, m.Term.Name(meth)) =>
      // `names.find(byName.contains)` (`Dialect.scala`'s `register`) — `byName.contains` is a
      // METHOD REFERENCE (eta-expansion), a different AST shape from the ORDINARY call
      // `byName.contains(k)` that the map-specific `contains` -> `contains_key` rewrite (a few
      // arms above, in `renderTerm` proper) is written against; this lane never reaches that one,
      // so it needs its own copy of the same map-vs-string decision.
      val rustMeth = if meth == "contains" && isKnownMapReceiver(qual, ctx) then "contains_key" else meth
      renderTerm(qual, ctx).map { q2 =>
        // NOT `move` for a REPEATED call (`find`/`exists`/`forall`/`filter`/`map`/`foreach` — every
        // shape here runs its closure once per element, an `FnMut`): a `move` closure captures
        // `$q2` fresh on every CONSTRUCTION, and `$q2` is CONSTRUCTED once but the closure literal
        // runs once per outer-iteration call when nested inside `find`'s own predicate the way
        // `names.find(byName.contains)` (`Dialect.scala`'s `register`) is — `error[E0507]: cannot
        // move out of byName, a captured variable in an FnMut closure`, the second call trying to
        ...
        // rustc's inference reaches on its own (`error[E0282]`); see the ONE call site that passes
        // `elemType` for why.
        // `Iterator::find`'s predicate takes `&Item`, not `Item` — `.cloned()` upstream already
        // makes the ITEM owned, but `find`'s own closure signature still borrows it one more time.
        // With `elemType` known, `__f` is typed as that reference DIRECTLY, and `$rustMeth` here is
        // always a map lookup (`contains_key`, the one caller that passes `elemType`), which itself
        // wants a `&Q` — so `__f` is passed AS-IS, not `.clone()`d, which would hand it an owned
        // value where a reference is expected (`error[E0308]: expected &_, found String`). The
        // untyped default keeps the original `.clone()` shape, unchanged, for every other caller.
        val (findParam, findArg) = elemType match
          case Some(t) => (s"__f: &$t", "__f")
          case None    => ("__f", "__f.clone()")
        method match
    ...
    // `_.isEmpty` / `_.kind.isEmpty` — the PLACEHOLDER shorthand for a one-arg lambda
    // (`closeBefore.exists(_.isEmpty)`, `TreeVm.scala`'s `reframeProblem`). `renderTerm` already
    // lowers `Term.AnonymousFunction` generically (elsewhere in this file — the `_phCounters`
    // stack), by design used from ANY context, so it has no way to know the receiver's element
    // type; without a type, `_`'s `isEmpty` had nothing to resolve against and refused. The single
    // placeholder here is deterministically `__p0` — `_phCounters` assigns placeholder names in
    // ENCOUNTER order starting at 0, and a one-arg lambda has exactly one — so `elemType` can be
    // registered under that exact name before rendering, matching what `renderTerm` is about to
    // assign it.
    //
    // Handled here rather than folded into the generic fallback below (which still covers this
    // shape too, minus the type): `exists`/`forall` need `.any`/`.all`, not a literal `.exists(…)` —
    // the fallback's catch-all (`s"$q.$other2($f)"`) does not know that either, matching the
    // Function-closure arm's OWN dispatch table two cases up rather than inventing a third copy.
    case af: m.Term.AnonymousFunction =>
      val phCtx = elemType match
        case Some(t) =>
          ctx.copy(
            paramTypes   = ctx.paramTypes + ("__p0" -> t),
      ...
      yield method match
        ...
        // `skip_while`, whose OWN signatures always take `&Item` — REGARDLESS of `.cloned()` already
        // having made the iterator's items owned — unlike `.find` two lines up (the SAME `&Item`
    ...
    // `Term.Apply(Term.Name(n), args)` case in `renderTerm` (see its own comment) only intercepts
    // an actual CALL to a lifted name, and this is the exact shape that reaches it as a plain
    // `Term.Name` instead — `renderTerm` alone would emit the bare Rust function ITEM name with no
    // captures appended at all, `.for_each(close)` calling a 3-arg fn where a 1-arg one was
    ...
    case m.Term.Name(n) if ctx.liftedDefExtraArgs.contains(n) =>
      ...
      // what broke `reframeProblem`'s `closeBefore.foreach(close)` THEN `closeAfter.foreach(close)`
  ...
  private def inlineArm(
    ...
    arg match
      ...
      // `Term.Function` — the case just above never fires — and fell to the `other` branch, which
      // wraps it as an IIFE `(|__p0| { … })(v)`. That reintroduces the EXACT type-inference gap this
      // whole function exists to avoid (see the doc comment above): `error[E0282]: type annotations
      // needed` on `__p0`, one match arm inside the very `match … { … }` this function was written to
      // keep out of. Same fix, same reason — bind `v` with a `let` instead of calling a closure —
      // using the SAME `_phCounters` placeholder-naming convention `renderTerm`'s own
      // `Term.AnonymousFunction` case uses, so `_` renders as `__p0` consistently everywhere in one
      // body. Only a SINGLE placeholder is bound this way — a lambda using `_` more than once needs
      // more than one argument, which is not this call's shape (`map`/`flatMap`/`fold`'s arm always
      // takes exactly one value) — so any other count falls through to the general IIFE below,
      // unchanged.
      case af: m.Term.AnonymousFunction =>
        _phCounters = 0 :: _phCounters
        val bodyR = renderTerm(af.body, enteringClosure(ctx, Set("__p0")))
        val count = _phCounters.headOption.getOrElse(0)
        _phCounters = _phCounters.drop(1)
        if count == 1 then bodyR.map(b => s"{ let __p0 = $v; $b }")
        else renderTerm(af, ctx).map(f => s"($f)($v)")
      case other =>
  ...
  /** Does this term evaluate to a `SscChar`? Either `s.charAt(i)` directly, or a NAME already
   *  known (`Ctx.localSscChars`) to have been bound from exactly that call — `val c =
   *  s.charAt(i); f(c)` needs the same `.0` coercion `f(s.charAt(i))` gets, and the binding must
   *  not erase what produced it, the same discipline `localStrings` already applies to String.
   *  Deliberately syntactic and deliberately narrow otherwise: a wrong `true` appends
  ...
  /** BROADER than `yieldsSscChar`: true for anything CONCEPTUALLY a `Char` on this lane, whether it
   *  is the genuine `SscChar` NEWTYPE (`.charAt`, needing `.0` to become a plain `i64`) or already a
   *  plain `i64` that just happens to carry a code point (a bare self-method reference whose OWN
   *  declared Scala return type is `Char` — `cur`, `uniml/xml`'s `Doc.scala`'s `Parser` — `.toChar`
   *  renders as plain `i64` too, this lane's convention, and `cur`'s own `if/else` branch-unification
   *  already made ITS return value a plain `i64` by the time it leaves `cur`, never a real `SscChar`).
   *
   *  These are TWO DIFFERENT QUESTIONS a single function used to answer at once, and conflating them
   *  was the bug: `sb.append(cur)` needs THIS one (does the value need `char::from_u32` to become
   *  DISPLAYABLE, regardless of its concrete Rust representation), while `self.isNameStart(self.
   *  cur())`'s argument coercion needs the NARROW `yieldsSscChar` (does the value need `.0` to
   *  UNWRAP a real newtype) — `cur()`'s result is already a plain `i64`, so widening this broader
   *  check to feed the unwrap decision added a bogus `.0` on a primitive: `error[E0610]: i64 is a
   *  primitive type and therefore doesn't have fields`. Reads the RAW Scala decltpe off `_defBodies`
   *  (bare-name keyed, same collision caveat `_zeroArgDefNames` already carries) rather than the
   *  mapped Rust type string, which has already collapsed every `Char`-returning def to the same
   *  `i64` every ordinary number uses too. A PATTERN match, not `Option.contains` (`==` against a
  ...
  /** The ONE enrichment `renderMatch`'s own per-arm bodyCtx applies for a `case n: Qualified.Ctor
   *  =>` WITH-FIELDS-VARIANT typed bind (its own inline case has the full reasoning), extracted so
   *  the SEPARATE PartialFunction-in-`.map`/`.filter`/… renderer (`{ case p => body }` passed
   *  directly to a Vec combinator, NOT a standalone `match` statement) can apply the SAME
   *  enrichment — that renderer builds its OWN, much simpler bodyCtx (just the pattern's bound
   *  names) and never had this at all. Deliberately just this one case, not the whole of
   *  `renderMatch`'s own bodyCtx match: the rest (untyped ctor destructures, `Some`/`Right`
   *  unwraps, …) has never been reported missing from THIS renderer, and copying the entire thing
   *  speculatively would be guessing at a shape no failing corpus has shown yet. */
  private def variantBodyCtxExtra(pat: m.Pat, ctx: Ctx): Ctx = pat match
  ...
  private def renderMatch(
    ...
    // WHILE ALSO destructuring its own fields by NAME requires `r` to be a place the match can
    // move OUT of piece by piece — which a field READ elsewhere on the SAME underlying value
    // (`TreeVm.scala`'s `step` reads `input.instruction` again after this arm) breaks: `error
    // [E0382]: use of partially moved value`. Cloning the SUBJECT first gives the match its own,
    // independent value to destructure, at the one-time cost every OTHER value this lane clones
    // liberally already pays.
    val hasFieldDestructurePat = cases.exists(_.pat match
      case m.Pat.Bind(_, m.Pat.Extract.After_4_6_0(m.Term.Select(_, m.Term.Name(ctorName)), _))
          if ctx.ctorMap.get(ctorName).exists(_.fieldNames.nonEmpty) => true
      case m.Pat.Bind(_, m.Pat.Extract.After_4_6_0(m.Term.Name(ctorName), _))
          if ctx.ctorMap.get(ctorName).exists(_.fieldNames.nonEmpty) => true
    ...
    val caseRendered = cases1.map { c =>
      ...
      // `case r: VmInstruction.Reframe => … r.open …` (`TreeVm.scala`'s `preflight`) — Rust's
      // typed-pattern bind gives `r` the FULL enum type, `VmInstruction`, same as a plain `r:
      // VmInstruction` parameter would; the field access needs the SPECIFIC variant, `Reframe`,
      // exactly the gap `paramCtorNames` exists to close for a def PARAMETER of this same shape
      ...
      for
        ...
        guard <- (c.cond match
                   ...
                   // guard (`_pendingPatternGuards`'s own comment); placed with the type tests, for
                   // the SAME short-circuit-before-a-guard-that-assumes-the-narrowed-shape reason.
                   val all = typedBinders.map(_._3) ++ litGuards ++ userGuards
  ...
  /** Local `val` names -> a Rust TYPE STRING, feeding `ctx.paramTypes` the same way a def's own
   *  PARAMETERS already do — so a zero-arg TRAIT-method call at `x.method` (`vm.start`, `lexer.
   *  start` in `UniML.scala`'s `parse`) is recognised as a call even though `x` is a LOCAL, not a
   *  parameter. Narrow on purpose, matching what these two call sites actually are and nothing
   *  more: a direct `val name = Ctor(...)` naming a known STRUCT constructor (`val vm =
   *  TreeVm(limits)` -> "TreeVm"), or `val name = recv.method(...)` where `method`'s OWN declared
   *  return type is known (`_returnTypes`, the same table `renderDef`'s own return-type mapping
   *  already builds) — `val lexer = dialect.instructions(source)` -> whatever `instructions`
      ...
      t match
        case v: m.Defn.Val =>
          v.pats match
            case List(m.Pat.Var(m.Term.Name(nm))) =>
              v.rhs match
                case m.Term.Apply.After_4_6_0(m.Term.Name(ctorName), _)
                    if ctorMap.get(ctorName).exists(_.isStruct) =>
                  out(nm) = ctorName
  ...
  private def selectOrNiladicCtor(qual: m.Term, q: String, field: String, ctx: Ctx): String =
    qual match
      case m.Term.Name(enumName)
        ...
        s"$enumName::$field"
      // `TreeVm.finishedDiagnostic` / `VmState.initial` / `SourcePosition.Start` — a companion-object
      // TOP VAL, reached through its owning type's name the same syntactic way an enum case is.
      // Every def whose body reaches this reference already carries the `let finishedDiagnostic =
      // …;` preamble `topValPreamble` injects — `topValsReferencedBy` collects a topval NAME
      // anywhere a `Term.Name` node with that name appears, which includes the field half of this
      // very `Term.Select` — so the qualifier is dropped: it is syntax Scala needs to find the val
      // and Rust does not, once the val is already bound as a local. Gated on `_topValOwners`
      // MATCHING the written qualifier, not just on the name existing as SOME topval — two
      ...
      // `Term.Apply` path); this is the same lookup for the shape that never reaches an `Apply`
      ...
      case _ if ctorNameOfExpr(qual, ctx).flatMap(ctx.ctorMap.get).exists(ec => !ec.isStruct && ec.fieldNames.contains(field)) =>
        ...
        // `doc.root` on `Markup.Doc` (`uniml/xml`'s `Doc.scala`'s `serialize`) — `root: Node` is a
        // DIRECTLY RECURSIVE field, so the STRUCT ITSELF boxes it (`ec.boxedFields`, the construction
        // side's own reason: an unboxed recursive enum has no fixed size). `$f.clone()` alone clones
        // the `Box<Node>`, not the `Node` inside it — `serializeNode(doc.root, …)`, declared to take
        // a plain `Node`, got `error[E0308]: expected Node, found Box<Node>`. `*` derefs the CLONED
        // box (never the original, so this never reaches into `$q` itself) to move the `Node` out.
        val unboxed = if ec.boxedFields.contains(field) then s"*$f.clone()" else s"$f.clone()"
        s"(match &$q { ${ec.enumName}::$cn { $f, .. } => $unboxed, _ => unreachable!() })"
  ...
  private def renderPattern(p: m.Pat, ctx: Ctx): Either[List[Diagnostic], String] = p match
    ...
    // `QName(Some("xmlns"), prefix, _)` — a STRING LITERAL nested one level inside a `Some(...)`
    // extractor, itself one positional arg of a with-fields ctor Extract (`_pendingPatternGuards`'s
    // own comment has the base-case story; this is that SAME fix, one level deeper, since the
    // outer Extract's own rewrite only ever looks at its DIRECT positional args and never recurses
    // into a `Some(literal)` sub-pattern).
    case m.Pat.Extract.After_4_6_0(m.Term.Name("Some"), argClause) if argClause.values.headOption.exists(_.isInstanceOf[m.Lit.String]) =>
      val lit = argClause.values.head.asInstanceOf[m.Lit.String]
    ...
    case m.Pat.Extract.After_4_6_0(m.Term.Name(ctor), argClause) =>
      ...
      ctx.ctorMap.get(ctor) match
        ...
        case Some(ec) =>
          val argPats = args.map {
            // `_pendingPatternGuards`'s own comment has the full E0308 story and why this is a
            // fresh-name-plus-guard rewrite rather than a text change.
            case lit: m.Lit.String =>
              _pendingPatternGuardCounter += 1
              val fresh = s"__litpat${_pendingPatternGuardCounter}"
              _pendingPatternGuards :+= s"""$fresh == "${escapeRustString(lit.value)}""""
              Right(fresh)
    ...
    // `r: VmInstruction.Reframe` — ascribed to a SPECIFIC WITH-FIELDS variant, not the enum/trait
    // itself (`preflight`'s own spelling, `TreeVm.scala`). Rust has no per-variant subtype `r`
    // could narrow to the way Scala's pattern does — `r: VmInstruction` (the bind-all this case
    // used to fall through to, below) has no `.open`/`.closeBefore` field, only the `Reframe`
    // variant does — so `r.open` in the arm body is not expressible by narrowing `r`'s TYPE at
    // all; the fields have to come out of a REAL destructure. Binding `r @ Enum::Ctor { fields… }`
    // gets both at once: it narrows (only a Reframe reaches this arm, matching Scala's own
    // semantics, which the bind-all below did not) and it destructures every field under its OWN
    ...
    case m.Pat.Typed(m.Pat.Var(m.Term.Name(n)), ty)
      ...
      // valid from that point on: `reframeProblem(stack, r.clone())` two lines later is `error
      // [E0382]: value used here after partial move`, on `r`, in the SAME arm that moved `role`
      // out of it. `ref` on every binder makes the whole pattern a BORROW instead — a "move" that
      // takes nothing, so `r` stays a complete, valid `&Enum` and the fields stay `&Field` reads
      // (which every existing use — `.iter()`, `.len()`, `.is_empty()` — already resolves through
      // autoderef either way, see `ctx.byRefMut`'s own doc). `r.clone()` alone would then clone the
      // REFERENCE (Rust's blanket `impl<T> Clone for &T`, giving BACK a `&Enum`, not an owned one)
      // rather than the value it points to — `renderMatch`'s own `bodyCtx` marks `r` in
      // `ctx.byRefMut` for exactly this reason, so `renderTerm`'s existing deref-on-read case
      // rewrites `r` to `(*r)` wherever it is read, `.clone()` included.
      Right(s"ref $n @ ${ec.enumName}::$ctorName { ${ec.fieldNames.map(f => s"ref $f").mkString(", ")} }")
  ...
  private def mapInfixOp(op: String, defName: String): Either[List[Diagnostic], String] = op match
    ...
    case "<" | "<=" | ">" | ">=" | "==" | "!=" => Right(op)
    case "&&" | "||" => Right(op)
    // `x += 1` parses as `Term.ApplyInfix(x, "+=", Nil, List(1))` at the syntax level (scalameta
    // does not desugar compound assignment the way scalac does) — it reaches the SAME infix path
    // as `x + 1`, just with a Unit-typed result. Rust spells all five identically and they are
    // valid as a standalone expression statement (`(count += 1);`), so this is a pure pass-through:
    // no new statement form, no new AST case, just widening what `mapInfixOp` already accepts.
