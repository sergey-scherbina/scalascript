...
| [content-toolkit-yaml-controls.ssc](examples/content-toolkit-yaml-controls.ssc) | Declarative `@ui=toolkit` YAML control tree whose `{type: button, action: <id>}` and `{type: table, source: <id>}` reference a registered action / data source by id (declarative-ui Scope B.1) |
