...
(
  ...
  "$ROOT/scripts/bc-parity-sweep" --only sql-browser-sqlite.ssc \
    --report "$tmp/backend-list-specific.tsv" --strict
...
grep -F $'sql-browser-sqlite.ssc\tskipped-backend' "$tmp/backend-list-specific.tsv" >/dev/null
