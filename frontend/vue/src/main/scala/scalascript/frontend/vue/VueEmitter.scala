...
private[vue] object VueEmitter:
  ...
  def emit(root: View[?]): String =
    ...
    val watchImport     = if fetchTables.nonEmpty || fetchSigs.nonEmpty then ", watch" else ""
    val mountedImport   = if fetchSigs.nonEmpty then ", onMounted" else ""
    ...
    val allNames = signals.keys.toList ++ lists.keys.toList ++ fetchTables.map(_.tableId) ++ fetchSigNames ++ refs.toList
    val returned =
