...
class RelayJwtTest extends AnyFunSuite:
  ...
  private val pub   = RelayJwt.publicKeyFromPrivate(priv)
  // Per RFC 8032 the matching pubkey is this exact value.
  private val knownPub =
  ...
  test("didKey starts with z6Mk for ed25519 (multicodec ed01 prefix property)") {
    ...
    val randPriv = new Array[Byte](32)
    rng.nextBytes(randPriv)
