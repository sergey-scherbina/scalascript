...
| [Graph storage](docs/graph-storage.md) | Planned, partially implemented: `backend/graph` provides a portable JVM graph runtime SPI plus in-memory property/RDF storage; `graphs:` front matter and TinkerGraph/RDF4J/database adapters remain planned |
...
| Graph storage | `backend/graph` now provides `GraphCapabilities`, `PropertyGraphBackend`, `RdfGraphBackend`, `GraphBackend`, and `GraphRuntime.inMemory()` for portable JVM property/RDF storage through typed codecs. `graphs:` front matter, `.ssc` `Graph.*` facade, TinkerGraph/TinkerPop, RDF4J, and production adapters such as Neo4j, JanusGraph/TinkerPop providers, and RDF4J-compatible repositories remain planned |
