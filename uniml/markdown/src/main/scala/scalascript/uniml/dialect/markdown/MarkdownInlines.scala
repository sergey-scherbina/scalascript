...
private[markdown] object MarkdownInlines:
  ...
  def normalizeLabel(raw: String): String =
    val trimmed: String = raw.trim  // annotated: captured by the lifted `walk`
  ...
  private def emailLocalBackscan(
    ...
    val chunk0: String = pending.mkString("")
  ...
  private def scanClosingTag(content: String, start: Int): Option[Int] =
    // annotated for the Rust lane: a lifted local def's CAPTURE must carry an explicit type
    val n: Int = content.length
