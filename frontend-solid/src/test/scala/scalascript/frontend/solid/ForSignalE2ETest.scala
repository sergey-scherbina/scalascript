...
/** A2e E2E — todo-list-style demo via the emitted Solid JS.
 *  Requires `--conditions=browser` like the other Solid E2Es. */
class ForSignalE2ETest extends AnyFunSuite:
  ...
  test("ForSignal — push + clear update the rendered <li>s via emitted Solid JS") {
    ...
    val tmp = Files.createTempDirectory("ssc-forsignal-solid-")
    try
