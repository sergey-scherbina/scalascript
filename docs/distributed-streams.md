...
Companion documents:
- [`docs/streams.md`](streams.md) — in-process `Source[A]` with backpressure (v1.51, single-node)
- [`docs/mapreduce.md`](mapreduce.md) — `Dataset[T]` batch map-reduce, local + distributed (v1.21–v1.22)
- [`docs/spark-streaming.md`](spark-streaming.md) — raw Spark Structured Streaming pass-through (v1.25 §9.5 Phase F)
- [`docs/algebraic-effects.md`](algebraic-effects.md) — typed effect rows (v1.12)
- [`docs/coroutines.md`](coroutines.md) — coroutine substrate (v1.9 / v1.10)
...
|-------------|---------------|---------|-----------|-------------|------------|--------------|---------|
| `Source[A]` | `docs/streams.md` | ✓ | ✓ | ✗ | ✗ | ✗ | JVM, interpreter, JS |
| `Dataset[T]` | `docs/mapreduce.md` | ✓ | ✗ | ✓ (v1.22 actors) | ✗ | ✗ | JVM, interpreter |
| Spark block | `docs/spark-streaming.md` | ✓ | ✓ | ✓ (Spark) | ✓ (pass-through) | opt-in (Spark) | Spark only |
...
(`docs/mapreduce.md §4`, `runtime/std/actors.ssc:197`). Each pipeline stage becomes a
...
`Coder[T]` supersedes `DatasetCodec[A]` from v1.22 (`docs/mapreduce.md §4.4`) for the DStream
context. The bridge:
...
The Spark pass-through (`docs/spark-streaming.md`) remains available and is **not deprecated**.
...
[`docs/algebraic-effects.md`](algebraic-effects.md). A `DStream` operator that performs effects
...
- **ML stages.** Streaming ML is covered by `docs/spark-mllib.md`; DStream / MLlib integration
  is deferred.
