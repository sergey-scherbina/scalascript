...
echo "# lark: type INFERENCE (Algorithm W) — principal types for UNANNOTATED lambdas"
...
echo "# lark POLYMORPHIC LISTS: infer [a], typecheck length/map, reject heterogeneous lists"
...
echo "# lark USER ADTs: one general ConApp/MatchT mechanism types Option/Either/Pair/Tree"
...
echo "# lark lists COMPILE: map/length erase to Core IR (ctor + Cons-match) and run on VM/JS/Rust"
...
echo "# lark textual surface (UNANNOTATED source text -> lex+parse+INFER, all in ssc0)"
chk_hm() { # file want
  got=$(ssc run bin/lark.ssc0 "$1" | tail -1)
  if [ "$got" = "$2" ]; then printf 'ok   %-26s => %s\n' "lark ${1##*/}" "$got"
  else printf 'FAIL %-26s got [%s] want [%s]\n' "lark ${1##*/}" "$got" "$2"; fail=1; fi
}
...
echo "# larkc: INFERRED-typed source text -> ir bytecode (all ssc0) -> run-ir on the VM"
hbc=$(ssc run bin/larkc.ssc0 examples/hm-prog.hm)
...
else printf 'FAIL %-26s bytecode mismatch [%s]\n' "larkc hm-prog" "$hbc"; fail=1; fi
printf '%s\n' "$hbc" > "${TMPDIR:-/tmp}/larkc-prog.coreir"
got=$(ssc run-ir "${TMPDIR:-/tmp}/larkc-prog.coreir" | tail -1)
if [ "$got" = "42" ]; then printf 'ok   %-26s => %s\n' "larkc prog -> run-ir" "$got"
else printf 'FAIL %-26s got [%s] want [42]\n' "larkc prog -> run-ir" "$got"; fail=1; fi

echo "# lark RECURSION: factorial (let rec + mul/sub/eq) infers Int, runs, AND compiles to letrec ir"
...
# the SAME factorial, written as SOURCE TEXT, compiled by larkc and run on the VM
ssc run bin/larkc.ssc0 examples/hm-fact.hm > "${TMPDIR:-/tmp}/hm-fact-text.coreir" 2>/dev/null
got=$(ssc run-ir "${TMPDIR:-/tmp}/hm-fact-text.coreir" | tail -1)
if [ "$got" = "120" ]; then printf 'ok   %-26s => %s\n' "larkc fact.hm -> run-ir" "$got"
else printf 'FAIL %-26s got [%s] want [120]\n' "larkc fact.hm -> run-ir" "$got"; fail=1; fi
# and the SAME factorial compiled to JavaScript (shared codegen reused via lark-js) and run on node
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-fact.hm > "${TMPDIR:-/tmp}/hm-fact.js" 2>/dev/null
  got=$(node "${TMPDIR:-/tmp}/hm-fact.js" 2>/dev/null | tail -1)
  if [ "$got" = "120" ]; then printf 'ok   %-26s => %s (node)\n' "lark-js fact.hm" "$got"
  else printf 'FAIL %-26s got [%s] want [120]\n' "lark-js fact.hm" "$got"; fail=1; fi
fi
# and the SAME factorial compiled to native Rust (shared Rust codegen reused via lark-rust)
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-fact.hm > "${TMPDIR:-/tmp}/hm-fact.rs" 2>/dev/null
  ...
  if [ "$got" = "120" ]; then printf 'ok   %-26s => %s (rustc)\n' "lark-rust fact.hm" "$got"
  else printf 'FAIL %-26s got [%s] want [120]\n' "lark-rust fact.hm" "$got"; fail=1; fi
fi

echo "# lark TEXTUAL LISTS: map written as source text (nil/cons/head/tail/isNil) -> all 3 backends"
chk_hm examples/hm-map.hm '"[Int]"'
chk_hm examples/hm-listlit.hm '"[Int]"'                            # [1, 2, 3] list-literal syntax
echo "# lark STRINGS: String type + literals + ++ (concat) + showInt, on every backend"
chk_hm examples/hm-string.hm '"String"'
SW='"n=42"'
ssc run bin/larkc.ssc0 examples/hm-string.hm > "${TMPDIR:-/tmp}/sw.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-string.hm > "${TMPDIR:-/tmp}/sw.js" 2>/dev/null
...
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-string.hm > "${TMPDIR:-/tmp}/sw.rs" 2>/dev/null
...
echo "# lark FLOAT: Float type + literals + fadd/fsub/fmul/fdiv/flt/feq + toFloat, on every backend"
chk_hm examples/hm-float.hm '"Float"'
FW="2.5"
ssc run bin/larkc.ssc0 examples/hm-float.hm > "${TMPDIR:-/tmp}/fw.coreir" 2>/dev/null
got=$(ssc run-ir "${TMPDIR:-/tmp}/fw.coreir" | tail -1)
if [ "$got" = "$FW" ]; then printf 'ok   %-26s => %s\n' "float -> run-ir" "$got"; else printf 'FAIL %-26s got [%s]\n' "float" "$got"; fail=1; fi
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-float.hm > "${TMPDIR:-/tmp}/fw.js" 2>/dev/null
...
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-float.hm > "${TMPDIR:-/tmp}/fw.rs" 2>/dev/null
...
echo "# lark TUPLES: (a, b) / (a, b, c) syntax + fst/snd, on every backend"
chk_hm examples/hm-tuptype.hm '"(Int, Bool)"'                       # (1, true) renders as a tuple type
TW="10"
ssc run bin/larkc.ssc0 examples/hm-tuple.hm > "${TMPDIR:-/tmp}/tp.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-tuple.hm > "${TMPDIR:-/tmp}/tp.js" 2>/dev/null
...
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-tuple.hm > "${TMPDIR:-/tmp}/tp.rs" 2>/dev/null
...
TFW="11"
ssc run bin/larkc.ssc0 examples/hm-tuptype-field.hm > "${TMPDIR:-/tmp}/tf.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then ssc run bin/lark-js.ssc0 examples/hm-tuptype-field.hm > "${TMPDIR:-/tmp}/tf.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/tf.js" 2>/dev/null | tail -1); if [ "$got" = "$TFW" ]; then printf 'ok   %-26s => %s (node)\n' "tuple-type field -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "tuptype-field JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/lark-rust.ssc0 examples/hm-tuptype-field.hm > "${TMPDIR:-/tmp}/tf.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/tf.rs" -o "${TMPDIR:-/tmp}/tf-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/tf-bin"); else got="(rustc err)"; fi; if [ "$got" = "$TFW" ]; then printf 'ok   %-26s => %s (rustc)\n' "tuple-type field -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "tuptype-field Rust" "$got"; fail=1; fi; fi
echo "# K43 — JSON SHOWCASE: a real ADT + recursive serializer + recursive-descent parser (mutual recursion) in lark, on 3 backends. Roundtrips {\"name\":\"ada\",...} and queries the parsed tree; result Int 3610072 = age 36 (×1e5) + idempotent 1 (×1e4) + serialized len 72. (Big program → typechecker needs -Xss512m, like the ssc0c fixpoint.)"
JX="java -Xss512m -jar $JAR"
jt=$($JX run bin/lark.ssc0 examples/hm-json.hm 2>/dev/null | tail -1)
if [ "$jt" = '"Int"' ]; then printf 'ok   %-26s => %s\n' "json showcase : type" "$jt"; else printf 'FAIL %-26s got [%s] want ["Int"]\n' "json type" "$jt"; fail=1; fi
JW="3610072"
$JX run bin/larkc.ssc0 examples/hm-json.hm > "${TMPDIR:-/tmp}/json.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then $JX run bin/lark-js.ssc0 examples/hm-json.hm > "${TMPDIR:-/tmp}/json.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/json.js" 2>/dev/null | tail -1); if [ "$got" = "$JW" ]; then printf 'ok   %-26s => %s (node)\n' "json roundtrip -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "json JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then $JX run bin/lark-rust.ssc0 examples/hm-json.hm > "${TMPDIR:-/tmp}/json.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/json.rs" -o "${TMPDIR:-/tmp}/json-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/json-bin"); else got="(rustc err)"; fi; if [ "$got" = "$JW" ]; then printf 'ok   %-26s => %s (rustc)\n' "json roundtrip -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "json Rust" "$got"; fail=1; fi; fi
echo '# lark TYPECLASS Show: one `show` resolves to the instance for the inferred type, on every backend'
chk_hm examples/hm-show.hm '"String"'
SHW='"x=42"'
ssc run bin/larkc.ssc0 examples/hm-show.hm > "${TMPDIR:-/tmp}/shw.coreir" 2>/dev/null
got=$(ssc run-ir "${TMPDIR:-/tmp}/shw.coreir" | tail -1)
if [ "$got" = "$SHW" ]; then printf 'ok   %-26s => %s\n' "show Int -> run-ir" "$got"; else printf 'FAIL %-26s got [%s]\n' "show" "$got"; fail=1; fi
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-show.hm > "${TMPDIR:-/tmp}/shw.js" 2>/dev/null
...
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-show.hm > "${TMPDIR:-/tmp}/shw.rs" 2>/dev/null
...
echo "# lark Show over LISTS: recursive type-directed (show [Int]/[[Int]]/[Bool]) on every backend"
chk_hm examples/hm-showlist.hm '"String"'
SLW='"[1, 2, 3]"'
ssc run bin/larkc.ssc0 examples/hm-showlist.hm > "${TMPDIR:-/tmp}/sl.coreir" 2>/dev/null
got=$(ssc run-ir "${TMPDIR:-/tmp}/sl.coreir" | tail -1)
if [ "$got" = "$SLW" ]; then printf 'ok   %-26s => %s\n' "show [Int] -> run-ir" "$got"; else printf 'FAIL %-26s got [%s]\n' "showlist" "$got"; fail=1; fi
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-showlist.hm > "${TMPDIR:-/tmp}/sl.js" 2>/dev/null
  got=$(node "${TMPDIR:-/tmp}/sl.js" 2>/dev/null | tail -1)
  if [ "$got" = "$SLW" ]; then printf 'ok   %-26s => %s (node)\n' "show [Int] -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "showlist JS" "$got"; fail=1; fi
fi
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-showlist.hm > "${TMPDIR:-/tmp}/sl.rs" 2>/dev/null
...
echo "# lark Eq over LISTS: eq [Int]/[[Int]] structural, recursive type-directed, on every backend"
ELW="1"
ssc run bin/larkc.ssc0 examples/hm-eqlist.hm > "${TMPDIR:-/tmp}/el.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-eqlist.hm > "${TMPDIR:-/tmp}/el.js" 2>/dev/null
...
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-eqlist.hm > "${TMPDIR:-/tmp}/el.rs" 2>/dev/null
...
echo "# lark Show/Eq over TUPLES: show (1,true) / eq (1,2) (1,2), type-directed, on every backend"
STW='"(1, true)"'
ssc run bin/larkc.ssc0 examples/hm-showtup.hm > "${TMPDIR:-/tmp}/st.coreir" 2>/dev/null
...
ssc run bin/larkc.ssc0 examples/hm-eqtup.hm > "${TMPDIR:-/tmp}/et.coreir" 2>/dev/null
got=$(ssc run-ir "${TMPDIR:-/tmp}/et.coreir" | tail -1)
if [ "$got" = "1" ]; then printf 'ok   %-26s => %s\n' "eq tuple -> run-ir" "$got"; else printf 'FAIL %-26s got [%s]\n' "eqtup" "$got"; fail=1; fi
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-showtup.hm > "${TMPDIR:-/tmp}/st.js" 2>/dev/null
...
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-showtup.hm > "${TMPDIR:-/tmp}/st.rs" 2>/dev/null
...
echo "# lark Show/Eq over Option/Either: show (Some 5)/eq, type-directed, on every backend"
SOW='"Some(5)"'
ssc run bin/larkc.ssc0 examples/hm-showopt.hm > "${TMPDIR:-/tmp}/so.coreir" 2>/dev/null
...
ssc run bin/larkc.ssc0 examples/hm-eqopt.hm > "${TMPDIR:-/tmp}/eo.coreir" 2>/dev/null
got=$(ssc run-ir "${TMPDIR:-/tmp}/eo.coreir" | tail -1)
if [ "$got" = "1" ]; then printf 'ok   %-26s => %s\n' "eq Option -> run-ir" "$got"; else printf 'FAIL %-26s got [%s]\n' "eqopt" "$got"; fail=1; fi
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-showopt.hm > "${TMPDIR:-/tmp}/so.js" 2>/dev/null
...
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-showopt.hm > "${TMPDIR:-/tmp}/so.rs" 2>/dev/null
...
echo "# lark Show/Eq over GENERAL/RECURSIVE ADTs: Tree + user enums, via per-type recursive helpers"
STW='"Node(Leaf(1), Node(Leaf(2), Leaf(3)))"'
ssc run bin/larkc.ssc0 examples/hm-showtree.hm > "${TMPDIR:-/tmp}/tr.coreir" 2>/dev/null
got=$(ssc run-ir "${TMPDIR:-/tmp}/tr.coreir" | tail -1)
if [ "$got" = "$STW" ]; then printf 'ok   %-26s => %s\n' "show Tree -> run-ir" "$got"; else printf 'FAIL %-26s got [%s]\n' "showtree" "$got"; fail=1; fi
ssc run bin/larkc.ssc0 examples/hm-eqtree.hm > "${TMPDIR:-/tmp}/etr.coreir" 2>/dev/null
got=$(ssc run-ir "${TMPDIR:-/tmp}/etr.coreir" | tail -1)
if [ "$got" = "1" ]; then printf 'ok   %-26s => %s\n' "eq Tree -> run-ir" "$got"; else printf 'FAIL %-26s got [%s]\n' "eqtree" "$got"; fail=1; fi
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-showtree.hm > "${TMPDIR:-/tmp}/tr.js" 2>/dev/null
  got=$(node "${TMPDIR:-/tmp}/tr.js" 2>/dev/null | tail -1)
  if [ "$got" = "$STW" ]; then printf 'ok   %-26s => %s (node)\n' "show Tree -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "showtree JS" "$got"; fail=1; fi
fi
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-showtree.hm > "${TMPDIR:-/tmp}/tr.rs" 2>/dev/null
...
echo "# lark TYPECLASS Eq/Ord: eq (4 base types) + compare (Int+Float) resolved by type"
ssc run bin/larkc.ssc0 examples/hm-cmp.hm > "${TMPDIR:-/tmp}/cmp.coreir" 2>/dev/null
got=$(ssc run-ir "${TMPDIR:-/tmp}/cmp.coreir" | tail -1)
if [ "$got" = "1" ]; then printf 'ok   %-26s => %s\n' "compare 5 3 -> run-ir" "$got"; else printf 'FAIL %-26s got [%s] want [1]\n' "compare" "$got"; fail=1; fi
EQW="1"
ssc run bin/larkc.ssc0 examples/hm-eq.hm > "${TMPDIR:-/tmp}/eq.coreir" 2>/dev/null
got=$(ssc run-ir "${TMPDIR:-/tmp}/eq.coreir" | tail -1)
if [ "$got" = "$EQW" ]; then printf 'ok   %-26s => %s\n' "eq String -> run-ir" "$got"; else printf 'FAIL %-26s got [%s]\n' "eq" "$got"; fail=1; fi
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-eq.hm > "${TMPDIR:-/tmp}/eq.js" 2>/dev/null
  got=$(node "${TMPDIR:-/tmp}/eq.js" 2>/dev/null | tail -1)
  if [ "$got" = "$EQW" ]; then printf 'ok   %-26s => %s (node)\n' "eq -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "eq JS" "$got"; fail=1; fi
fi
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-eq.hm > "${TMPDIR:-/tmp}/eq.rs" 2>/dev/null
...
echo '# lark USER TYPECLASSES: `method m` + `instance m T = impl`, m resolves by arg type-head, on every backend'
chk_hm examples/hm-userclass.hm '"String"'
UCW='"int"'
ssc run bin/larkc.ssc0 examples/hm-userclass.hm > "${TMPDIR:-/tmp}/uc.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-userclass.hm > "${TMPDIR:-/tmp}/uc.js" 2>/dev/null
...
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-userclass.hm > "${TMPDIR:-/tmp}/uc.rs" 2>/dev/null
...
echo "# lark USER TYPECLASS over a user data type: instance name Color resolves + dispatches the impl"
chk_hm examples/hm-userclass-color.hm '"String"'
UCC='"green"'
ssc run bin/larkc.ssc0 examples/hm-userclass-color.hm > "${TMPDIR:-/tmp}/ucc.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-userclass-color.hm > "${TMPDIR:-/tmp}/ucc.js" 2>/dev/null
...
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-userclass-color.hm > "${TMPDIR:-/tmp}/ucc.rs" 2>/dev/null
...
echo "# lark RECORDS: { x = e, y = e } literals + r.x access (structural record types), 3 backends"
chk_hm examples/hm-rectype.hm '"{x: Int, y: Bool}"'
RW="10"
ssc run bin/larkc.ssc0 examples/hm-record.hm > "${TMPDIR:-/tmp}/rec.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-record.hm > "${TMPDIR:-/tmp}/rec.js" 2>/dev/null
...
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-record.hm > "${TMPDIR:-/tmp}/rec.rs" 2>/dev/null
...
echo "# lark ARITH: div/mod/neg (Int) + fneg/fsqrt (Float), on every backend"
chk_hm examples/hm-arith.hm '"Int"'
AW="2"
ssc run bin/larkc.ssc0 examples/hm-arith.hm > "${TMPDIR:-/tmp}/ar.coreir" 2>/dev/null
got=$(ssc run-ir "${TMPDIR:-/tmp}/ar.coreir" | tail -1)
if [ "$got" = "$AW" ]; then printf 'ok   %-26s => %s\n' "mod 17 5 -> run-ir" "$got"; else printf 'FAIL %-26s got [%s]\n' "arith" "$got"; fail=1; fi
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-arith.hm > "${TMPDIR:-/tmp}/ar.js" 2>/dev/null
...
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-arith.hm > "${TMPDIR:-/tmp}/ar.rs" 2>/dev/null
...
echo "# lark SHOWCASE: a typed arithmetic-expression interpreter, written in lark, on 3 backends"
chk_hm examples/hm-eval.hm '"Int"'                                  # data Expr = Num | Plus | Times ; eval
ssc run bin/larkc.ssc0 examples/hm-eval.hm > "${TMPDIR:-/tmp}/ev.coreir" 2>/dev/null
got=$(ssc run-ir "${TMPDIR:-/tmp}/ev.coreir" | tail -1)
if [ "$got" = "7" ]; then printf 'ok   %-26s => %s\n' "eval interp -> run-ir" "$got"; else printf 'FAIL %-26s got [%s]\n' "eval" "$got"; fail=1; fi
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-eval.hm > "${TMPDIR:-/tmp}/ev.js" 2>/dev/null
...
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-eval.hm > "${TMPDIR:-/tmp}/ev.rs" 2>/dev/null
...
ssc run bin/larkc.ssc0 examples/hm-streq.hm > "${TMPDIR:-/tmp}/se.coreir" 2>/dev/null
got=$(ssc run-ir "${TMPDIR:-/tmp}/se.coreir" | tail -1)
if [ "$got" = "1" ]; then printf 'ok   %-26s => %s\n' "strEq -> run-ir" "$got"; else printf 'FAIL %-26s got [%s]\n' "strEq" "$got"; fail=1; fi
echo "# lark Ord operators: > <= >= <> derived from </=/if (consistent on all backends)"
chk_hm examples/hm-ord.hm '"Int"'
ssc run bin/larkc.ssc0 examples/hm-ord.hm > "${TMPDIR:-/tmp}/o.coreir" 2>/dev/null
...
ssc run bin/larkc.ssc0 examples/hm-qsort2.hm > "${TMPDIR:-/tmp}/q2.coreir" 2>/dev/null
...
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-qsort2.hm > "${TMPDIR:-/tmp}/q2.rs" 2>/dev/null
...
echo "# lark BOOLEAN operators: && (tighter) / || (looser) / not — desugar to If, consistent on all backends"
chk_hm examples/hm-bool.hm '"Bool"'
BLW="true"
ssc run bin/larkc.ssc0 examples/hm-bool.hm > "${TMPDIR:-/tmp}/hb.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-bool.hm > "${TMPDIR:-/tmp}/hb.js" 2>/dev/null
...
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-bool.hm > "${TMPDIR:-/tmp}/hb.rs" 2>/dev/null
...
echo "# lark STRING ops: strLen / charAt / substr (typed builtins -> slen/scodeAt/sslice) on every backend"
chk_hm examples/hm-strops.hm '"String"'
SOW='"hello/11/119"'
ssc run bin/larkc.ssc0 examples/hm-strops.hm > "${TMPDIR:-/tmp}/sop.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-strops.hm > "${TMPDIR:-/tmp}/sop.js" 2>/dev/null
...
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-strops.hm > "${TMPDIR:-/tmp}/sop.rs" 2>/dev/null
...
echo "# lark CURRY sugar: multi-arg 'fun x y => e' + function-def 'let f x y = e' / 'let rec f n = e'"
chk_hm examples/hm-curry.hm '"Int"'
CUW="132"
ssc run bin/larkc.ssc0 examples/hm-curry.hm > "${TMPDIR:-/tmp}/cur.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-curry.hm > "${TMPDIR:-/tmp}/cur.js" 2>/dev/null
...
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-curry.hm > "${TMPDIR:-/tmp}/cur.rs" 2>/dev/null
...
echo "# lark WILDCARD match: 'case _ => e' catch-all arm -> IrMatch default slot, on every backend"
chk_hm examples/hm-wildcard.hm '"String"'
WCW='"red/other/other"'
ssc run bin/larkc.ssc0 examples/hm-wildcard.hm > "${TMPDIR:-/tmp}/wc.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-wildcard.hm > "${TMPDIR:-/tmp}/wc.js" 2>/dev/null
...
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-wildcard.hm > "${TMPDIR:-/tmp}/wc.rs" 2>/dev/null
...
echo "# lark LINE COMMENTS: '// ...' to end of line (lexer skips; strings keep their slashes), all backends"
chk_hm examples/hm-comments.hm '"Int"'
CMW="120"
ssc run bin/larkc.ssc0 examples/hm-comments.hm > "${TMPDIR:-/tmp}/cmt.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-comments.hm > "${TMPDIR:-/tmp}/cmt.js" 2>/dev/null
...
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-comments.hm > "${TMPDIR:-/tmp}/cmt.rs" 2>/dev/null
...
echo "# lark VARIABLE pattern: 'case x => body' binds the whole scrutinee (let x = scrut in match _), all backends"
chk_hm examples/hm-varpat.hm '"String"'
VPW='"C/B/C"'
ssc run bin/larkc.ssc0 examples/hm-varpat.hm > "${TMPDIR:-/tmp}/vp.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-varpat.hm > "${TMPDIR:-/tmp}/vp.js" 2>/dev/null
...
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-varpat.hm > "${TMPDIR:-/tmp}/vp.rs" 2>/dev/null
...
echo "# lark PRELUDE: map/filter/foldr/foldl/append/reverse/length/sum/range auto-injected ONLY when used free"
chk_hm examples/hm-prelude.hm '"Int"'
PLW="14"
ssc run bin/larkc.ssc0 examples/hm-prelude.hm > "${TMPDIR:-/tmp}/pl.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-prelude.hm > "${TMPDIR:-/tmp}/pl.js" 2>/dev/null
...
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-prelude.hm > "${TMPDIR:-/tmp}/pl.rs" 2>/dev/null
...
ssc run bin/larkc.ssc0 examples/hm-map.hm > "${TMPDIR:-/tmp}/plchk.coreir" 2>/dev/null
nlet=$(ssc run bin/larkc.ssc0 examples/hm-map.hm 2>/dev/null | grep -o 'letrec' | wc -l | tr -d ' ')
...
echo "# lark PRELUDE (more): take/drop/zip/replicate/all/any — pure polymorphic list functions"
chk_hm examples/hm-prelude2.hm '"Int"'
PL2="2"
ssc run bin/larkc.ssc0 examples/hm-prelude2.hm > "${TMPDIR:-/tmp}/pl2.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-prelude2.hm > "${TMPDIR:-/tmp}/pl2.js" 2>/dev/null
...
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-prelude2.hm > "${TMPDIR:-/tmp}/pl2.rs" 2>/dev/null
...
echo "# lark LITERAL Int patterns: 'match n { 0 => .. | 1 => .. | _ => .. }' -> if-chain, all backends"
chk_hm examples/hm-litpat.hm '"Int"'
LPW="55"
ssc run bin/larkc.ssc0 examples/hm-litpat.hm > "${TMPDIR:-/tmp}/lp.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-litpat.hm > "${TMPDIR:-/tmp}/lp.js" 2>/dev/null
...
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-litpat.hm > "${TMPDIR:-/tmp}/lp.rs" 2>/dev/null
...
echo "# lark STRING/BOOL literal patterns: 'match s { \"add\" => .. | _ => .. }' (strEq) / true|false (if)"
chk_hm examples/hm-litpat-str.hm '"Int"'
LSW="45"
ssc run bin/larkc.ssc0 examples/hm-litpat-str.hm > "${TMPDIR:-/tmp}/ls.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-litpat-str.hm > "${TMPDIR:-/tmp}/ls.js" 2>/dev/null
...
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-litpat-str.hm > "${TMPDIR:-/tmp}/ls.rs" 2>/dev/null
...
echo "# lark NESTED patterns: Con (Sub ..) / tuple / list (Cons/Nil) / literal sub-patterns, backtracking"
chk_hm examples/hm-nestpat.hm '"Int"'
NPW="15"
ssc run bin/larkc.ssc0 examples/hm-nestpat.hm > "${TMPDIR:-/tmp}/ne.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-nestpat.hm > "${TMPDIR:-/tmp}/ne.js" 2>/dev/null
...
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-nestpat.hm > "${TMPDIR:-/tmp}/ne.rs" 2>/dev/null
...
echo "# lark PRELUDE Option/list: concatMap (transitive dep on append) / mapOption / getOrElse / find"
chk_hm examples/hm-option.hm '"Int"'
OEW="30"
ssc run bin/larkc.ssc0 examples/hm-option.hm > "${TMPDIR:-/tmp}/oe.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-option.hm > "${TMPDIR:-/tmp}/oe.js" 2>/dev/null
...
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-option.hm > "${TMPDIR:-/tmp}/oe.rs" 2>/dev/null
...
EIW="143"
ssc run bin/larkc.ssc0 examples/hm-either.hm > "${TMPDIR:-/tmp}/ei.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then ssc run bin/lark-js.ssc0 examples/hm-either.hm > "${TMPDIR:-/tmp}/ei.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/ei.js" 2>/dev/null | tail -1); if [ "$got" = "$EIW" ]; then printf 'ok   %-26s => %s (node)\n' "either combinators -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "either JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/lark-rust.ssc0 examples/hm-either.hm > "${TMPDIR:-/tmp}/ei.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/ei.rs" -o "${TMPDIR:-/tmp}/ei-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/ei-bin"); else got="(rustc err)"; fi; if [ "$got" = "$EIW" ]; then printf 'ok   %-26s => %s (rustc)\n' "either combinators -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "either Rust" "$got"; fail=1; fi; fi
echo "# lark DO-NOTATION: do { x <- e ; ... ; r } -> bind/pure (Option monad by default), all backends"
chk_hm examples/hm-do.hm '"Int"'
DOW="130"
ssc run bin/larkc.ssc0 examples/hm-do.hm > "${TMPDIR:-/tmp}/de.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-do.hm > "${TMPDIR:-/tmp}/de.js" 2>/dev/null
...
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-do.hm > "${TMPDIR:-/tmp}/de.rs" 2>/dev/null
...
echo "# lark PRELUDE (sort): concat / zipWith / elemBy / sortBy — polymorphic, comparator-passing"
chk_hm examples/hm-sort.hm '"Int"'
SRW="3210"
ssc run bin/larkc.ssc0 examples/hm-sort.hm > "${TMPDIR:-/tmp}/se.coreir" 2>/dev/null
got=$(ssc run-ir "${TMPDIR:-/tmp}/se.coreir" | tail -1)
if [ "$got" = "$SRW" ]; then printf 'ok   %-26s => %s\n' "sortBy/zipWith -> run-ir" "$got"; else printf 'FAIL %-26s got [%s]\n' "sort" "$got"; fail=1; fi
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-sort.hm > "${TMPDIR:-/tmp}/se.js" 2>/dev/null
...
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-sort.hm > "${TMPDIR:-/tmp}/se.rs" 2>/dev/null
...
echo "# lark TYPE ASCRIPTION: (e : T) checks/documents, and disambiguates typeclasses (show (None : Option Int))"
chk_hm examples/hm-ascribe.hm '"String"'
ASW='"None/6"'
ssc run bin/larkc.ssc0 examples/hm-ascribe.hm > "${TMPDIR:-/tmp}/ae.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-ascribe.hm > "${TMPDIR:-/tmp}/ae.js" 2>/dev/null
...
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-ascribe.hm > "${TMPDIR:-/tmp}/ae.rs" 2>/dev/null
...
echo "# lark PRELUDE combinators: id / const / abs / minBy / maxBy"
chk_hm examples/hm-combinators.hm '"Int"'
CBW="22"
ssc run bin/larkc.ssc0 examples/hm-combinators.hm > "${TMPDIR:-/tmp}/ce.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-combinators.hm > "${TMPDIR:-/tmp}/ce.js" 2>/dev/null
...
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-combinators.hm > "${TMPDIR:-/tmp}/ce.rs" 2>/dev/null
...
echo "# lark FUNCTION-TYPED data fields: data F a = Op (Int -> F a) | Ret a  (free monads, K7-P1)"
chk_hm examples/hm-fnfield.hm '"Int"'
FFW="20"
ssc run bin/larkc.ssc0 examples/hm-fnfield.hm > "${TMPDIR:-/tmp}/ffe.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-fnfield.hm > "${TMPDIR:-/tmp}/ffe.js" 2>/dev/null
...
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-fnfield.hm > "${TMPDIR:-/tmp}/ffe.rs" 2>/dev/null
...
echo "# lark TYPED EFFECTS (K7-P): State (one-shot) + Nondeterminism (MULTI-SHOT) as typed free monads + do-notation"
chk_hm examples/hm-eff-state.hm '"Int"'
ESW="22"
ssc run bin/larkc.ssc0 examples/hm-eff-state.hm > "${TMPDIR:-/tmp}/es.coreir" 2>/dev/null
...
ENW="102"
ssc run bin/larkc.ssc0 examples/hm-eff-nondet.hm > "${TMPDIR:-/tmp}/en.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-eff-state.hm > "${TMPDIR:-/tmp}/es.js" 2>/dev/null
  ...
  ssc run bin/lark-js.ssc0 examples/hm-eff-nondet.hm > "${TMPDIR:-/tmp}/en.js" 2>/dev/null
...
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-eff-state.hm > "${TMPDIR:-/tmp}/es.rs" 2>/dev/null
  ...
  ssc run bin/lark-rust.ssc0 examples/hm-eff-nondet.hm > "${TMPDIR:-/tmp}/en.rs" 2>/dev/null
...
echo "# lark UNIVERSAL Comp via Dyn (K7-E): one effect monad for all ops; perform + label-dispatch handler"
chk_hm examples/hm-eff-comp.hm '"Int"'
ECW="41"
ssc run bin/larkc.ssc0 examples/hm-eff-comp.hm > "${TMPDIR:-/tmp}/ec.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-eff-comp.hm > "${TMPDIR:-/tmp}/ec.js" 2>/dev/null
...
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-eff-comp.hm > "${TMPDIR:-/tmp}/ec.rs" 2>/dev/null
...
echo -n "ok   Dyn round-trip          => "; printf '((5 : Dyn) : Int) + 1' > "${TMPDIR:-/tmp}/dyn.hm"; ssc run bin/larkc.ssc0 "${TMPDIR:-/tmp}/dyn.hm" > "${TMPDIR:-/tmp}/dyn.coreir" 2>/dev/null; dg=$(ssc run-ir "${TMPDIR:-/tmp}/dyn.coreir" | tail -1); if [ "$dg" = "6" ]; then echo "6"; else echo "FAIL [$dg]"; fail=1; fi
echo "# lark OVERLOADED arithmetic (K8.1): + - * resolve to Int or Float by operand type, all backends"
chk_hm examples/hm-numops.hm '"Float"'
NOW="13.56"
ssc run bin/larkc.ssc0 examples/hm-numops.hm > "${TMPDIR:-/tmp}/no.coreir" 2>/dev/null
got=$(ssc run-ir "${TMPDIR:-/tmp}/no.coreir" | tail -1)
if [ "$got" = "$NOW" ]; then printf 'ok   %-26s => %s\n' "float +/* -> run-ir" "$got"; else printf 'FAIL %-26s got [%s]\n' "numops" "$got"; fail=1; fi
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-numops.hm > "${TMPDIR:-/tmp}/no.js" 2>/dev/null
  got=$(node "${TMPDIR:-/tmp}/no.js" 2>/dev/null | tail -1)
  if [ "$got" = "$NOW" ]; then printf 'ok   %-26s => %s (node)\n' "float +/* -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "numops JS" "$got"; fail=1; fi
fi
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-numops.hm > "${TMPDIR:-/tmp}/no.rs" 2>/dev/null
...
echo -n "ok   int +/* still Int       => "; printf 'let sq = fun n => n * n + 1 in sq 6' > "${TMPDIR:-/tmp}/ia.hm"; ssc run bin/larkc.ssc0 "${TMPDIR:-/tmp}/ia.hm" > "${TMPDIR:-/tmp}/ia.coreir" 2>/dev/null; ig=$(ssc run-ir "${TMPDIR:-/tmp}/ia.coreir" | tail -1); if [ "$ig" = "37" ]; then echo "37"; else echo "FAIL [$ig]"; fail=1; fi
echo "# lark OVERLOADED comparisons (K8.2): < = (and > <= >= <>) on Int or Float, all backends"
chk_hm examples/hm-numcmp.hm '"Float"'
NCW="1.75"
ssc run bin/larkc.ssc0 examples/hm-numcmp.hm > "${TMPDIR:-/tmp}/nc.coreir" 2>/dev/null
got=$(ssc run-ir "${TMPDIR:-/tmp}/nc.coreir" | tail -1)
if [ "$got" = "$NCW" ]; then printf 'ok   %-26s => %s\n' "float < -> run-ir" "$got"; else printf 'FAIL %-26s got [%s]\n' "numcmp" "$got"; fail=1; fi
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-numcmp.hm > "${TMPDIR:-/tmp}/nc.js" 2>/dev/null
  got=$(node "${TMPDIR:-/tmp}/nc.js" 2>/dev/null | tail -1)
  if [ "$got" = "$NCW" ]; then printf 'ok   %-26s => %s (node)\n' "float < -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "numcmp JS" "$got"; fail=1; fi
fi
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-numcmp.hm > "${TMPDIR:-/tmp}/nc.rs" 2>/dev/null
...
echo -n "ok   int < still Int         => "; printf 'if 3 < 5 then 1 else 0' > "${TMPDIR:-/tmp}/ic.hm"; ssc run bin/larkc.ssc0 "${TMPDIR:-/tmp}/ic.hm" > "${TMPDIR:-/tmp}/ic.coreir" 2>/dev/null; icg=$(ssc run-ir "${TMPDIR:-/tmp}/ic.coreir" | tail -1); if [ "$icg" = "1" ]; then echo "1"; else echo "FAIL [$icg]"; fail=1; fi
echo "# lark OVERLOADED division (K8.3): '/' resolves to Int (truncating) or Float by operand type, all backends"
chk_hm examples/hm-div.hm '"(Int, Float)"'                            # (20 / 6, 9.0 / 2.0): Int division truncates, Float division does not
DIVW="Pair(3, 4.5)"
ssc run bin/larkc.ssc0 examples/hm-div.hm > "${TMPDIR:-/tmp}/dv.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-div.hm > "${TMPDIR:-/tmp}/dv.js" 2>/dev/null
...
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-div.hm > "${TMPDIR:-/tmp}/dv.rs" 2>/dev/null
...
echo -n "ok   mixed div rejected       => "; printf '9.0 / 2' > "${TMPDIR:-/tmp}/dmx.hm"; dmx=$(ssc run bin/lark.ssc0 "${TMPDIR:-/tmp}/dmx.hm" | tail -1); if [ "$dmx" = '"TypeError: arithmetic operands must have the same type"' ]; then echo "Int/Float mix rejected (correct)"; else echo "FAIL [$dmx]"; fail=1; fi
echo "# lark TYPED ASYNC: cooperative scheduler (yield/log) on the typed effect monad, all backends"
chk_hm examples/hm-async.hm '"[Int]"'
AYW="Cons(1, Cons(2, Cons(101, Cons(102, Nil))))"
ssc run bin/larkc.ssc0 examples/hm-async.hm > "${TMPDIR:-/tmp}/ay.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-async.hm > "${TMPDIR:-/tmp}/ay.js" 2>/dev/null
...
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-async.hm > "${TMPDIR:-/tmp}/ay.rs" 2>/dev/null
...
echo "# lark TYPED ACTORS: stateful behavior (state, msg) -> (state', out) over a message stream, all backends"
chk_hm examples/hm-actors.hm '"[Int]"'
ATW="Cons(2, Cons(3, Cons(2, Nil)))"
ssc run bin/larkc.ssc0 examples/hm-actors.hm > "${TMPDIR:-/tmp}/at.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-actors.hm > "${TMPDIR:-/tmp}/at.js" 2>/dev/null
...
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-actors.hm > "${TMPDIR:-/tmp}/at.rs" 2>/dev/null
...
echo "# lark EFFECT ROWS (K10): type tracks effects + runE rejects unhandled; runs on all backends"
chk_hm examples/hm-effrow.hm '"(Int, Int)"'                          # put 5; get; return get+100 -> handled, runs
ERV="Pair(105, 5)"
ssc run bin/larkc.ssc0 examples/hm-effrow.hm > "${TMPDIR:-/tmp}/er.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-effrow.hm > "${TMPDIR:-/tmp}/er.js" 2>/dev/null
...
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-effrow.hm > "${TMPDIR:-/tmp}/er.rs" 2>/dev/null
...
echo -n "ok   effect tracked in type   => "; printf 'getE' > "${TMPDIR:-/tmp}/er1.hm"; et=$(ssc run bin/lark.ssc0 "${TMPDIR:-/tmp}/er1.hm" | tail -1); if [ "$et" = '"Comp {State | e0} Dyn"' ]; then echo "Comp {State | e0} Dyn"; else echo "FAIL [$et]"; fail=1; fi
echo -n "ok   unhandled effect = error => "; printf 'runE getE' > "${TMPDIR:-/tmp}/er2.hm"; eu=$(ssc run bin/lark.ssc0 "${TMPDIR:-/tmp}/er2.hm" | tail -1); if [ "$eu" = '"TypeError: effect not handled: State"' ]; then echo "rejected (correct)"; else echo "FAIL [$eu]"; fail=1; fi
...
ssc run bin/larkc.ssc0 examples/hm-eff2.hm > "${TMPDIR:-/tmp}/e2.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-eff2.hm > "${TMPDIR:-/tmp}/e2.js" 2>/dev/null
...
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-eff2.hm > "${TMPDIR:-/tmp}/e2.rs" 2>/dev/null
...
echo -n "ok   second effect tracked too => "; printf 'runE (runStateE (bindE (logE (7 : Dyn)) (fun w => bindE getE (fun x => pureE ((x : Int) + 1)))) 0)' > "${TMPDIR:-/tmp}/er3.hm"; el=$(ssc run bin/lark.ssc0 "${TMPDIR:-/tmp}/er3.hm" | tail -1); if [ "$el" = '"TypeError: effect not handled: Log"' ]; then echo "Log unhandled rejected (correct)"; else echo "FAIL [$el]"; fail=1; fi
...
echo -n "ok   traverseE is effect-poly  => "; printf 'let rec traverseE = fun f => fun xs => if isNil xs then pureE nil else bindE (f (head xs)) (fun y => bindE (traverseE f (tail xs)) (fun ys => pureE (cons y ys))) in traverseE' > "${TMPDIR:-/tmp}/tvt.hm"; tvt=$(ssc run bin/lark.ssc0 "${TMPDIR:-/tmp}/tvt.hm" | tail -1); if [ "$tvt" = '"((t22 -> Comp e21 t20) -> ([t22] -> Comp e21 [t20]))"' ]; then echo "row var e21 inferred + propagated"; else echo "FAIL [$tvt]"; fail=1; fi
...
ssc run bin/larkc.ssc0 examples/hm-eff-traverse.hm > "${TMPDIR:-/tmp}/tv.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then ssc run bin/lark-js.ssc0 examples/hm-eff-traverse.hm > "${TMPDIR:-/tmp}/tv.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/tv.js" 2>/dev/null | tail -1); if [ "$got" = "$TVV" ]; then printf 'ok   %-26s => %s (node)\n' "eff-poly traverse -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "eff-traverse JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/lark-rust.ssc0 examples/hm-eff-traverse.hm > "${TMPDIR:-/tmp}/tv.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/tv.rs" -o "${TMPDIR:-/tmp}/tv-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/tv-bin"); else got="(rustc err)"; fi; if [ "$got" = "$TVV" ]; then printf 'ok   %-26s => %s (rustc)\n' "eff-poly traverse -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "eff-traverse Rust" "$got"; fail=1; fi; fi
...
ssc run bin/larkc.ssc0 examples/hm-eff-handle.hm > "${TMPDIR:-/tmp}/eh.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-eff-handle.hm > "${TMPDIR:-/tmp}/eh.js" 2>/dev/null
...
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-eff-handle.hm > "${TMPDIR:-/tmp}/eh.rs" 2>/dev/null
...
echo -n "ok   user effect tracked too   => "; printf 'runE (perform "Choose" "flip" (0 : Dyn))' > "${TMPDIR:-/tmp}/er4.hm"; ec=$(ssc run bin/lark.ssc0 "${TMPDIR:-/tmp}/er4.hm" | tail -1); if [ "$ec" = '"TypeError: effect not handled: Choose"' ]; then echo "user Choose unhandled rejected (correct)"; else echo "FAIL [$ec]"; fail=1; fi
...
USV="Pair(105, 5)"
ssc run bin/larkc.ssc0 examples/hm-eff-userstate.hm > "${TMPDIR:-/tmp}/us.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-eff-userstate.hm > "${TMPDIR:-/tmp}/us.js" 2>/dev/null
...
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-eff-userstate.hm > "${TMPDIR:-/tmp}/us.rs" 2>/dev/null
...
DOV="Pair(105, 5)"
ssc run bin/larkc.ssc0 examples/hm-eff-do.hm > "${TMPDIR:-/tmp}/edo.coreir" 2>/dev/null
...
ssc run bin/larkc.ssc0 examples/hm-eff-do-nondet.hm > "${TMPDIR:-/tmp}/edn.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-eff-do.hm > "${TMPDIR:-/tmp}/edo.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/edo.js" 2>/dev/null | tail -1)
  if [ "$got" = "$DOV" ]; then printf 'ok   %-26s => %s (node)\n' "doE State -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "eff-do JS" "$got"; fail=1; fi
  ssc run bin/lark-js.ssc0 examples/hm-eff-do-nondet.hm > "${TMPDIR:-/tmp}/edn.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/edn.js" 2>/dev/null | tail -1)
  if [ "$got" = "$DNV" ]; then printf 'ok   %-26s => %s (node)\n' "doE nondet -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "eff-do-nondet JS" "$got"; fail=1; fi
fi
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-eff-do.hm > "${TMPDIR:-/tmp}/edo.rs" 2>/dev/null
...
EDV="Pair(105, 5)"
ssc run bin/larkc.ssc0 examples/hm-eff-decl.hm > "${TMPDIR:-/tmp}/edl.coreir" 2>/dev/null
...
ssc run bin/larkc.ssc0 examples/hm-eff-decl-choose.hm > "${TMPDIR:-/tmp}/edc.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-eff-decl.hm > "${TMPDIR:-/tmp}/edl.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/edl.js" 2>/dev/null | tail -1)
  if [ "$got" = "$EDV" ]; then printf 'ok   %-26s => %s (node)\n' "effect decl State -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "eff-decl JS" "$got"; fail=1; fi
  ssc run bin/lark-js.ssc0 examples/hm-eff-decl-choose.hm > "${TMPDIR:-/tmp}/edc.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/edc.js" 2>/dev/null | tail -1)
...
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-eff-decl-choose.hm > "${TMPDIR:-/tmp}/edc.rs" 2>/dev/null
...
echo -n "ok   declared op tracked too   => "; printf 'effect Choose flip in runE (flip (0 : Dyn))' > "${TMPDIR:-/tmp}/edu.hm"; edu=$(ssc run bin/lark.ssc0 "${TMPDIR:-/tmp}/edu.hm" | tail -1); if [ "$edu" = '"TypeError: effect not handled: Choose"' ]; then echo "declared Choose unhandled rejected (correct)"; else echo "FAIL [$edu]"; fail=1; fi
echo "# K11.2 — effect-row syntax in the type parser: {} / {l} / {l, m} / {l | r} in ascriptions"
echo -n "ok   closed row ascription     => "; printf '(getE : Comp {State} Dyn)' > "${TMPDIR:-/tmp}/rs1.hm"; rs1=$(ssc run bin/lark.ssc0 "${TMPDIR:-/tmp}/rs1.hm" | tail -1); if [ "$rs1" = '"Comp {State} Dyn"' ]; then echo "Comp {State} Dyn"; else echo "FAIL [$rs1]"; fail=1; fi
echo -n "ok   open row ascription       => "; printf '(getE : Comp {State | r} Dyn)' > "${TMPDIR:-/tmp}/rs2.hm"; rs2=$(ssc run bin/lark.ssc0 "${TMPDIR:-/tmp}/rs2.hm" | tail -1); case "$rs2" in '"Comp {State | e'*'} Dyn"') echo "$rs2 (open)";; *) echo "FAIL [$rs2]"; fail=1;; esac
echo -n "ok   wrong row ascr rejected   => "; printf '(getE : Comp {Log} Dyn)' > "${TMPDIR:-/tmp}/rs3.hm"; rs3=$(ssc run bin/lark.ssc0 "${TMPDIR:-/tmp}/rs3.hm" | tail -1); if [ "$rs3" = '"TypeError: type ascription mismatch"' ]; then echo "State vs Log rejected (correct)"; else echo "FAIL [$rs3]"; fail=1; fi
chk_hm examples/hm-eff-rowann.hm '"(Int, Int)"'                       # the doE block ascribed : Comp {State} Int
RAV="Pair(105, 5)"
ssc run bin/larkc.ssc0 examples/hm-eff-rowann.hm > "${TMPDIR:-/tmp}/ra.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-eff-rowann.hm > "${TMPDIR:-/tmp}/ra.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/ra.js" 2>/dev/null | tail -1)
  if [ "$got" = "$RAV" ]; then printf 'ok   %-26s => %s (node)\n' "row ascription -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "eff-rowann JS" "$got"; fail=1; fi
fi
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-eff-rowann.hm > "${TMPDIR:-/tmp}/ra.rs" 2>/dev/null
...
PNV="Pair(10, 4.5)"
ssc run bin/larkc.ssc0 examples/hm-poly-num.hm > "${TMPDIR:-/tmp}/pn.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-poly-num.hm > "${TMPDIR:-/tmp}/pn.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/pn.js" 2>/dev/null | tail -1)
  if [ "$got" = "$PNV" ]; then printf 'ok   %-26s => %s (node)\n' "poly numeric -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "poly-num JS" "$got"; fail=1; fi
fi
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-poly-num.hm > "${TMPDIR:-/tmp}/pn.rs" 2>/dev/null
...
echo -n "ok   int helper still Int     => "; printf 'let sq = fun n => n * n in sq 7' > "${TMPDIR:-/tmp}/sq.hm"; sqg=$(ssc run bin/larkc.ssc0 "${TMPDIR:-/tmp}/sq.hm" 2>/dev/null > "${TMPDIR:-/tmp}/sq.coreir"; ssc run-ir "${TMPDIR:-/tmp}/sq.coreir" | tail -1); if [ "$sqg" = "49" ]; then echo "49"; else echo "FAIL [$sqg]"; fail=1; fi
...
TPV="Pair(105, 5)"
ssc run bin/larkc.ssc0 examples/hm-eff-typed.hm > "${TMPDIR:-/tmp}/tp.coreir" 2>/dev/null
got=$(ssc run-ir "${TMPDIR:-/tmp}/tp.coreir" | tail -1)
if [ "$got" = "$TPV" ]; then printf 'ok   %-26s => %s\n' "typed payloads -> run-ir" "$got"; else printf 'FAIL %-26s got [%s]\n' "eff-typed" "$got"; fail=1; fi
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-eff-typed.hm > "${TMPDIR:-/tmp}/tp.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/tp.js" 2>/dev/null | tail -1)
  if [ "$got" = "$TPV" ]; then printf 'ok   %-26s => %s (node)\n' "typed payloads -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "eff-typed JS" "$got"; fail=1; fi
fi
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-eff-typed.hm > "${TMPDIR:-/tmp}/tp.rs" 2>/dev/null
  if rustc -O "${TMPDIR:-/tmp}/tp.rs" -o "${TMPDIR:-/tmp}/tp-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/tp-bin"); else got="(rustc err)"; fi
  if [ "$got" = "$TPV" ]; then printf 'ok   %-26s => %s (rustc)\n' "typed payloads -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "eff-typed Rust" "$got"; fail=1; fi
fi
echo -n "ok   typed op arg checked     => "; printf 'effect State { get : Dyn -> Int , put : Int -> Dyn } in runE (runStateE (doE { u <- put "x" ; x <- get ; pureE (x + 1) }) 0)' > "${TMPDIR:-/tmp}/tpb.hm"; tpb=$(ssc run bin/lark.ssc0 "${TMPDIR:-/tmp}/tpb.hm" | tail -1); if [ "$tpb" = '"TypeError: effect op arg type mismatch for put"' ]; then echo "put String rejected (correct)"; else echo "FAIL [$tpb]"; fail=1; fi
...
ssc run bin/larkc.ssc0 examples/hm-eff-typed-resume.hm > "${TMPDIR:-/tmp}/tr.coreir" 2>/dev/null
got=$(ssc run-ir "${TMPDIR:-/tmp}/tr.coreir" | tail -1)
if [ "$got" = "2" ]; then printf 'ok   %-26s => %s\n' "typed resume -> run-ir" "$got"; else printf 'FAIL %-26s got [%s]\n' "typed resume" "$got"; fail=1; fi
if command -v node >/dev/null 2>&1; then ssc run bin/lark-js.ssc0 examples/hm-eff-typed-resume.hm > "${TMPDIR:-/tmp}/tr.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/tr.js" 2>/dev/null | tail -1); if [ "$got" = "2" ]; then printf 'ok   %-26s => %s (node)\n' "typed resume -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "typed resume JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/lark-rust.ssc0 examples/hm-eff-typed-resume.hm > "${TMPDIR:-/tmp}/tr.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/tr.rs" -o "${TMPDIR:-/tmp}/tr-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/tr-bin"); else got="(rustc err)"; fi; if [ "$got" = "2" ]; then printf 'ok   %-26s => %s (rustc)\n' "typed resume -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "typed resume Rust" "$got"; fail=1; fi; fi
echo -n "ok   wrong-type resume rejected => "; printf 'effect Ask { ask : Int -> String } in runE (handle "Ask" (bindE (ask 41) (fun s => pureE (strLen s))) (fun v => pureE v) (fun o => fun a => fun k => k (a + 1)))' > "${TMPDIR:-/tmp}/trb.hm"; trb=$(ssc run bin/lark.ssc0 "${TMPDIR:-/tmp}/trb.hm" | tail -1); if [ "$trb" = '"TypeError: handle: op clause type mismatch"' ]; then echo "resuming with Int (k expects String) rejected (correct)"; else echo "FAIL [$trb]"; fail=1; fi
...
MV="42"
ssc run bin/larkc.ssc0 examples/hm-eff-multiop.hm > "${TMPDIR:-/tmp}/mo48.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then ssc run bin/lark-js.ssc0 examples/hm-eff-multiop.hm > "${TMPDIR:-/tmp}/mo48.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/mo48.js" 2>/dev/null | tail -1); if [ "$got" = "$MV" ]; then printf 'ok   %-26s => %s (node)\n' "handleM -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "handleM JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/lark-rust.ssc0 examples/hm-eff-multiop.hm > "${TMPDIR:-/tmp}/mo48.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/mo48.rs" -o "${TMPDIR:-/tmp}/mo48-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/mo48-bin"); else got="(rustc err)"; fi; if [ "$got" = "$MV" ]; then printf 'ok   %-26s => %s (rustc)\n' "handleM -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "handleM Rust" "$got"; fail=1; fi; fi
echo -n "ok   handleM row composition => "; printf 'effect QA { ask : Dyn -> Int , tell : String -> String } in let prog = doE { x <- ask ; u <- logE "seen" ; t <- tell "hello" ; pureE (x * 2) } in runE (runLogE (handleM "QA" prog { | ask a k => k 21 | tell a k => k "done" } (fun v => pureE v)))' > "${TMPDIR:-/tmp}/mo48compose.hm"; mc=$(ssc run bin/lark.ssc0 "${TMPDIR:-/tmp}/mo48compose.hm" | tail -1); ssc run bin/larkc.ssc0 "${TMPDIR:-/tmp}/mo48compose.hm" > "${TMPDIR:-/tmp}/mo48compose.coreir" 2>/dev/null; got=$(ssc run-ir "${TMPDIR:-/tmp}/mo48compose.coreir" | tail -1); if [ "$mc" = '"(Int, [Dyn])"' ] && [ "$got" = 'Pair(42, Cons("seen", Nil))' ]; then echo "handled QA composes with Log"; else echo "FAIL type [$mc] run [$got]"; fail=1; fi
echo -n "ok   handleM wrong-type rejected => "; printf 'effect QA { ask : Dyn -> Int , tell : String -> String } in runE (handleM "QA" (doE { x <- ask ; u <- tell "hi" ; pureE (x * 2) }) { | ask a k => k 21 | tell a k => k 99 } (fun v => pureE v))' > "${TMPDIR:-/tmp}/mo48b.hm"; mb=$(ssc run bin/lark.ssc0 "${TMPDIR:-/tmp}/mo48b.hm" | tail -1); if echo "$mb" | grep -q TypeError; then echo "tell arm k 99 (Int, not String) rejected (correct)"; else echo "FAIL [$mb]"; fail=1; fi
echo -n "ok   handleM missing-arm rejected => "; printf 'effect QA { ask : Dyn -> Int , tell : String -> String } in runE (handleM "QA" (doE { x <- ask ; u <- tell "hi" ; pureE x }) { | ask a k => k 21 } (fun v => pureE v))' > "${TMPDIR:-/tmp}/mo48missing.hm"; mm=$(ssc run bin/lark.ssc0 "${TMPDIR:-/tmp}/mo48missing.hm" | tail -1); if echo "$mm" | grep -q TypeError; then echo "missing tell arm rejected (correct)"; else echo "FAIL [$mm]"; fail=1; fi
echo -n "ok   handleM foreign-arm rejected => "; printf 'effect QA { ask : Dyn -> Int , tell : String -> String } in effect Other { ping : Dyn -> Int } in runE (handleM "QA" (doE { x <- ask ; u <- tell "hi" ; pureE x }) { | ask a k => k 21 | tell a k => k "ok" | ping a k => k 1 } (fun v => pureE v))' > "${TMPDIR:-/tmp}/mo48foreign.hm"; mf=$(ssc run bin/lark.ssc0 "${TMPDIR:-/tmp}/mo48foreign.hm" | tail -1); if echo "$mf" | grep -q TypeError; then echo "foreign ping arm rejected (correct)"; else echo "FAIL [$mf]"; fail=1; fi
echo -n "ok   handleM duplicate-arm rejected => "; printf 'effect QA { ask : Dyn -> Int , tell : String -> String } in runE (handleM "QA" (doE { x <- ask ; u <- tell "hi" ; pureE x }) { | ask a k => k 21 | ask a k => k 22 | tell a k => k "ok" } (fun v => pureE v))' > "${TMPDIR:-/tmp}/mo48dup.hm"; md=$(ssc run bin/lark.ssc0 "${TMPDIR:-/tmp}/mo48dup.hm" | tail -1); if echo "$md" | grep -q TypeError; then echo "duplicate ask arm rejected (correct)"; else echo "FAIL [$md]"; fail=1; fi
...
ssc run bin/larkc.ssc0 examples/hm-method-poly.hm > "${TMPDIR:-/tmp}/mp.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-method-poly.hm > "${TMPDIR:-/tmp}/mp.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/mp.js" 2>/dev/null | tail -1)
  if [ "$got" = "$MPV" ]; then printf 'ok   %-26s => %s (node)\n' "method poly -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "method-poly JS" "$got"; fail=1; fi
fi
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-method-poly.hm > "${TMPDIR:-/tmp}/mp.rs" 2>/dev/null
...
echo -n "ok   eager method still works => "; printf 'method sz in instance sz Int = fun n => n + 1 in instance sz Bool = fun b => 0 in sz 5' > "${TMPDIR:-/tmp}/eg.hm"; ssc run bin/larkc.ssc0 "${TMPDIR:-/tmp}/eg.hm" 2>/dev/null > "${TMPDIR:-/tmp}/eg.coreir"; egg=$(ssc run-ir "${TMPDIR:-/tmp}/eg.coreir" | tail -1); if [ "$egg" = "6" ]; then echo "6 (monomorphic, no sig)"; else echo "FAIL [$egg]"; fail=1; fi
...
MOV="Pair(200, 111)"
ssc run bin/larkc.ssc0 examples/hm-method-poly-ops.hm > "${TMPDIR:-/tmp}/mo.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-method-poly-ops.hm > "${TMPDIR:-/tmp}/mo.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/mo.js" 2>/dev/null | tail -1)
  if [ "$got" = "$MOV" ]; then printf 'ok   %-26s => %s (node)\n' "method poly (ops) -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "method-poly-ops JS" "$got"; fail=1; fi
fi
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-method-poly-ops.hm > "${TMPDIR:-/tmp}/mo.rs" 2>/dev/null
...
MSV="Pair(-5, -2.5)"
ssc run bin/larkc.ssc0 examples/hm-method-self.hm > "${TMPDIR:-/tmp}/ms.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-method-self.hm > "${TMPDIR:-/tmp}/ms.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/ms.js" 2>/dev/null | tail -1)
  if [ "$got" = "$MSV" ]; then printf 'ok   %-26s => %s (node)\n' "method self -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "method-self JS" "$got"; fail=1; fi
fi
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-method-self.hm > "${TMPDIR:-/tmp}/ms.rs" 2>/dev/null
...
K50W="Pair(3, 1.5)"
ssc run bin/larkc.ssc0 examples/hm-method-binary.hm > "${TMPDIR:-/tmp}/mb.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-method-binary.hm > "${TMPDIR:-/tmp}/mb.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/mb.js" 2>/dev/null | tail -1)
  if [ "$got" = "$K50W" ]; then printf 'ok   %-26s => %s (node)\n' "method binary -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "method-binary JS" "$got"; fail=1; fi
fi
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-method-binary.hm > "${TMPDIR:-/tmp}/mb.rs" 2>/dev/null
...
ssc run bin/larkc.ssc0 examples/hm-mutual.hm > "${TMPDIR:-/tmp}/mut.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-mutual.hm > "${TMPDIR:-/tmp}/mut.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/mut.js" 2>/dev/null | tail -1)
  if [ "$got" = "true" ]; then printf 'ok   %-26s => %s (node)\n' "mutual rec -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "mutual JS" "$got"; fail=1; fi
fi
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-mutual.hm > "${TMPDIR:-/tmp}/mut.rs" 2>/dev/null
...
echo -n "ok   3-way mutual rec         => "; printf 'let rec f = fun n => if n = 0 then 0 else g (n - 1) and g = fun n => if n = 0 then 1 else h (n - 1) and h = fun n => if n = 0 then 2 else f (n - 1) in f 7' > "${TMPDIR:-/tmp}/m3.hm"; ssc run bin/larkc.ssc0 "${TMPDIR:-/tmp}/m3.hm" 2>/dev/null > "${TMPDIR:-/tmp}/m3.coreir"; m3=$(ssc run-ir "${TMPDIR:-/tmp}/m3.coreir" | tail -1); if [ "$m3" = "1" ]; then echo "1 (f→g→h→f…)"; else echo "FAIL [$m3]"; fail=1; fi
echo '# 4-TUPLES (Quad): `(a,b,c,d)` no longer silently truncates to Triple; 5+ nest the tail (no data loss)'
echo -n "ok   4-tuple type            => "; printf '(1, 2, 3, 4)' > "${TMPDIR:-/tmp}/q.hm"; qt=$(ssc run bin/lark.ssc0 "${TMPDIR:-/tmp}/q.hm" | tail -1); if [ "$qt" = '"(Int, Int, Int, Int)"' ]; then echo "(Int, Int, Int, Int)"; else echo "FAIL [$qt]"; fail=1; fi
chk_hm examples/hm-quad.hm '"Int"'                                    # 4-tuple pattern (a, b, c, d) => sum
ssc run bin/larkc.ssc0 examples/hm-quad.hm > "${TMPDIR:-/tmp}/q.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then ssc run bin/lark-js.ssc0 examples/hm-quad.hm > "${TMPDIR:-/tmp}/q.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/q.js" 2>/dev/null | tail -1); if [ "$got" = "10" ]; then printf 'ok   %-26s => %s (node)\n' "4-tuple -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "quad JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/lark-rust.ssc0 examples/hm-quad.hm > "${TMPDIR:-/tmp}/q.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/q.rs" -o "${TMPDIR:-/tmp}/q-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/q-bin"); else got="(rustc err)"; fi; if [ "$got" = "10" ]; then printf 'ok   %-26s => %s (rustc)\n' "4-tuple -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "quad Rust" "$got"; fail=1; fi; fi
echo -n "ok   5-tuple no data loss     => "; printf 'match (1, 2, 3, 4, 5) { (a, b, c, (d, e)) => a + e }' > "${TMPDIR:-/tmp}/q5.hm"; ssc run bin/larkc.ssc0 "${TMPDIR:-/tmp}/q5.hm" 2>/dev/null > "${TMPDIR:-/tmp}/q5.coreir"; q5=$(ssc run-ir "${TMPDIR:-/tmp}/q5.coreir" | tail -1); if [ "$q5" = "6" ]; then echo "6 (nested, a+e)"; else echo "FAIL [$q5]"; fail=1; fi
...
ssc run bin/larkc.ssc0 examples/hm-guard.hm > "${TMPDIR:-/tmp}/g.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then ssc run bin/lark-js.ssc0 examples/hm-guard.hm > "${TMPDIR:-/tmp}/g.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/g.js" 2>/dev/null | tail -1); if [ "$got" = "0" ]; then printf 'ok   %-26s => %s (node)\n' "pattern guards -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "guard JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/lark-rust.ssc0 examples/hm-guard.hm > "${TMPDIR:-/tmp}/g.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/g.rs" -o "${TMPDIR:-/tmp}/g-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/g-bin"); else got="(rustc err)"; fi; if [ "$got" = "0" ]; then printf 'ok   %-26s => %s (rustc)\n' "pattern guards -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "guard Rust" "$got"; fail=1; fi; fi
echo -n "ok   guard on Con + fallthrough => "; printf 'data O = Som Int | Non in match (Som 5) { Som x if x > 10 => 100 | Som x => x | Non => 0 }' > "${TMPDIR:-/tmp}/gc.hm"; ssc run bin/larkc.ssc0 "${TMPDIR:-/tmp}/gc.hm" 2>/dev/null > "${TMPDIR:-/tmp}/gc.coreir"; gc=$(ssc run-ir "${TMPDIR:-/tmp}/gc.coreir" | tail -1); if [ "$gc" = "5" ]; then echo "5 (guard false -> next arm)"; else echo "FAIL [$gc]"; fail=1; fi
echo "# STRING ESCAPES: \\n \\t \\r \\\\ \\\" in string literals; backends re-escape on emit (Core IR already round-trips)"
chk_hm examples/hm-escapes.hm '"Int"'                                 # strLen "a\nb\tc"
ssc run bin/larkc.ssc0 examples/hm-escapes.hm > "${TMPDIR:-/tmp}/se.coreir" 2>/dev/null
got=$(ssc run-ir "${TMPDIR:-/tmp}/se.coreir" | tail -1)
if [ "$got" = "5" ]; then printf 'ok   %-26s => %s\n' "string escapes -> run-ir" "$got"; else printf 'FAIL %-26s got [%s]\n' "escapes" "$got"; fail=1; fi
if command -v node >/dev/null 2>&1; then ssc run bin/lark-js.ssc0 examples/hm-escapes.hm > "${TMPDIR:-/tmp}/se.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/se.js" 2>/dev/null | tail -1); if [ "$got" = "5" ]; then printf 'ok   %-26s => %s (node)\n' "string escapes -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "escapes JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/lark-rust.ssc0 examples/hm-escapes.hm > "${TMPDIR:-/tmp}/se.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/se.rs" -o "${TMPDIR:-/tmp}/se-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/se-bin"); else got="(rustc err)"; fi; if [ "$got" = "5" ]; then printf 'ok   %-26s => %s (rustc)\n' "string escapes -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "escapes Rust" "$got"; fail=1; fi; fi
echo -n "ok   escaped quote in string  => "; printf 'strLen "x\\"y"' > "${TMPDIR:-/tmp}/sq.hm"; ssc run bin/larkc.ssc0 "${TMPDIR:-/tmp}/sq.hm" 2>/dev/null > "${TMPDIR:-/tmp}/sq.coreir"; sq=$(ssc run-ir "${TMPDIR:-/tmp}/sq.coreir" | tail -1); if [ "$sq" = "3" ]; then echo "3 (x\"y, quote not a terminator)"; else echo "FAIL [$sq]"; fail=1; fi
...
ssc run bin/larkc.ssc0 examples/hm-recupd.hm > "${TMPDIR:-/tmp}/ru.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then ssc run bin/lark-js.ssc0 examples/hm-recupd.hm > "${TMPDIR:-/tmp}/ru.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/ru.js" 2>/dev/null | tail -1); if [ "$got" = "11" ]; then printf 'ok   %-26s => %s (node)\n' "record update -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "recupd JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/lark-rust.ssc0 examples/hm-recupd.hm > "${TMPDIR:-/tmp}/ru.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/ru.rs" -o "${TMPDIR:-/tmp}/ru-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/ru-bin"); else got="(rustc err)"; fi; if [ "$got" = "11" ]; then printf 'ok   %-26s => %s (rustc)\n' "record update -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "recupd Rust" "$got"; fail=1; fi; fi
echo -n "ok   multi-field + unknown-field => "; printf 'let r = {x = 1, y = 2} in let r2 = {r with x = 9, y = 8} in r2.x + r2.y' > "${TMPDIR:-/tmp}/ru2.hm"; ssc run bin/larkc.ssc0 "${TMPDIR:-/tmp}/ru2.hm" 2>/dev/null > "${TMPDIR:-/tmp}/ru2.coreir"; ru2=$(ssc run-ir "${TMPDIR:-/tmp}/ru2.coreir" | tail -1); printf 'let r = {x = 1} in {r with z = 9}' > "${TMPDIR:-/tmp}/ru3.hm"; ru3=$(ssc run bin/lark.ssc0 "${TMPDIR:-/tmp}/ru3.hm" | tail -1); if [ "$ru2" = "17" ] && [ "$ru3" = '"TypeError: record has no field z"' ]; then echo "17 ; unknown field rejected"; else echo "FAIL [$ru2 / $ru3]"; fail=1; fi
...
ssc run bin/larkc.ssc0 examples/hm-neg.hm > "${TMPDIR:-/tmp}/neg.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then ssc run bin/lark-js.ssc0 examples/hm-neg.hm > "${TMPDIR:-/tmp}/neg.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/neg.js" 2>/dev/null | tail -1); if [ "$got" = "14" ]; then printf 'ok   %-26s => %s (node)\n' "neg literal -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "neg literal JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/lark-rust.ssc0 examples/hm-neg.hm > "${TMPDIR:-/tmp}/neg.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/neg.rs" -o "${TMPDIR:-/tmp}/neg-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/neg-bin"); else got="(rustc err)"; fi; if [ "$got" = "14" ]; then printf 'ok   %-26s => %s (rustc)\n' "neg literal -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "neg literal Rust" "$got"; fail=1; fi; fi
echo -n "ok   neg precedence + binary '-' => "; printf 'let n = -3 in n * -2' > "${TMPDIR:-/tmp}/ng2.hm"; ssc run bin/larkc.ssc0 "${TMPDIR:-/tmp}/ng2.hm" 2>/dev/null > "${TMPDIR:-/tmp}/ng2.coreir"; ng2=$(ssc run-ir "${TMPDIR:-/tmp}/ng2.coreir" | tail -1); printf 'let f = fun a => a - 1 in f 10' > "${TMPDIR:-/tmp}/ng3.hm"; ssc run bin/larkc.ssc0 "${TMPDIR:-/tmp}/ng3.hm" 2>/dev/null > "${TMPDIR:-/tmp}/ng3.coreir"; ng3=$(ssc run-ir "${TMPDIR:-/tmp}/ng3.coreir" | tail -1); if [ "$ng2" = "6" ] && [ "$ng3" = "9" ]; then echo "-3 * -2 = 6 ; binary a-1 still subtraction (=9)"; else echo "FAIL [$ng2 / $ng3]"; fail=1; fi
...
ssc run bin/larkc.ssc0 examples/hm-letann.hm > "${TMPDIR:-/tmp}/la.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then ssc run bin/lark-js.ssc0 examples/hm-letann.hm > "${TMPDIR:-/tmp}/la.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/la.js" 2>/dev/null | tail -1); if [ "$got" = "$LANN" ]; then printf 'ok   %-26s => %s (node)\n' "let annot -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "let annot JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/lark-rust.ssc0 examples/hm-letann.hm > "${TMPDIR:-/tmp}/la.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/la.rs" -o "${TMPDIR:-/tmp}/la-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/la-bin"); else got="(rustc err)"; fi; if [ "$got" = "$LANN" ]; then printf 'ok   %-26s => %s (rustc)\n' "let annot -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "let annot Rust" "$got"; fail=1; fi; fi
echo -n "ok   let annot enforced + plain   => "; printf 'let x : Int = true in x' > "${TMPDIR:-/tmp}/la2.hm"; la2=$(ssc run bin/lark.ssc0 "${TMPDIR:-/tmp}/la2.hm" | tail -1); printf 'let f x = x + 1 in f 9' > "${TMPDIR:-/tmp}/la3.hm"; ssc run bin/larkc.ssc0 "${TMPDIR:-/tmp}/la3.hm" 2>/dev/null > "${TMPDIR:-/tmp}/la3.coreir"; la3=$(ssc run-ir "${TMPDIR:-/tmp}/la3.coreir" | tail -1); if [ "$la2" = '"TypeError: type ascription mismatch"' ] && [ "$la3" = "10" ]; then echo "bad annot rejected ; fn-sugar 'let f x =' still works (=10)"; else echo "FAIL [$la2 / $la3]"; fail=1; fi
...
ssc run bin/larkc.ssc0 examples/hm-strord.hm > "${TMPDIR:-/tmp}/so.coreir" 2>/dev/null
got=$(ssc run-ir "${TMPDIR:-/tmp}/so.coreir" | tail -1)
if [ "$got" = "101" ]; then printf 'ok   %-26s => %s\n' "string ord -> run-ir" "$got"; else printf 'FAIL %-26s got [%s]\n' "string ord" "$got"; fail=1; fi
if command -v node >/dev/null 2>&1; then ssc run bin/lark-js.ssc0 examples/hm-strord.hm > "${TMPDIR:-/tmp}/so.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/so.js" 2>/dev/null | tail -1); if [ "$got" = "101" ]; then printf 'ok   %-26s => %s (node)\n' "string ord -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "string ord JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/lark-rust.ssc0 examples/hm-strord.hm > "${TMPDIR:-/tmp}/so.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/so.rs" -o "${TMPDIR:-/tmp}/so-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/so-bin"); else got="(rustc err)"; fi; if [ "$got" = "101" ]; then printf 'ok   %-26s => %s (rustc)\n' "string ord -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "string ord Rust" "$got"; fail=1; fi; fi
echo -n "ok   string lexicographic + guard => "; printf 'if "apple" < "banana" then (if "cat" < "car" then 1 else 7) else 0' > "${TMPDIR:-/tmp}/so2.hm"; ssc run bin/larkc.ssc0 "${TMPDIR:-/tmp}/so2.hm" 2>/dev/null > "${TMPDIR:-/tmp}/so2.coreir"; so2=$(ssc run-ir "${TMPDIR:-/tmp}/so2.coreir" | tail -1); printf '"x" + "y"' > "${TMPDIR:-/tmp}/so3.hm"; so3=$(ssc run bin/lark.ssc0 "${TMPDIR:-/tmp}/so3.hm" | tail -1); if [ "$so2" = "7" ] && [ "$so3" = '"TypeError: + - * need Int or Float operands (use ++ to concatenate strings)"' ]; then echo "apple<banana ✓, cat<car ✗ => 7 ; string + still rejected"; else echo "FAIL [$so2 / $so3]"; fail=1; fi
...
ssc run bin/larkc.ssc0 examples/hm-listconcat.hm > "${TMPDIR:-/tmp}/lc.coreir" 2>/dev/null
got=$(ssc run-ir "${TMPDIR:-/tmp}/lc.coreir" | tail -1)
if [ "$got" = "15" ]; then printf 'ok   %-26s => %s\n' "list ++ -> run-ir" "$got"; else printf 'FAIL %-26s got [%s]\n' "list ++" "$got"; fail=1; fi
if command -v node >/dev/null 2>&1; then ssc run bin/lark-js.ssc0 examples/hm-listconcat.hm > "${TMPDIR:-/tmp}/lc.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/lc.js" 2>/dev/null | tail -1); if [ "$got" = "15" ]; then printf 'ok   %-26s => %s (node)\n' "list ++ -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "list ++ JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/lark-rust.ssc0 examples/hm-listconcat.hm > "${TMPDIR:-/tmp}/lc.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/lc.rs" -o "${TMPDIR:-/tmp}/lc-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/lc-bin"); else got="(rustc err)"; fi; if [ "$got" = "15" ]; then printf 'ok   %-26s => %s (rustc)\n' "list ++ -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "list ++ Rust" "$got"; fail=1; fi; fi
echo -n "ok   list ++ shape + string ++ both => "; printf '[1, 2] ++ [3]' > "${TMPDIR:-/tmp}/lc2.hm"; ssc run bin/larkc.ssc0 "${TMPDIR:-/tmp}/lc2.hm" 2>/dev/null > "${TMPDIR:-/tmp}/lc2.coreir"; lc2=$(ssc run-ir "${TMPDIR:-/tmp}/lc2.coreir" | tail -1); printf '"foo" ++ "bar"' > "${TMPDIR:-/tmp}/lc3.hm"; ssc run bin/larkc.ssc0 "${TMPDIR:-/tmp}/lc3.hm" 2>/dev/null > "${TMPDIR:-/tmp}/lc3.coreir"; lc3=$(ssc run-ir "${TMPDIR:-/tmp}/lc3.coreir" | tail -1); if [ "$lc2" = "Cons(1, Cons(2, Cons(3, Nil)))" ] && [ "$lc3" = '"foobar"' ]; then echo "[1,2]++[3]=list ; \"foo\"++\"bar\"=foobar (one ++, both types)"; else echo "FAIL [$lc2 / $lc3]"; fail=1; fi
...
ssc run bin/larkc.ssc0 examples/hm-letdestr.hm > "${TMPDIR:-/tmp}/ld.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then ssc run bin/lark-js.ssc0 examples/hm-letdestr.hm > "${TMPDIR:-/tmp}/ld.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/ld.js" 2>/dev/null | tail -1); if [ "$got" = "25" ]; then printf 'ok   %-26s => %s (node)\n' "let destructure -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "let destructure JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/lark-rust.ssc0 examples/hm-letdestr.hm > "${TMPDIR:-/tmp}/ld.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/ld.rs" -o "${TMPDIR:-/tmp}/ld-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/ld-bin"); else got="(rustc err)"; fi; if [ "$got" = "25" ]; then printf 'ok   %-26s => %s (rustc)\n' "let destructure -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "let destructure Rust" "$got"; fail=1; fi; fi
echo -n "ok   destructure fn-result + plain => "; printf 'let swap = fun p => match p { (x, y) => (y, x) } in let (a, b) = swap (1, 2) in a * 10 + b' > "${TMPDIR:-/tmp}/ld2.hm"; ssc run bin/larkc.ssc0 "${TMPDIR:-/tmp}/ld2.hm" 2>/dev/null > "${TMPDIR:-/tmp}/ld2.coreir"; ld2=$(ssc run-ir "${TMPDIR:-/tmp}/ld2.coreir" | tail -1); printf 'let x = 5 in x + 1' > "${TMPDIR:-/tmp}/ld3.hm"; ssc run bin/larkc.ssc0 "${TMPDIR:-/tmp}/ld3.hm" 2>/dev/null > "${TMPDIR:-/tmp}/ld3.coreir"; ld3=$(ssc run-ir "${TMPDIR:-/tmp}/ld3.coreir" | tail -1); if [ "$ld2" = "21" ] && [ "$ld3" = "6" ]; then echo "swap then (a,b) => 21 ; plain let x= still works (=6)"; else echo "FAIL [$ld2 / $ld3]"; fail=1; fi
...
ssc run bin/larkc.ssc0 examples/hm-negpat.hm > "${TMPDIR:-/tmp}/np.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then ssc run bin/lark-js.ssc0 examples/hm-negpat.hm > "${TMPDIR:-/tmp}/np.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/np.js" 2>/dev/null | tail -1); if [ "$got" = "601" ]; then printf 'ok   %-26s => %s (node)\n' "neg-lit pattern -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "neg-lit pattern JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/lark-rust.ssc0 examples/hm-negpat.hm > "${TMPDIR:-/tmp}/np.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/np.rs" -o "${TMPDIR:-/tmp}/np-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/np-bin"); else got="(rustc err)"; fi; if [ "$got" = "601" ]; then printf 'ok   %-26s => %s (rustc)\n' "neg-lit pattern -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "neg-lit pattern Rust" "$got"; fail=1; fi; fi
echo -n "ok   float pattern + neg-no-match  => "; printf 'match 2.5 { 1.5 => 0 | 2.5 => 42 | _ => 9 }' > "${TMPDIR:-/tmp}/lp2.hm"; ssc run bin/larkc.ssc0 "${TMPDIR:-/tmp}/lp2.hm" 2>/dev/null > "${TMPDIR:-/tmp}/lp2.coreir"; lp2=$(ssc run-ir "${TMPDIR:-/tmp}/lp2.coreir" | tail -1); printf 'match 5 { -1 => 7 | _ => 0 }' > "${TMPDIR:-/tmp}/lp3.hm"; ssc run bin/larkc.ssc0 "${TMPDIR:-/tmp}/lp3.hm" 2>/dev/null > "${TMPDIR:-/tmp}/lp3.coreir"; lp3=$(ssc run-ir "${TMPDIR:-/tmp}/lp3.coreir" | tail -1); if [ "$lp2" = "42" ] && [ "$lp3" = "0" ]; then echo "float 2.5=>42 ; -1 vs 5 falls through =>0"; else echo "FAIL [$lp2 / $lp3]"; fail=1; fi
...
ssc run bin/larkc.ssc0 examples/hm-listpat.hm > "${TMPDIR:-/tmp}/lpt.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then ssc run bin/lark-js.ssc0 examples/hm-listpat.hm > "${TMPDIR:-/tmp}/lpt.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/lpt.js" 2>/dev/null | tail -1); if [ "$got" = "21" ]; then printf 'ok   %-26s => %s (node)\n' "list pattern -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "list pattern JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/lark-rust.ssc0 examples/hm-listpat.hm > "${TMPDIR:-/tmp}/lpt.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/lpt.rs" -o "${TMPDIR:-/tmp}/lpt-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/lpt-bin"); else got="(rustc err)"; fi; if [ "$got" = "21" ]; then printf 'ok   %-26s => %s (rustc)\n' "list pattern -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "list pattern Rust" "$got"; fail=1; fi; fi
echo -n "ok   list-pat shapes + empty       => "; printf 'match [3, 4, 5] { [a, b] => 0 | [a, b, c] => a + b + c | _ => 9 }' > "${TMPDIR:-/tmp}/lpt2.hm"; ssc run bin/larkc.ssc0 "${TMPDIR:-/tmp}/lpt2.hm" 2>/dev/null > "${TMPDIR:-/tmp}/lpt2.coreir"; l2=$(ssc run-ir "${TMPDIR:-/tmp}/lpt2.coreir" | tail -1); printf 'match ([] : [Int]) { [] => 7 | _ => 0 }' > "${TMPDIR:-/tmp}/lpt3.hm"; ssc run bin/larkc.ssc0 "${TMPDIR:-/tmp}/lpt3.hm" 2>/dev/null > "${TMPDIR:-/tmp}/lpt3.coreir"; l3=$(ssc run-ir "${TMPDIR:-/tmp}/lpt3.coreir" | tail -1); if [ "$l2" = "12" ] && [ "$l3" = "7" ]; then echo "[a,b,c] picks len-3 =>12 ; [] matches empty =>7"; else echo "FAIL [$l2 / $l3]"; fail=1; fi
...
ssc run bin/larkc.ssc0 examples/hm-recdestr.hm > "${TMPDIR:-/tmp}/rdc.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then ssc run bin/lark-js.ssc0 examples/hm-recdestr.hm > "${TMPDIR:-/tmp}/rdc.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/rdc.js" 2>/dev/null | tail -1); if [ "$got" = "344" ]; then printf 'ok   %-26s => %s (node)\n' "rec destructure -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "rec destructure JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/lark-rust.ssc0 examples/hm-recdestr.hm > "${TMPDIR:-/tmp}/rdc.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/rdc.rs" -o "${TMPDIR:-/tmp}/rdc-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/rdc-bin"); else got="(rustc err)"; fi; if [ "$got" = "344" ]; then printf 'ok   %-26s => %s (rustc)\n' "rec destructure -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "rec destructure Rust" "$got"; fail=1; fi; fi
echo -n "ok   rec destr order-free + plain  => "; printf 'let {y = b, x = a} = {x = 3, y = 4} in a * 10 + b' > "${TMPDIR:-/tmp}/rd2.hm"; ssc run bin/larkc.ssc0 "${TMPDIR:-/tmp}/rd2.hm" 2>/dev/null > "${TMPDIR:-/tmp}/rd2.coreir"; rd2=$(ssc run-ir "${TMPDIR:-/tmp}/rd2.coreir" | tail -1); printf 'let r = {x = 1, y = 2} in r.x + r.y' > "${TMPDIR:-/tmp}/rd3.hm"; ssc run bin/larkc.ssc0 "${TMPDIR:-/tmp}/rd3.hm" 2>/dev/null > "${TMPDIR:-/tmp}/rd3.coreir"; rd3=$(ssc run-ir "${TMPDIR:-/tmp}/rd3.coreir" | tail -1); if [ "$rd2" = "34" ] && [ "$rd3" = "3" ]; then echo "{y=b,x=a} binds by name =>34 ; .x access still =>3"; else echo "FAIL [$rd2 / $rd3]"; fail=1; fi

echo -n "ok   parse error = clean (no crash) => "; printf 'if true then 1' > "${TMPDIR:-/tmp}/pe.hm"; pe=$(ssc run bin/lark.ssc0 "${TMPDIR:-/tmp}/pe.hm" 2>&1 | tail -1); printf 'if true then 1 else 2' > "${TMPDIR:-/tmp}/pe2.hm"; pe2=$(ssc run bin/lark.ssc0 "${TMPDIR:-/tmp}/pe2.hm" 2>&1 | tail -1); case "$pe" in *"parse error"*) peok=1;; *) peok=0;; esac; if [ "$peok" = "1" ] && [ "$pe2" = '"Int"' ]; then echo "if-no-else => clean TypeError (not a Java crash) ; valid if/else => Int"; else echo "FAIL [$pe / $pe2]"; fail=1; fi
...
ssc run bin/larkc.ssc0 examples/hm-charlit.hm > "${TMPDIR:-/tmp}/cl.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then ssc run bin/lark-js.ssc0 examples/hm-charlit.hm > "${TMPDIR:-/tmp}/cl.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/cl.js" 2>/dev/null | tail -1); if [ "$got" = "1025" ]; then printf 'ok   %-26s => %s (node)\n' "char literal -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "char literal JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/lark-rust.ssc0 examples/hm-charlit.hm > "${TMPDIR:-/tmp}/cl.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/cl.rs" -o "${TMPDIR:-/tmp}/cl-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/cl-bin"); else got="(rustc err)"; fi; if [ "$got" = "1025" ]; then printf 'ok   %-26s => %s (rustc)\n' "char literal -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "char literal Rust" "$got"; fail=1; fi; fi
echo -n "ok   char code + escape + cmp      => "; printf "('a' + 0) * 100 + '\\\\n'" > "${TMPDIR:-/tmp}/cl2.hm"; ssc run bin/larkc.ssc0 "${TMPDIR:-/tmp}/cl2.hm" 2>/dev/null > "${TMPDIR:-/tmp}/cl2.coreir"; cl2=$(ssc run-ir "${TMPDIR:-/tmp}/cl2.coreir" | tail -1); printf "(charAt \"xyz\" 1) = 'y'" > "${TMPDIR:-/tmp}/cl3.hm"; cl3=$(ssc run bin/lark.ssc0 "${TMPDIR:-/tmp}/cl3.hm" | tail -1); if [ "$cl2" = "9710" ] && [ "$cl3" = '"Bool"' ]; then echo "'a'=97,'\\n'=10 => 9710 ; charAt = 'y' typechecks Bool"; else echo "FAIL [$cl2 / $cl3]"; fail=1; fi
...
ssc run bin/larkc.ssc0 examples/hm-floatmath.hm > "${TMPDIR:-/tmp}/fm.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then ssc run bin/lark-js.ssc0 examples/hm-floatmath.hm > "${TMPDIR:-/tmp}/fm.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/fm.js" 2>/dev/null | tail -1); if [ "$got" = "1111" ]; then printf 'ok   %-26s => %s (node)\n' "float math -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "float math JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/lark-rust.ssc0 examples/hm-floatmath.hm > "${TMPDIR:-/tmp}/fm.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/fm.rs" -o "${TMPDIR:-/tmp}/fm-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/fm-bin"); else got="(rustc err)"; fi; if [ "$got" = "1111" ]; then printf 'ok   %-26s => %s (rustc)\n' "float math -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "float math Rust" "$got"; fail=1; fi; fi

echo "# TRANSCENDENTALS (mathx): exp/ln/sin/cos/tan/pow/sqrt/pi — pure lark prelude (Taylor series over + - * / + fsqrt; ln range-reduced by e). IEEE-754, so bit-identical across all 3 backends"
chk_hm examples/hm-mathx.hm '"Int"'                                  # near(exp/ln/sin/cos/pow/sqrt/tan results) summed -> 1111111
ssc run bin/larkc.ssc0 examples/hm-mathx.hm > "${TMPDIR:-/tmp}/mx.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then ssc run bin/lark-js.ssc0 examples/hm-mathx.hm > "${TMPDIR:-/tmp}/mx.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/mx.js" 2>/dev/null | tail -1); if [ "$got" = "1111111" ]; then printf 'ok   %-26s => %s (node)\n' "transcendentals -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "transcendentals JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/lark-rust.ssc0 examples/hm-mathx.hm > "${TMPDIR:-/tmp}/mx.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/mx.rs" -o "${TMPDIR:-/tmp}/mx-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/mx-bin"); else got="(rustc err)"; fi; if [ "$got" = "1111111" ]; then printf 'ok   %-26s => %s (rustc)\n' "transcendentals -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "transcendentals Rust" "$got"; fail=1; fi; fi
echo -n "ok   exp 1.0 bit-identical on 3 backends => "; printf 'exp 1.0' > "${TMPDIR:-/tmp}/mxe.hm"; ssc run bin/larkc.ssc0 "${TMPDIR:-/tmp}/mxe.hm" 2>/dev/null > "${TMPDIR:-/tmp}/mxe.coreir"; mxv=$(ssc run-ir "${TMPDIR:-/tmp}/mxe.coreir" | tail -1); mxok=1; if command -v node >/dev/null 2>&1; then ssc run bin/lark-js.ssc0 "${TMPDIR:-/tmp}/mxe.hm" > "${TMPDIR:-/tmp}/mxe.js" 2>/dev/null; [ "$(node "${TMPDIR:-/tmp}/mxe.js" 2>/dev/null | tail -1)" = "$mxv" ] || mxok=0; fi; if command -v rustc >/dev/null 2>&1; then ssc run bin/lark-rust.ssc0 "${TMPDIR:-/tmp}/mxe.hm" > "${TMPDIR:-/tmp}/mxe.rs" 2>/dev/null; rustc -O "${TMPDIR:-/tmp}/mxe.rs" -o "${TMPDIR:-/tmp}/mxe-bin" 2>/dev/null && [ "$("${TMPDIR:-/tmp}/mxe-bin")" = "$mxv" ] || mxok=0; fi; if [ "$mxok" = "1" ]; then echo "$mxv (run-ir == JS == Rust)"; else echo "FAIL diverged"; fail=1; fi
...
ssc run bin/larkc.ssc0 examples/hm-rounding.hm > "${TMPDIR:-/tmp}/rd.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then ssc run bin/lark-js.ssc0 examples/hm-rounding.hm > "${TMPDIR:-/tmp}/rd.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/rd.js" 2>/dev/null | tail -1); if [ "$got" = "1111111" ]; then printf 'ok   %-26s => %s (node)\n' "float rounding -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "float rounding JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/lark-rust.ssc0 examples/hm-rounding.hm > "${TMPDIR:-/tmp}/rd.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/rd.rs" -o "${TMPDIR:-/tmp}/rd-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/rd-bin"); else got="(rustc err)"; fi; if [ "$got" = "1111111" ]; then printf 'ok   %-26s => %s (rustc)\n' "float rounding -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "float rounding Rust" "$got"; fail=1; fi; fi
...
ssc run bin/larkc.ssc0 examples/hm-mathx2.hm > "${TMPDIR:-/tmp}/m2.coreir" 2>/dev/null
got=$(ssc run-ir "${TMPDIR:-/tmp}/m2.coreir" | tail -1)
if [ "$got" = "14" ]; then printf 'ok   %-26s => %s\n' "math library -> run-ir" "$got"; else printf 'FAIL %-26s got [%s]\n' "math library" "$got"; fail=1; fi
if command -v node >/dev/null 2>&1; then ssc run bin/lark-js.ssc0 examples/hm-mathx2.hm > "${TMPDIR:-/tmp}/m2.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/m2.js" 2>/dev/null | tail -1); if [ "$got" = "14" ]; then printf 'ok   %-26s => %s (node)\n' "math library -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "math library JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/lark-rust.ssc0 examples/hm-mathx2.hm > "${TMPDIR:-/tmp}/m2.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/m2.rs" -o "${TMPDIR:-/tmp}/m2-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/m2-bin"); else got="(rustc err)"; fi; if [ "$got" = "14" ]; then printf 'ok   %-26s => %s (rustc)\n' "math library -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "math library Rust" "$got"; fail=1; fi; fi
...
ssc run bin/larkc.ssc0 examples/hm-recpat.hm > "${TMPDIR:-/tmp}/rpc.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then ssc run bin/lark-js.ssc0 examples/hm-recpat.hm > "${TMPDIR:-/tmp}/rpc.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/rpc.js" 2>/dev/null | tail -1); if [ "$got" = "6530" ]; then printf 'ok   %-26s => %s (node)\n' "record pattern -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "record pattern JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/lark-rust.ssc0 examples/hm-recpat.hm > "${TMPDIR:-/tmp}/rpc.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/rpc.rs" -o "${TMPDIR:-/tmp}/rpc-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/rpc-bin"); else got="(rustc err)"; fi; if [ "$got" = "6530" ]; then printf 'ok   %-26s => %s (rustc)\n' "record pattern -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "record pattern Rust" "$got"; fail=1; fi; fi
echo -n "ok   record pat order-free + ctor pat => "; printf 'match {x = 3, y = 4} { {y = b, x = a} => a * 10 + b }' > "${TMPDIR:-/tmp}/rp2.hm"; ssc run bin/larkc.ssc0 "${TMPDIR:-/tmp}/rp2.hm" 2>/dev/null > "${TMPDIR:-/tmp}/rp2.coreir"; rp2=$(ssc run-ir "${TMPDIR:-/tmp}/rp2.coreir" | tail -1); printf 'match (Some 5) { Some n => n | None => 0 }' > "${TMPDIR:-/tmp}/rp3.hm"; ssc run bin/larkc.ssc0 "${TMPDIR:-/tmp}/rp3.hm" 2>/dev/null > "${TMPDIR:-/tmp}/rp3.coreir"; rp3=$(ssc run-ir "${TMPDIR:-/tmp}/rp3.coreir" | tail -1); if [ "$rp2" = "34" ] && [ "$rp3" = "5" ]; then echo "{y=b,x=a} binds by name =>34 ; ctor pattern still =>5"; else echo "FAIL [$rp2 / $rp3]"; fail=1; fi
...
ssc run bin/larkc.ssc0 examples/hm-letrecpoly.hm > "${TMPDIR:-/tmp}/lrp.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then ssc run bin/lark-js.ssc0 examples/hm-letrecpoly.hm > "${TMPDIR:-/tmp}/lrp.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/lrp.js" 2>/dev/null | tail -1); if [ "$got" = "618" ]; then printf 'ok   %-26s => %s (node)\n' "letrec poly -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "letrec poly JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/lark-rust.ssc0 examples/hm-letrecpoly.hm > "${TMPDIR:-/tmp}/lrp.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/lrp.rs" -o "${TMPDIR:-/tmp}/lrp-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/lrp-bin"); else got="(rustc err)"; fi; if [ "$got" = "618" ]; then printf 'ok   %-26s => %s (rustc)\n' "letrec poly -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "letrec poly Rust" "$got"; fail=1; fi; fi
echo -n "ok   genuine recursion still works => "; printf 'let rec f = fun n => if n = 0 then 1 else n * f (n - 1) in f 6' > "${TMPDIR:-/tmp}/lr2.hm"; ssc run bin/larkc.ssc0 "${TMPDIR:-/tmp}/lr2.hm" 2>/dev/null > "${TMPDIR:-/tmp}/lr2.coreir"; lr2=$(ssc run-ir "${TMPDIR:-/tmp}/lr2.coreir" | tail -1); printf 'let rec sum = fun l => if isNil l then 0 else head l + sum (tail l) in sum [1, 2, 3, 4, 5]' > "${TMPDIR:-/tmp}/lr3.hm"; ssc run bin/larkc.ssc0 "${TMPDIR:-/tmp}/lr3.hm" 2>/dev/null > "${TMPDIR:-/tmp}/lr3.coreir"; lr3=$(ssc run-ir "${TMPDIR:-/tmp}/lr3.coreir" | tail -1); if [ "$lr2" = "720" ] && [ "$lr3" = "15" ]; then echo "fact 6 => 720 ; sum [1..5] => 15 (self-recursive unaffected)"; else echo "FAIL [$lr2 / $lr3]"; fail=1; fi
...
ssc run bin/larkc.ssc0 examples/hm-dictpass.hm > "${TMPDIR:-/tmp}/dp.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then ssc run bin/lark-js.ssc0 examples/hm-dictpass.hm > "${TMPDIR:-/tmp}/dp.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/dp.js" 2>/dev/null | tail -1); if [ "$got" = "920" ]; then printf 'ok   %-26s => %s (node)\n' "dict-passing -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "dict-passing JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/lark-rust.ssc0 examples/hm-dictpass.hm > "${TMPDIR:-/tmp}/dp.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/dp.rs" -o "${TMPDIR:-/tmp}/dp-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/dp-bin"); else got="(rustc err)"; fi; if [ "$got" = "920" ]; then printf 'ok   %-26s => %s (rustc)\n' "dict-passing -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "dict-passing Rust" "$got"; fail=1; fi; fi
echo -n "ok   dict-fn (* op) at Int & Float => "; printf 'let rec acc = fun x => fun n => if n = 0 then x else x * acc x (n - 1) in (acc 2 3) * 100 + (if flt (fabs (acc 1.5 1 - 2.25)) 0.01 then 1 else 0)' > "${TMPDIR:-/tmp}/dp2.hm"; ssc run bin/larkc.ssc0 "${TMPDIR:-/tmp}/dp2.hm" 2>/dev/null > "${TMPDIR:-/tmp}/dp2.coreir"; dp2=$(ssc run-ir "${TMPDIR:-/tmp}/dp2.coreir" | tail -1); if [ "$dp2" = "1601" ]; then echo "acc 2 3 = 16 (*100) ; acc 1.5 1 = 2.25 (~) => 1601"; else echo "FAIL [$dp2]"; fail=1; fi
...
ssc run bin/larkc.ssc0 examples/hm-dictord.hm > "${TMPDIR:-/tmp}/do.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then ssc run bin/lark-js.ssc0 examples/hm-dictord.hm > "${TMPDIR:-/tmp}/do.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/do.js" 2>/dev/null | tail -1); if [ "$got" = "511" ]; then printf 'ok   %-26s => %s (node)\n' "dict ord -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "dict ord JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/lark-rust.ssc0 examples/hm-dictord.hm > "${TMPDIR:-/tmp}/do.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/do.rs" -o "${TMPDIR:-/tmp}/do-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/do-bin"); else got="(rustc err)"; fi; if [ "$got" = "511" ]; then printf 'ok   %-26s => %s (rustc)\n' "dict ord -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "dict ord Rust" "$got"; fail=1; fi; fi
echo -n "ok   dict-fn = at Int & String     => "; printf 'let rec countEq = fun x => fun xs => match xs { Cons h t => if x = h then 1 + countEq x t else countEq x t | Nil => 0 } in (countEq 2 [2, 3, 2, 2]) * 10 + (countEq "a" ["a", "b", "a"])' > "${TMPDIR:-/tmp}/do2.hm"; ssc run bin/larkc.ssc0 "${TMPDIR:-/tmp}/do2.hm" 2>/dev/null > "${TMPDIR:-/tmp}/do2.coreir"; do2=$(ssc run-ir "${TMPDIR:-/tmp}/do2.coreir" | tail -1); if [ "$do2" = "32" ]; then echo "countEq 2 (3 times) =3*10 ; countEq a (2 times) =2 => 32"; else echo "FAIL [$do2]"; fail=1; fi
...
ssc run bin/larkc.ssc0 examples/hm-scalarmatch.hm > "${TMPDIR:-/tmp}/sm.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then ssc run bin/lark-js.ssc0 examples/hm-scalarmatch.hm > "${TMPDIR:-/tmp}/sm.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/sm.js" 2>/dev/null | tail -1); if [ "$got" = "861" ]; then printf 'ok   %-26s => %s (node)\n' "scalar var-match -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "scalar var-match JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/lark-rust.ssc0 examples/hm-scalarmatch.hm > "${TMPDIR:-/tmp}/sm.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/sm.rs" -o "${TMPDIR:-/tmp}/sm-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/sm-bin"); else got="(rustc err)"; fi; if [ "$got" = "861" ]; then printf 'ok   %-26s => %s (rustc)\n' "scalar var-match -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "scalar var-match Rust" "$got"; fail=1; fi; fi
...
ssc run bin/larkc.ssc0 examples/hm-userdict.hm > "${TMPDIR:-/tmp}/ud.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then ssc run bin/lark-js.ssc0 examples/hm-userdict.hm > "${TMPDIR:-/tmp}/ud.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/ud.js" 2>/dev/null | tail -1); if [ "$got" = "1433" ]; then printf 'ok   %-26s => %s (node)\n' "user-class dict -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "user-class dict JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/lark-rust.ssc0 examples/hm-userdict.hm > "${TMPDIR:-/tmp}/ud.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/ud.rs" -o "${TMPDIR:-/tmp}/ud-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/ud-bin"); else got="(rustc err)"; fi; if [ "$got" = "1433" ]; then printf 'ok   %-26s => %s (rustc)\n' "user-class dict -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "user-class dict Rust" "$got"; fail=1; fi; fi
echo -n "ok   user method at Int & Bool too => "; printf 'method sz : Int in instance sz Int = fun n => 5 in instance sz Bool = fun b => 9 in let rec count = fun x => fun n => if n = 0 then 0 else sz x + count x (n - 1) in (count 0 3) * 10 + (count true 2)' > "${TMPDIR:-/tmp}/ud2.hm"; ssc run bin/larkc.ssc0 "${TMPDIR:-/tmp}/ud2.hm" 2>/dev/null > "${TMPDIR:-/tmp}/ud2.coreir"; ud2=$(ssc run-ir "${TMPDIR:-/tmp}/ud2.coreir" | tail -1); if [ "$ud2" = "168" ]; then echo "count(sz) at Int=15 (*10), Bool=18 => 168"; else echo "FAIL [$ud2]"; fail=1; fi
...
ssc run bin/larkc.ssc0 examples/hm-prelude3.hm > "${TMPDIR:-/tmp}/p3.coreir" 2>/dev/null
got=$(ssc run-ir "${TMPDIR:-/tmp}/p3.coreir" | tail -1)
if [ "$got" = "5329" ]; then printf 'ok   %-26s => %s\n' "prelude2 -> run-ir" "$got"; else printf 'FAIL %-26s got [%s]\n' "prelude2" "$got"; fail=1; fi
if command -v node >/dev/null 2>&1; then ssc run bin/lark-js.ssc0 examples/hm-prelude3.hm > "${TMPDIR:-/tmp}/p3.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/p3.js" 2>/dev/null | tail -1); if [ "$got" = "5329" ]; then printf 'ok   %-26s => %s (node)\n' "prelude2 -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "prelude2 JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/lark-rust.ssc0 examples/hm-prelude3.hm > "${TMPDIR:-/tmp}/p3.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/p3.rs" -o "${TMPDIR:-/tmp}/p3-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/p3-bin"); else got="(rustc err)"; fi; if [ "$got" = "5329" ]; then printf 'ok   %-26s => %s (rustc)\n' "prelude2 -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "prelude2 Rust" "$got"; fail=1; fi; fi
echo -n "ok   takeWhile/span/partition shape => "; printf 'match span (fun x => x < 3) [1, 2, 3, 4] { (a, b) => length a * 10 + length b }' > "${TMPDIR:-/tmp}/p32.hm"; ssc run bin/larkc.ssc0 "${TMPDIR:-/tmp}/p32.hm" 2>/dev/null > "${TMPDIR:-/tmp}/p32.coreir"; p32=$(ssc run-ir "${TMPDIR:-/tmp}/p32.coreir" | tail -1); printf 'match partition (fun x => x > 2) [1, 2, 3, 4] { (a, b) => length a * 10 + length b }' > "${TMPDIR:-/tmp}/p33.hm"; ssc run bin/larkc.ssc0 "${TMPDIR:-/tmp}/p33.hm" 2>/dev/null > "${TMPDIR:-/tmp}/p33.coreir"; p33=$(ssc run-ir "${TMPDIR:-/tmp}/p33.coreir" | tail -1); if [ "$p32" = "22" ] && [ "$p33" = "22" ]; then echo "span (<3) => (2 | 2) => 22 ; partition (>2) => (2 | 2) => 22"; else echo "FAIL [$p32 / $p33]"; fail=1; fi
...
ssc run bin/larkc.ssc0 examples/hm-strfns2.hm > "${TMPDIR:-/tmp}/sf.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then ssc run bin/lark-js.ssc0 examples/hm-strfns2.hm > "${TMPDIR:-/tmp}/sf.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/sf.js" 2>/dev/null | tail -1); if [ "$got" = "1112" ]; then printf 'ok   %-26s => %s (node)\n' "string fns 2 -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "string fns 2 JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/lark-rust.ssc0 examples/hm-strfns2.hm > "${TMPDIR:-/tmp}/sf.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/sf.rs" -o "${TMPDIR:-/tmp}/sf-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/sf-bin"); else got="(rustc err)"; fi; if [ "$got" = "1112" ]; then printf 'ok   %-26s => %s (rustc)\n' "string fns 2 -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "string fns 2 Rust" "$got"; fail=1; fi; fi
echo -n "ok   trim edges + contains miss   => "; printf 'strLen (trim "   ") * 100 + strLen (trim "  z  ") * 10 + (if strContains "q" "hello" then 1 else 0)' > "${TMPDIR:-/tmp}/sf2.hm"; ssc run bin/larkc.ssc0 "${TMPDIR:-/tmp}/sf2.hm" 2>/dev/null > "${TMPDIR:-/tmp}/sf2.coreir"; sf2=$(ssc run-ir "${TMPDIR:-/tmp}/sf2.coreir" | tail -1); if [ "$sf2" = "10" ]; then echo "trim all-spaces=0 ; trim \"  z  \"=1 (*10) ; contains miss=0 => 10"; else echo "FAIL [$sf2]"; fail=1; fi

echo -n "ok   empty match = clean error    => "; printf 'match 5 { }' > "${TMPDIR:-/tmp}/em.hm"; em=$(ssc run bin/lark.ssc0 "${TMPDIR:-/tmp}/em.hm" 2>&1 | tail -1); printf 'match 5 { 1 => 10 | _ => 0 }' > "${TMPDIR:-/tmp}/em2.hm"; ssc run bin/larkc.ssc0 "${TMPDIR:-/tmp}/em2.hm" 2>/dev/null > "${TMPDIR:-/tmp}/em2.coreir"; em2=$(ssc run-ir "${TMPDIR:-/tmp}/em2.coreir" | tail -1); case "$em" in *"empty match"*) emok=1;; *) emok=0;; esac; if [ "$emok" = "1" ] && [ "$em2" = "0" ]; then echo "empty match => clean TypeError (not a crash) ; normal match still works"; else echo "FAIL [$em / $em2]"; fail=1; fi
...
ssc run bin/larkc.ssc0 examples/hm-strbuild.hm > "${TMPDIR:-/tmp}/sb.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then ssc run bin/lark-js.ssc0 examples/hm-strbuild.hm > "${TMPDIR:-/tmp}/sb.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/sb.js" 2>/dev/null | tail -1); if [ "$got" = "662" ]; then printf 'ok   %-26s => %s (node)\n' "string build -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "string build JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/lark-rust.ssc0 examples/hm-strbuild.hm > "${TMPDIR:-/tmp}/sb.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/sb.rs" -o "${TMPDIR:-/tmp}/sb-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/sb-bin"); else got="(rustc err)"; fi; if [ "$got" = "662" ]; then printf 'ok   %-26s => %s (rustc)\n' "string build -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "string build Rust" "$got"; fail=1; fi; fi
echo -n "ok   fromCodes + chr + roundtrip   => "; printf 'strLen (fromCodes [72, 105]) * 100 + (charAt (chr 90) 0) + (if toLower (toUpper "Yo") = "yo" then 1 else 0)' > "${TMPDIR:-/tmp}/sb2.hm"; ssc run bin/larkc.ssc0 "${TMPDIR:-/tmp}/sb2.hm" 2>/dev/null > "${TMPDIR:-/tmp}/sb2.coreir"; sb2=$(ssc run-ir "${TMPDIR:-/tmp}/sb2.coreir" | tail -1); if [ "$sb2" = "291" ]; then echo "fromCodes[72,105]=\"Hi\" len 2 (*100) ; chr 90='Z'=90 ; roundtrip=1 => 291"; else echo "FAIL [$sb2]"; fail=1; fi
...
ssc run bin/larkc.ssc0 examples/hm-preludecombi.hm > "${TMPDIR:-/tmp}/pc.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then ssc run bin/lark-js.ssc0 examples/hm-preludecombi.hm > "${TMPDIR:-/tmp}/pc.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/pc.js" 2>/dev/null | tail -1); if [ "$got" = "11453" ]; then printf 'ok   %-26s => %s (node)\n' "prelude batch -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "prelude batch JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/lark-rust.ssc0 examples/hm-preludecombi.hm > "${TMPDIR:-/tmp}/pc.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/pc.rs" -o "${TMPDIR:-/tmp}/pc-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/pc-bin"); else got="(rustc err)"; fi; if [ "$got" = "11453" ]; then printf 'ok   %-26s => %s (rustc)\n' "prelude batch -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "prelude batch Rust" "$got"; fail=1; fi; fi
echo -n "ok   compose/flip/elem/notElem/join => "; printf '(compose (fun x => x + 1) (fun x => x * 2)) 5 * 100 + (flip (fun a => fun b => a - b) 3 10) * 10 + (if elem 2 [1, 2] then (if notElem 9 [1, 2] then strLen (join "-" ["x", "y"]) else 0) else 0)' > "${TMPDIR:-/tmp}/pc2.hm"; ssc run bin/larkc.ssc0 "${TMPDIR:-/tmp}/pc2.hm" 2>/dev/null > "${TMPDIR:-/tmp}/pc2.coreir"; pc2=$(ssc run-ir "${TMPDIR:-/tmp}/pc2.coreir" | tail -1); if [ "$pc2" = "1173" ]; then echo "compose=11*100 ; flip sub=7*10 ; elem&notElem -> join len 3 => 1173"; else echo "FAIL [$pc2]"; fail=1; fi
...
ssc run bin/larkc.ssc0 examples/hm-aspat.hm > "${TMPDIR:-/tmp}/apc.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then ssc run bin/lark-js.ssc0 examples/hm-aspat.hm > "${TMPDIR:-/tmp}/apc.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/apc.js" 2>/dev/null | tail -1); if [ "$got" = "4" ]; then printf 'ok   %-26s => %s (node)\n' "as-pattern -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "as-pattern JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/lark-rust.ssc0 examples/hm-aspat.hm > "${TMPDIR:-/tmp}/apc.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/apc.rs" -o "${TMPDIR:-/tmp}/apc-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/apc-bin"); else got="(rustc err)"; fi; if [ "$got" = "4" ]; then printf 'ok   %-26s => %s (rustc)\n' "as-pattern -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "as-pattern Rust" "$got"; fail=1; fi; fi
echo -n "ok   as-pattern top + literal     => "; printf 'match [1, 2, 3] { all@(Cons h t) => h + length all | Nil => 0 }' > "${TMPDIR:-/tmp}/ap2.hm"; ssc run bin/larkc.ssc0 "${TMPDIR:-/tmp}/ap2.hm" 2>/dev/null > "${TMPDIR:-/tmp}/ap2.coreir"; ap2=$(ssc run-ir "${TMPDIR:-/tmp}/ap2.coreir" | tail -1); printf 'match 5 { n@5 => n * 2 | _ => 0 }' > "${TMPDIR:-/tmp}/ap3.hm"; ssc run bin/larkc.ssc0 "${TMPDIR:-/tmp}/ap3.hm" 2>/dev/null > "${TMPDIR:-/tmp}/ap3.coreir"; ap3=$(ssc run-ir "${TMPDIR:-/tmp}/ap3.coreir" | tail -1); if [ "$ap2" = "4" ] && [ "$ap3" = "10" ]; then echo "all@(Cons h t): 1+len 3 => 4 ; n@5: 5*2 => 10"; else echo "FAIL [$ap2 / $ap3]"; fail=1; fi
...
ssc run bin/larkc.ssc0 examples/hm-opsections.hm > "${TMPDIR:-/tmp}/osc.coreir" 2>/dev/null
got=$(ssc run-ir "${TMPDIR:-/tmp}/osc.coreir" | tail -1)
if [ "$got" = "21" ]; then printf 'ok   %-26s => %s\n' "op section -> run-ir" "$got"; else printf 'FAIL %-26s got [%s]\n' "op section" "$got"; fail=1; fi
if command -v node >/dev/null 2>&1; then ssc run bin/lark-js.ssc0 examples/hm-opsections.hm > "${TMPDIR:-/tmp}/osc.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/osc.js" 2>/dev/null | tail -1); if [ "$got" = "21" ]; then printf 'ok   %-26s => %s (node)\n' "op section -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "op section JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/lark-rust.ssc0 examples/hm-opsections.hm > "${TMPDIR:-/tmp}/osc.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/osc.rs" -o "${TMPDIR:-/tmp}/osc-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/osc-bin"); else got="(rustc err)"; fi; if [ "$got" = "21" ]; then printf 'ok   %-26s => %s (rustc)\n' "op section -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "op section Rust" "$got"; fail=1; fi; fi
echo -n "ok   section + paren/neg unchanged => "; printf '(++ "!") "hi"' > "${TMPDIR:-/tmp}/os2.hm"; os2=$(ssc run bin/lark.ssc0 "${TMPDIR:-/tmp}/os2.hm" | tail -1); printf 'let x = -5 in x * (3 + 1)' > "${TMPDIR:-/tmp}/os3.hm"; ssc run bin/larkc.ssc0 "${TMPDIR:-/tmp}/os3.hm" 2>/dev/null > "${TMPDIR:-/tmp}/os3.coreir"; os3=$(ssc run-ir "${TMPDIR:-/tmp}/os3.coreir" | tail -1); if [ "$os2" = '"String"' ] && [ "$os3" = "-20" ]; then echo "(++ \"!\") : String->String ; (3+1) paren + -5 neg => -20"; else echo "FAIL [$os2 / $os3]"; fail=1; fi
...
ssc run bin/larkc.ssc0 examples/hm-lsections.hm > "${TMPDIR:-/tmp}/lsc.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then ssc run bin/lark-js.ssc0 examples/hm-lsections.hm > "${TMPDIR:-/tmp}/lsc.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/lsc.js" 2>/dev/null | tail -1); if [ "$got" = "247" ]; then printf 'ok   %-26s => %s (node)\n' "left section -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "left section JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/lark-rust.ssc0 examples/hm-lsections.hm > "${TMPDIR:-/tmp}/lsc.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/lsc.rs" -o "${TMPDIR:-/tmp}/lsc-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/lsc-bin"); else got="(rustc err)"; fi; if [ "$got" = "247" ]; then printf 'ok   %-26s => %s (rustc)\n' "left section -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "left section Rust" "$got"; fail=1; fi; fi
echo -n "ok   left/right sections + tuple/ascr coexist => "; printf '((10 -) 3) + ((+ 1) 4)' > "${TMPDIR:-/tmp}/ls2.hm"; ssc run bin/larkc.ssc0 "${TMPDIR:-/tmp}/ls2.hm" 2>/dev/null > "${TMPDIR:-/tmp}/ls2.coreir"; ls2=$(ssc run-ir "${TMPDIR:-/tmp}/ls2.coreir" | tail -1); printf '(3, 4)' > "${TMPDIR:-/tmp}/ls3.hm"; ls3=$(ssc run bin/lark.ssc0 "${TMPDIR:-/tmp}/ls3.hm" | tail -1); if [ "$ls2" = "12" ] && [ "$ls3" = '"(Int, Int)"' ]; then echo "(10 -) 3 + (+ 1) 4 => 12 ; (3,4) tuple intact"; else echo "FAIL [$ls2 / $ls3]"; fail=1; fi
...
ssc run bin/larkc.ssc0 examples/hm-stringfns.hm > "${TMPDIR:-/tmp}/sfc.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then ssc run bin/lark-js.ssc0 examples/hm-stringfns.hm > "${TMPDIR:-/tmp}/sfc.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/sfc.js" 2>/dev/null | tail -1); if [ "$got" = "431" ]; then printf 'ok   %-26s => %s (node)\n' "string fns -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "string fns JS" "$got"; fail=1; fi; fi
if command -v rustc >/dev/null 2>&1; then ssc run bin/lark-rust.ssc0 examples/hm-stringfns.hm > "${TMPDIR:-/tmp}/sfc.rs" 2>/dev/null; if rustc -O "${TMPDIR:-/tmp}/sfc.rs" -o "${TMPDIR:-/tmp}/sfc-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/sfc-bin"); else got="(rustc err)"; fi; if [ "$got" = "431" ]; then printf 'ok   %-26s => %s (rustc)\n' "string fns -> Rust" "$got"; else printf 'FAIL %-26s got [%s]\n' "string fns Rust" "$got"; fail=1; fi; fi
echo -n "ok   split by char literal + words => "; printf "length (split ',' \"a,b,c\") * 10 + length (words \"p q r s\")" > "${TMPDIR:-/tmp}/sf2.hm"; ssc run bin/larkc.ssc0 "${TMPDIR:-/tmp}/sf2.hm" 2>/dev/null > "${TMPDIR:-/tmp}/sf2.coreir"; sf2=$(ssc run-ir "${TMPDIR:-/tmp}/sf2.coreir" | tail -1); if [ "$sf2" = "34" ]; then echo "split ',' (3) *10 + words (4) => 34"; else echo "FAIL [$sf2]"; fail=1; fi

LMAP="Cons(1, Cons(4, Cons(9, Nil)))"
ssc run bin/larkc.ssc0 examples/hm-map.hm > "${TMPDIR:-/tmp}/tmap.coreir" 2>/dev/null
got=$(ssc run-ir "${TMPDIR:-/tmp}/tmap.coreir" | tail -1)
if [ "$got" = "$LMAP" ]; then printf 'ok   %-26s => %s\n' "larkc map.hm -> run-ir" "$got"; else printf 'FAIL %-26s got [%s]\n' "larkc map.hm" "$got"; fail=1; fi
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-map.hm > "${TMPDIR:-/tmp}/tmap.js" 2>/dev/null
  got=$(node "${TMPDIR:-/tmp}/tmap.js" 2>/dev/null | tail -1)
  if [ "$got" = "$LMAP" ]; then printf 'ok   %-26s => %s (node)\n' "lark-js map.hm" "$got"; else printf 'FAIL %-26s got [%s]\n' "lark-js map.hm" "$got"; fail=1; fi
fi
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-map.hm > "${TMPDIR:-/tmp}/tmap.rs" 2>/dev/null
  if rustc -O "${TMPDIR:-/tmp}/tmap.rs" -o "${TMPDIR:-/tmp}/tmap-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/tmap-bin"); else got="(rustc err)"; fi
  if [ "$got" = "$LMAP" ]; then printf 'ok   %-26s => %s (rustc)\n' "lark-rust map.hm" "$got"; else printf 'FAIL %-26s got [%s]\n' "lark-rust map.hm" "$got"; fail=1; fi
fi

echo "# lark TYPED QUICKSORT: filter/append/qsort + less-than as source text -> infer [Int] -> 3 backends"
chk_hm examples/hm-qsort.hm '"[Int]"'
QS="Cons(1, Cons(2, Cons(3, Cons(4, Nil))))"
ssc run bin/larkc.ssc0 examples/hm-qsort.hm > "${TMPDIR:-/tmp}/qs.coreir" 2>/dev/null
got=$(ssc run-ir "${TMPDIR:-/tmp}/qs.coreir" | tail -1)
if [ "$got" = "$QS" ]; then printf 'ok   %-26s => %s\n' "larkc qsort.hm -> run-ir" "$got"; else printf 'FAIL %-26s got [%s]\n' "larkc qsort.hm" "$got"; fail=1; fi
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-qsort.hm > "${TMPDIR:-/tmp}/qs.js" 2>/dev/null
  got=$(node "${TMPDIR:-/tmp}/qs.js" 2>/dev/null | tail -1)
  if [ "$got" = "$QS" ]; then printf 'ok   %-26s => %s (node)\n' "lark-js qsort.hm" "$got"; else printf 'FAIL %-26s got [%s]\n' "lark-js qsort.hm" "$got"; fail=1; fi
fi
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-qsort.hm > "${TMPDIR:-/tmp}/qs.rs" 2>/dev/null
  if rustc -O "${TMPDIR:-/tmp}/qs.rs" -o "${TMPDIR:-/tmp}/qs-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/qs-bin"); else got="(rustc err)"; fi
  if [ "$got" = "$QS" ]; then printf 'ok   %-26s => %s (rustc)\n' "lark-rust qsort.hm" "$got"; else printf 'FAIL %-26s got [%s]\n' "lark-rust qsort.hm" "$got"; fail=1; fi
fi

echo "# lark TEXTUAL ADTs: capitalized constructors + match { | } as source text -> 3 backends"
chk_hm examples/hm-adt-match.hm '"Int"'                              # match (Some 5) { Some x => x | None => 0 }
ssc run bin/larkc.ssc0 examples/hm-adt-match.hm > "${TMPDIR:-/tmp}/om.coreir" 2>/dev/null
got=$(ssc run-ir "${TMPDIR:-/tmp}/om.coreir" | tail -1)
if [ "$got" = "5" ]; then printf 'ok   %-26s => %s\n' "larkc match.hm -> run-ir" "$got"; else printf 'FAIL %-26s got [%s]\n' "larkc match.hm" "$got"; fail=1; fi
chk_hm examples/hm-adt-tree.hm  '"Int"'                              # recursive tree-sum via match
echo "# lark USER data DECLS: declare your own types in text -> type-checked + compiled"
...
ssc run bin/larkc.ssc0 examples/hm-data-shape.hm > "${TMPDIR:-/tmp}/sh.coreir" 2>/dev/null
...
ssc run bin/larkc.ssc0 examples/hm-data-tree.hm > "${TMPDIR:-/tmp}/mt.coreir" 2>/dev/null
got=$(ssc run-ir "${TMPDIR:-/tmp}/mt.coreir" | tail -1)
if [ "$got" = "6" ]; then printf 'ok   %-26s => %s\n' "data MyTree -> run-ir" "$got"; else printf 'FAIL %-26s got [%s]\n' "data MyTree" "$got"; fail=1; fi

ssc run bin/larkc.ssc0 examples/hm-adt-tree.hm > "${TMPDIR:-/tmp}/tt.coreir" 2>/dev/null
got=$(ssc run-ir "${TMPDIR:-/tmp}/tt.coreir" | tail -1)
if [ "$got" = "6" ]; then printf 'ok   %-26s => %s\n' "larkc tree.hm -> run-ir" "$got"; else printf 'FAIL %-26s got [%s]\n' "larkc tree.hm" "$got"; fail=1; fi
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-adt-tree.hm > "${TMPDIR:-/tmp}/tt.js" 2>/dev/null
  got=$(node "${TMPDIR:-/tmp}/tt.js" 2>/dev/null | tail -1)
  if [ "$got" = "6" ]; then printf 'ok   %-26s => %s (node)\n' "lark-js tree.hm" "$got"; else printf 'FAIL %-26s got [%s]\n' "lark-js tree.hm" "$got"; fail=1; fi
fi
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-adt-tree.hm > "${TMPDIR:-/tmp}/tt.rs" 2>/dev/null
  if rustc -O "${TMPDIR:-/tmp}/tt.rs" -o "${TMPDIR:-/tmp}/tt-bin" 2>/dev/null; then got=$("${TMPDIR:-/tmp}/tt-bin"); else got="(rustc err)"; fi
  if [ "$got" = "6" ]; then printf 'ok   %-26s => %s (rustc)\n' "lark-rust tree.hm" "$got"; else printf 'FAIL %-26s got [%s]\n' "lark-rust tree.hm" "$got"; fail=1; fi
fi
...
K52LV='"a"'
ssc run bin/larkc.ssc0 examples/hm-lambda.hm > "${TMPDIR:-/tmp}/lc.coreir" 2>/dev/null
got=$(ssc run-ir "${TMPDIR:-/tmp}/lc.coreir" | tail -1)
if [ "$got" = "$K52LV" ]; then printf 'ok   %-26s => %s\n' "lambda calc -> run-ir" "$got"; else printf 'FAIL %-26s got [%s]\n' "lambda-calc" "$got"; fail=1; fi
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-lambda.hm > "${TMPDIR:-/tmp}/lc.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/lc.js" 2>/dev/null | tail -1)
  if [ "$got" = "$K52LV" ]; then printf 'ok   %-26s => %s (node)\n' "lambda calc -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "lambda-calc JS" "$got"; fail=1; fi
fi
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-lambda.hm > "${TMPDIR:-/tmp}/lc.rs" 2>/dev/null
...
K52AV="7"
ssc run bin/larkc.ssc0 examples/hm-arith-parser.hm > "${TMPDIR:-/tmp}/ap.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-arith-parser.hm > "${TMPDIR:-/tmp}/ap.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/ap.js" 2>/dev/null | tail -1)
  if [ "$got" = "$K52AV" ]; then printf 'ok   %-26s => %s (node)\n' "arith parser -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "arith-parser JS" "$got"; fail=1; fi
fi
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-arith-parser.hm > "${TMPDIR:-/tmp}/ap.rs" 2>/dev/null
...
echo '# K51 — lark STDLIB EXPANSION: assoc-list map ops (assocInsert/Delete/MapKV/UnionWith) — type check + JS'
...
K51MV="30055"
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-stdlib-map.hm > "${TMPDIR:-/tmp}/k51m.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/k51m.js" 2>/dev/null | tail -1)
  if [ "$got" = "$K51MV" ]; then printf 'ok   %-26s => %s (node)\n' "stdlib-map -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "stdlib-map JS" "$got"; fail=1; fi
fi
echo '# K51 — lark STDLIB EXPANSION: parser combinators (pChar/pStr/pSeq/pAlt/pMap/pMany/pInt) — all 3 backends'
chk_hm examples/hm-parser-comb.hm '"Int"'                             # parse "3+4*2" with combinator grammar -> 11
K51PV="11"
ssc run bin/larkc.ssc0 examples/hm-parser-comb.hm > "${TMPDIR:-/tmp}/k51p.coreir" 2>/dev/null
...
if command -v node >/dev/null 2>&1; then
  ssc run bin/lark-js.ssc0 examples/hm-parser-comb.hm > "${TMPDIR:-/tmp}/k51p.js" 2>/dev/null; got=$(node "${TMPDIR:-/tmp}/k51p.js" 2>/dev/null | tail -1)
  if [ "$got" = "$K51PV" ]; then printf 'ok   %-26s => %s (node)\n' "parser-comb -> JS" "$got"; else printf 'FAIL %-26s got [%s]\n' "parser-comb JS" "$got"; fail=1; fi
fi
if command -v rustc >/dev/null 2>&1; then
  ssc run bin/lark-rust.ssc0 examples/hm-parser-comb.hm > "${TMPDIR:-/tmp}/k51p.rs" 2>/dev/null
