...
| **C.3 — DataFrame ergonomics + schema bridge** | Six landed slices so far: (1) `>10` binds via `Map.ofEntries[String, Object]` once `SparkGen.MapOfMaxPairs = 10` is exceeded; (2) widen the emitted `sparkImports` to include `Row`, `DataFrame`, and `types._`; (3) `spark-config:` front-matter map → sorted `.config(k, v)` lines on `SparkSession.builder()`; (4) `spark-app-name:` front-matter overrides `.appName(...)`; (5)+(6) typed reader convenience shims `Dataset.{fromParquet,fromJson,fromCsv}(path, options: (String, String)*): DataFrame` — variadic option pairs cover the common-case `header=true`, `inferSchema=true`, `multiLine=true` knobs inline, while 0-options collapses to the bare reader.  Still open: map `std/parsing` schemas to Spark `StructType`. | C.1+C.2 ship the binding/runtime contract; C.3 polishes the typed surface. | partially landed (slices 1–6) |
