 ...
 *  signature** (`encode(R) ‖ scalarLE(z)`, 64 bytes) verifiable by any RFC 8032 verifier against the group
...
object FrostSign:
  ...
  import Ed25519Group.{L, mulBase, mul, add, encode, Point}
  ...
  def round1(id: Int, rng: SecureRandom = new SecureRandom()): (Nonce, Commitment) =
    ...
    (Nonce(d, e), Commitment(id, encode(mulBase(d)), encode(mulBase(e))))

  private def sha512(parts: Array[Byte]*): Array[Byte] =
  ...
  def groupCommitment(msg: Array[Byte], commitments: List[Commitment]): Point =
    commitments.foldLeft(Ed25519Group.Identity) { (acc, c) =>
      val di = Ed25519Group.decode(c.D).getOrElse(sys.error(s"bad D for signer ${c.id}"))
      val ei = Ed25519Group.decode(c.E).getOrElse(sys.error(s"bad E for signer ${c.id}"))
      val rho = bindingFactor(c.id, msg, commitments)
      add(acc, add(di, mul(rho, ei)))
    }

  /** Ed25519 challenge `c = SHA-512(encode(R) ‖ groupPublicKey ‖ msg) mod L`. */
  ...
  def partialSign(
    ...
    val c   = challenge(encode(r), groupPublicKey, msg)
  ...
  /** Aggregate the partials into a standard 64-byte Ed25519 signature `encode(R) ‖ scalarLE(z)`. */
  def aggregate(
    ...
    encode(r) ++ Ed25519Group.toLE(z, 32)
