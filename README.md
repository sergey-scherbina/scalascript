...
| Scala 3 control API (explicit Tier 1 + lexical direct M1 implemented) | Publication-ready `_3` leaf in `scalascript.control`: typed `Eff`, deep handlers, generative multi-prompt `shift`/`reset`, reusable and one-shot local continuations, stackless state machines, and bounded `direct.reset`/`direct.shift` macros that lower only to that explicit protocol. Local `save()` rejects with typed `UnmanagedCapture`; see the [runnable Scala example](v2/host/scala/control/src/test/scala/scalascript/controlapi/examples/ControlApiExample.scala), [macro contract](specs/scala3-control-macros.md), and [Scala/JVM profile](specs/scala3-bidirectional-control.md) |
| JavaScript/TypeScript explicit control API (first slice implemented) | ESM-only `@scalascript/control` leaf in [`v2/host/js/control`](v2/host/js/control): zero-production-dependency iterative `Eff`, deep handlers, reusable/one-shot resumptions, named generative effect owners, true `shift`/`reset`, opaque runtime authority, branded `.d.ts`, and an Apache-licensed five-file tarball. Its 31 package tests cover all 17 currently specified local semantic vectors plus 1,000,000-bind/state and 100,000-operation stress gates; generated facades, bridges, durable runners, and lane qualification remain open |
| JavaScript/TypeScript lexical direct control (T1 fourth repair implemented; rereview pending) | Zero-production-dependency `@scalascript/control-direct` package in [`v2/host/js/control-direct`](v2/host/js/control-direct): a TypeScript 5.9 consumer-owned compiler transform plus real packed `ssc-control-tsc` bin, lowering closed synchronous `direct.reset` blocks and top-level `const`/`let` shift bindings to the one explicit ABI. Its exact eight-file manifest is self-contained—only the registry TypeScript 5.9.3 dev pin, never a repository-local sibling—and is clean-extraction install tested. JavaScript emit removes completed marker/type-only edges while declaration emit retains types; runtime CommonJS `import = require` is rejected while `import type = require` erases. Checker-resolved ownership also rejects suffix capture, surviving values, direct eval, and unmanaged frames. Package-local differentials cover vectors 18/22/23/24 without claiming the pending generated-JS lane. See the [bounded contract](specs/javascript-typescript-control-direct.md) |
| Shared control semantic vectors (implemented) | One [validated catalog and lane matrix](tests/interop-conformance/README.md) carries 26 stable law vectors across 9 declared lanes. Portable VM and direct ASM each pass 13/13 eligible process vectors; the explicit Scala API passes 17 semantic vectors plus coverage, and the lexical direct lane passes vectors 18/23 plus coverage. Vector 22 additionally requires prompt forwarding through a nested different-prompt reset and remains explicit/plugin evidence; unqualified generated host and AOT lanes remain explicitly `PENDING` |
| Full control interoperability (in progress) | One target-neutral contract in [`specs/control-interoperability.md`](specs/control-interoperability.md). The bounded Scala lexical macro M1 is implemented; successful durable `save`/`run`, network transfer, Scala↔ScalaScript typed bridges, managed callbacks/TCO, compiler plugins, admission, and exact/portable runners remain post-X1 work |
...
| Host/runner profiles (in progress) | The JS/TS explicit local leaf is landed and the bounded lexical direct-transform repair is at pre-integration rereview; native typed bidirectional bridges and remaining host/runner slices for [Scala/JVM](specs/scala3-bidirectional-control.md), [JS/TS](specs/javascript-typescript-bidirectional-control.md), [Rust](specs/rust-bidirectional-control.md), and [Swift](specs/swift-bidirectional-control.md) stay mandatory, measured against the [portable-VM reference runner](specs/control-interop-profile-portable-vm.md), plus the [WASM/WASI runner](specs/wasm-wasi-control-runner.md) |
...
| NFC NDEF (`std.nfc`) | `nfcCapabilities`, `nfcPermissionStatus`, `readNdef`, `writeNdef`, `textRecord`, `uriRecord`, `mimeRecord`; in the standard graph since 2026-07-31, so plain `ssc` supplies the native VM/ASM host provider and its deterministic unsupported-hardware snapshot; Android/iOS/Web adapters remain capability-gated |
