...
## jsgen-emitjs-effect-handler — `fixed` (direct + single-import) / transitive multi-level still `open` (2026-06-22)
...
- **STILL OPEN — transitive multi-level imports (3+ levels).** When the effect, the effectful reader, and the handler are split across a chain (busi: `ledger.ssc` defines `accountBalance` → calls `query` in `journal.ssc` → performs `Journal`), the per-module `analyzeEffects` runs before that module's own imports are processed, so a function calling a *transitively*-imported effectful function isn't marked effectful → not CPS'd → its Free value leaks downstream (busi: into `formatMoney`'s `.currency` dispatch). The fix is a **whole-program / import-order-aware effect pre-pass** (resolve the import graph depth-first, seed each module's `analyzeEffects` with its imports' effect sets, share the result across child generators) — a larger, higher-risk change deferred to a focused effort. Until then busi's full-domain standalone bundle stays on the JIT path; offline *read* (lf-read) needs none of this.
