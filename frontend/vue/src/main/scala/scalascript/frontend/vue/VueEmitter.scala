...
private[vue] object VueEmitter:
  ...
  def emit(root: View[?]): String =
    ...
    val watchImport     = if fetchSigs.nonEmpty || dataTableSigs.nonEmpty || seedSignals.nonEmpty then ", watch" else ""
    ...
    seedSignals.foreach { seed =>
      sb ++= s"    const ${seedPristineName(seed.id)} = ref(true);\n"
    }
    ...
    seedSignals.foreach { seed =>
      sb ++= s"    watch(${seed.source.id}, (v) => { if (${seedPristineName(seed.id)}.value) ${seed.id}.value = v; });\n"
    }
    ...
    val allNames = signals.keys.toList ++ seedSignals.map(seed => seedPristineName(seed.id)).toList ++
      lists.keys.toList ++ dataTableSigs.map(_.id) ++ fetchSigNames ++ refs.toList
    val returned =
  ...
  private def renderView(view: View[?], itemCtx: Option[ReactiveSignalList[?]] = None, modelItemVar: String = ""): String = view match
    ...
    case View.TextInput(value, placeholder, multiline, secure, style) =>
      ...
      val css  = StyleUtils.mergeCSS(base, StyleUtils.styleToCSS(style))
      if multiline then
        s"h('textarea', { style: ${jsString(css)}, 'value': this.${value.id}, 'onInput': (e) => { ${setSignalStmt(value, "e.target.value")} }, 'placeholder': ${jsString(placeholder)} })"
      else
        val typeAttr = if secure then "'type': 'password'" else "'type': 'text'"
        s"h('input', { style: ${jsString(css)}, $typeAttr, 'value': this.${value.id}, 'onInput': (e) => { ${setSignalStmt(value, "e.target.value")} }, 'placeholder': ${jsString(placeholder)} })"
    ...
    case View.Picker(options, selected, placeholder, style) =>
      ...
      s"h('select', { style: ${jsString(css)}, 'value': this.${selected.id}, 'onChange': (e) => { ${setSignalStmt(selected, "e.target.value")} } }, [${phOpt.mkString(", ")}])"
    ...
    case View.FormField(label, value, validate, style) =>
      ...
      s"h('label', { style: ${jsString(labelCss)} }, [${jsString(label)}, h('input', { 'type': $inputType, style: ${jsString(inputCss)}, 'value': this.${value.id}, 'onInput': (e) => { ${setSignalStmt(value, "e.target.value")} } })$errorJs])"
  ...
  private def eventHandlerJs(
    ...
    handler match
      case EventHandler.SetSignalLiteral(signal, value) =>
        ...
        Some(s"${jsString(onKey)}: () => { ${setSignalStmt(signal, v)} }")
      ...
      case EventHandler.InputChange(signal) =>
        Some(s"'onInput': (e) => { ${setSignalStmt(signal, "e.target.value")} }")
      ...
      case EventHandler.SetFieldToSignal(signal, fieldPath) if modelItemVar.nonEmpty =>
        Some(s"${jsString(onKey)}: () => { ${setSignalStmt(signal, s"String($modelItemVar.$fieldPath)")} }")
  ...
  private def seedDirtyStmt(signal: ReactiveSignal[?]): String =
    signal match
      case _: SeedSignal => s"this.${seedPristineName(signal.id)} = false; "
      case _             => ""

  private def setSignalStmt(signal: ReactiveSignal[?], valueJs: String): String =
    s"${seedDirtyStmt(signal)}this.${signal.id} = $valueJs;"
