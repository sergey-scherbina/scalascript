...
| Scala 3 explicit control API (Tier 1 implemented) | Publication-ready `_3` leaf in `scalascript.control`: typed `Eff`, deep handlers, generative multi-prompt `shift`/`reset`, reusable and one-shot local continuations, and stackless state machines. Local `save()` rejects with typed `UnmanagedCapture`; see the [runnable Scala example](v2/host/scala/control-api/src/test/scala/scalascript/controlapi/examples/ControlApiExample.scala) and [Scala/JVM profile](specs/scala3-bidirectional-control.md) |
