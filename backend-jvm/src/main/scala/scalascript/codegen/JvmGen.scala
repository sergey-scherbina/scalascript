...
class JvmGen(
   ...
   *
   *  Phase 3 (Option A from docs/runtime-server-strategic-plan.md) is
   *  in progress — the REST half migrated from a `"""|..."""` string
   *  template into `runtime-server-jvm/RestRuntime.scala` (3b ✓);
   *  remaining content (WS, proxy, TLS, outbound clients) still lives
   *  in `serveRuntimePart2` below until phases 3c–3e move them.
   *
   *  `lazy` to break the forward-reference to `serveRuntimePart2`
   *  declared below — a plain `val` initialises in source order, so
   *  the half is still null when this line runs and the emit gets
   *  garbage. */
  private lazy val serveRuntime: String =
    ...
    jvmHeader + loadJvmRuntimeSource("RestRuntime") + serveRuntimePart2
