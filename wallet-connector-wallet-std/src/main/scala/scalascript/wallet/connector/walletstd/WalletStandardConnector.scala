...
import scalascript.blockchain.solana.{Base58, SolanaInstruction, SolanaMessage, SolanaTx}
 ...
 *  pure JVM. The Scala.js port wires the same handler to
 *  `window.standard.wallets.registerWallet` once the cross-compile
 *  toolchain is on; until then this lets dApps integrate against
 *  the wallet over any JSON transport (gRPC, local Unix socket,
 *  bare HTTP) and exercises the codepath end-to-end.
 *
...
class WalletStandardConnector(
  ...
  import WalletStandardConnector.*
  ...
  private def handleSignTransaction(input: ujson.Value): Future[ujson.Value] =
    ...
    forActiveSolana(account) { (strategy, adapter) =>
      val tx = SolanaTx(message)
      strategy.signTransaction(adapter)(tx.asInstanceOf[adapter.Tx]).map { signed =>
        val rawBase64 = signed.asInstanceOf[scalascript.blockchain.solana.SolanaSignedTx].rawBase64
        ujson.Obj(
          "signedTransaction" -> ujson.Str(rawBase64),
        )
      }
    }
...
object WalletStandardConnector:
  ...
  private[walletstd] case class WsError(code: Int, message: String) extends RuntimeException(message)
  ...
  private final class ByteReader(buf: Array[Byte]):
    ...
    def readBytes(n: Int): Array[Byte] =
      ...
      val out = java.util.Arrays.copyOfRange(buf, pos, pos + n)
      pos += n
      out
