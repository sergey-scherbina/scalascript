...
## select-from-signal — reactive-options variant of std/ui `select` (2026-07-13)

**Why.** `BACKLOG.md` § `select-from-signal`: busi wants a `<select>` dropdown
whose `<option>` list tracks a `Signal[List[T]]` (e.g. a fetched list of
active contracts) instead of the static `List[(String,String)]` the base
`select()` (`specs/std-ui-select.md`, `84187250b`) takes. Busi's own
`forKeyed`/`KeyedForNode` already has real fine-grained reactive DOM
reconciliation for exactly this list-driven-by-a-signal shape (verified: the
JS runtime's `_ssc_ui_forKeyedView` + `_mountKeyed` in
`v1/runtime/backend/js/src/main/resources/scalascript/js-runtime/signals.mjs`,
proven live by `JsRuntimeKeyedForTest.scala` — reorders/removes/inserts while
preserving row identity). This slice reuses that ALGORITHM (key-based
reconcile: existing-by-key map, keep-or-create, append-in-order, remove
stale) for `<select>`'s `<option>` children specifically.

**Key finding (changes the design vs. naive reuse):** `<select>`'s HTML
content model only allows `<option>`/`<optgroup>`/`<hr>` as children — the
generic `_ForKeyed` mechanism wraps each row in a `<span data-ssc-key
style="display:contents">`, which the browser's "in select" insertion mode
silently drops (parse error, ignored) when nested inside `<select>`, losing
the key attribute. So `<option>`s can't be wrapped; the key must live
directly on the `<option>` element, and the reconcile container must be the
`<select>` tag itself (only place a marker attribute can legally live). This
means a NEW dedicated View-level case (`_SelectFrom`, mirroring the existing
`_DataTableView`/`dataTableView`) that owns the whole `<select>` element's
render+reconcile, rather than composing `forKeyedView` as a child inside a
generic `element("select", ...)` call. Two backends only: interpreter
snapshot fallback (parity with `forKeyedView`'s own "int" lane) + the real
JS runtime (`signals.mjs`) — JVM/Swift native backends out of scope (no
existing test exercises `select`/`selectFrom` there; `backends: [int, js]`
matches the base slice's own precedent).

**Plan:**
- [ ] spec-update commit: amend `specs/std-ui-select.md` with the
      `selectFrom` interface + design (this must land BEFORE code, per
      AGENTS.md).
- [ ] `runtime/std/ui/primitives.ssc`: `extern def selectFromView[A](items:
      Signal[List[A]], key: A => String, optionFn: A => (String,String),
      selected: Signal[String], style: String, placeholder: String,
      disabled: Boolean): View`.
- [ ] `runtime/std/ui/nodes.ssc`: `SelectFromNode[A]` case class.
- [ ] `runtime/std/ui/input.ssc`: `selectFrom[A](...): TkNode` public ctor.
- [ ] `runtime/std/ui/lower.ssc`: `case SelectFromNode(...) =>` (mirrors
      `SelectNode`'s style-computation, delegates option-list+select-shell
      to `selectFromView`).
- [ ] `v1/runtime/std/frontend-plugin/.../FrontendIntrinsics.scala`: native
      interpreter fallback for `selectFromView` (snapshot rows once, mirrors
      `forKeyedView`'s existing fallback).
- [ ] `v1/runtime/backend/js/src/main/resources/scalascript/js-runtime/signals.mjs`:
      `_ssc_ui_selectFromView` ctor + `_SelectFrom` case in `walk()`
      (renders `<select data-ssc-forkeyed-options="seq">` + keyed
      `<option data-ssc-key>` children directly, no wrapper) + a
      `_mountSelectFrom` dispatch branch inside `_mountKeyed` (reconcile:
      existing-by-key on `select.children`, create/keep/remove, then
      `select.value = curSel`).
- [ ] `v1/runtime/backend/js/src/main/scala/scalascript/codegen/JsGen.scala`:
      add `"selectFrom"` to the `hasUiHelpers` substring list (capability
      detection for import-only usage — cheap, matches precedent).
- [ ] `examples/frontend/` demo: N options, a button that appends one more
      to the source signal, proving N+1 re-render — built via `emit-js`
      (NOT `bin/ssc-tools run`/serve, which snapshots via the interpreter
      fallback and is NOT reactive — confirmed by reading `serve`'s native
      impl: it evaluates the View tree once via the interpreter BEFORE
      handing it to the custom/StaticJsEmitter backend).
- [ ] `tests/conformance/tkv2-select-reactive.ssc` (+ expected) — shallow
      API-level check on `[int, js]`, mirrors `tkv2-select.ssc`.
- [ ] A REAL proof test mirroring `JsRuntimeKeyedForTest.scala` (hand-rolled
      DOM mock + real `signals.mjs` runtime + real Node execution): initial
      N options render, `.set()` an appended list, assert N+1 `<option>`s
      with original DOM node identity preserved for kept items. This is the
      actual "not just compiles" proof requested.
- [ ] Docs: README capabilities table, docs/user-guide.md §17.5, spec
      Results section.
- [ ] BACKLOG.md: close `select-from-signal`. CHANGELOG.md: new entry.

Worktree: `scalascript-wt-select-from-signal` / `feature/select-from-signal`.
