...
| [Electron JVM REST Backend](docs/electron-jvm-rest-backend.md) | Partially implemented: explicit `ssc run --frontend electron --backend jvm-rest` and plain `ssc run` for `frontend: electron` full-stack sources; distributed client/server modes remain planned |
