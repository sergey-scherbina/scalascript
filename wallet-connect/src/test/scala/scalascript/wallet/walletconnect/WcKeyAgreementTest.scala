...
class WcKeyAgreementTest extends AnyFunSuite:
  ...
  test("topicFromSymKey returns sha256(symKey)") {
    ...
    // Cross-check against java.security MessageDigest, which is
    // distinct from BC's SHA256Digest used inside the impl.
    val md = java.security.MessageDigest.getInstance("SHA-256")
    val expected = md.digest(symKey)
    assert(topic.toSeq == expected.toSeq)
  }
