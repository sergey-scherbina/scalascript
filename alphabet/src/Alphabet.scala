package scalascript.alphabet

/** THE LEXICAL ALPHABET, IN ONE COPY, compiled into both v3's kernel and UniML.
  *
  * It is a SOURCE directory that two builds include, not a library either one depends on, and that
  * distinction is the whole design:
  *
  *   - v3's kernel keeps invariant I-1 — zero external dependencies. `v3/ssc3` compiles
  *     `v3/src` and `alphabet/src` together, so the kernel is still "compile these repo files",
  *     which is what self-hosting has to remain.
  *   - UniML's `core` adds it as an unmanaged source directory, so it is in UniML's own jars and
  *     ships wherever UniML ships, including the Scala.js lane.
  *
  * WHY IT MUST BE ONE COPY. `Chars.isUpperStart` and `UniAlphabet.isTypeNameStart` decide the same
  * question — does this name start a CONSTRUCTOR or bind? — for the two fronts of one language. Two
  * tables answering that is the duplicated-helper shape this repository keeps paying for, and it
  * was two tables for a day: v3 carried a hand-copied 606 ranges with a gate sweeping the BMP to
  * catch drift. A gate that watches for drift is worth having; not being able to drift is better.
  *
  * WHY NOT THE HOST. `v3/specs/20-core-language.md` §3 bans host character classification, and
  * UniML's own `UniAlphabet` says why at length: route an alphabet through the host and the same
  * source lexes differently on different runtimes. The measured half of that is case FOLDING, not
  * classification — `String.toLowerCase()` uses the JVM's default locale and mis-folds `<LI>` under
  * `-Duser.language=tr`, proved in the locale by `MdLocaleIndependenceSpec`. For CLASSIFICATION the
  * divergence needs a different runtime and none has been observed, which is worth stating plainly
  * rather than overselling: the rule is kept because a compiler's alphabet should be a property of
  * the language, not of where it happens to run.
  */
object Alphabet:

  /** The five whitespace characters, and no others. */
  def isWhitespace(c: Char): Boolean =
    c == ' ' || c == '\t' || c == '\n' || c == '\r' || c == '\f'

  def isDigit(c: Char): Boolean = c >= '0' && c <= '9'
  def isOctDigit(c: Char): Boolean = c >= '0' && c <= '7'
  def isHexDigit(c: Char): Boolean =
    isDigit(c) || (c >= 'a' && c <= 'f') || (c >= 'A' && c <= 'F')

  def isAsciiUpper(c: Char): Boolean = c >= 'A' && c <= 'Z'

  /** Does this character start an UPPERCASE name — a constructor rather than a binder?
    *
    * The one class that could not be decided away into a range comparison. `A`–`Z` plus the ranges
    * below; the alternative was "`A`–`Z` or anything ≥ U+0080", one comparison and no table, which
    * makes `case имя =>` a constructor and stops Cyrillic and Greek names binding in patterns. A
    * language that cannot bind a Russian name is worse than 606 ranges of data. */
  def isUpperStart(c: Char): Boolean =
    isAsciiUpper(c) || (c >= 128 && inUpperRanges(c.toInt))
  ...
  /** Unicode uppercase as SORTED RANGE PAIRS. The only table in the alphabet, and now the only copy
    * of it in the repository — `v3/toolchain-gate.sh` asserts that. */
