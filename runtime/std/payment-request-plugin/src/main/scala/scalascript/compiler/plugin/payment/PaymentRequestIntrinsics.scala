...
object PaymentRequestIntrinsics:
  ...
  val table: Map[QualifiedName, IntrinsicImpl] = Map(
    ...
    QualifiedName("PaymentRequest") -> NativeImpl((_, args) =>
      ...
      val fields = args match
        case List(methods, total, items, options, shippingOpts) =>
          Map("methods" -> methods, "total" -> total, "items" -> items,
              "options" -> options, "shippingOptions" -> shippingOpts)
        case List(methods, total, items) =>
          Map("methods" -> methods, "total" -> total, "items" -> items)
        case List(methods, total) =>
          Map("methods" -> methods, "total" -> total)
    ...
    QualifiedName("PaymentRequest.canMakePayment") -> NativeImpl((_, _) =>
      // Interpreter always reports payment is available
      Value.FutureV(scala.concurrent.Future.successful(Value.BoolV(true)))
    ),
    ...
    QualifiedName("PaymentRequest.show") -> NativeImpl((_, _) =>
      ...
      Value.FutureV(scala.concurrent.Future.successful(response))
    ),

    // ── complete ──────────────────────────────────────────────────────────

    QualifiedName("PaymentResponse.complete") -> NativeImpl((_, _) =>
      Value.FutureV(scala.concurrent.Future.successful(Value.UnitV))
    ),
    ...
    QualifiedName("ApplePay.validateMerchant") -> NativeImpl((_, args) =>
      args match
        case List(Value.StringV(url), Value.StringV(id), Value.StringV(name), Value.StringV(domain), _, _) =>
    ...
    QualifiedName("ApplePay.decryptToken") -> NativeImpl((_, _) =>
      Value.InstanceV("ApplePayDecryptedToken", Map(
        ...
        "transactionAmount"               -> Value.InstanceV("Some", Map("value" -> Value.NumV(4999))),
    ...
    QualifiedName("PaymentMethod.Card") -> NativeImpl((_, args) =>
      val networks = args.headOption.getOrElse(Value.ListV(Nil))
      val types    = args.lift(1).getOrElse(Value.ListV(Nil))
    ...
    QualifiedName("PaymentItem") -> NativeImpl((_, args) =>
      args match
        case List(Value.StringV(label), amount) =>
          Value.InstanceV("PaymentItem", Map("label" -> Value.StringV(label), "amount" -> amount,
