...
| `ssc run --native file.ssc` | `v2-native` | Opt-in ScalaScript 2.1 path: staged self-hosted frontend → CoreIR → v2 VM, with no Scala CLI/scalac/javac process. Standard host, crypto, `std.fs`, `std.os`, typed `std.json`, outbound HTTP/`Response`, basic JDK `route`/`serve[Async]`/`stop`, named JDBC `Db.query`/`Db.execute`, and core-free UI signals/static `emit` load through the native provider SPI; advanced HTTP hooks, typed/fenced SQL, and framework UI serving remain explicit migration slices. `--native --bytecode` selects direct ASM; unresolved runtime dispatch/effects fail instead of printing `Stub`/`Op`; `--compat-frontend` explicitly selects the Scalameta bridge during migration. |
