...
ScalaScript v1.38 landed `docs/specs/payment-request.md` (516 lines) and
...
placeholder calls sit in `docs/specs/payment-request.md` with no backing code:

```scala
// docs/specs/payment-request.md:314
val result = chargeCard(decrypted)

// docs/specs/payment-request.md:331
val result = chargeCard(card)

// docs/specs/payment-request.md:388
chargeApplePay(card)

// docs/specs/payment-request.md:394
...
- Zero references to Stripe, PayPal, Braintree, Square, Adyen, Checkout.com,
  ...
  `docs/specs/modularity.md:242` explicitly marks *"Integrations with third-party
...
`docs/specs/micropayment-spi.md` (910 lines, phases 1-4 landed) is **crypto-only**:
...
**v1.53 fills this gap at the design level.**  The deliverable is `docs/specs/traditional-payments.md`
...
accepted for x402 cross-compat (see `docs/specs/x402.md`).
...
Error messages follow the format established in `docs/specs/deploy.md §12`.
...
error: STRIPE_SECRET_KEY is not set
  ...
  see: docs/specs/config-system.md §3 (secret resolvers)
```
...
- **Crypto-fiat bridge**: unify x402 (`docs/specs/x402.md`) and v1.53 under one `Settle`
...
**3 — Apple Pay completion (closes `docs/specs/payment-request.md:388`):**
```scala
...
- v1.38 Payment Request (`docs/specs/payment-request.md`) already handles wallet token
