...
## Result 6 (2026-08-12) — CLOSED. The cost was the per-call prim slot lookup

`ee53eff5d` caches the resolved `Slot` in a static field per `(arity, op)` instead of hashing the
prim's NAME on every invocation of generated bytecode. Re-measured with the ALTERNATING protocol
this repository requires on a contended host — legacy and F interleaved, several rounds, at load 11
rather than the 64 that made Result 5 suspend every timing:

| program | legacy | F | before the fix |
|---|---|---|---|
| `B` (build + open) | 30 / 22 / 32 s | 18 / 22 / 32 s | — |
| `B5` (+ one INSERT) | 58 / 31 / 40 s | **33 / 28 / 36 s** | **F: cap at 400 s** |
| `scljet-hello` | 24 / 40 / 36 s | 38 / 27 / 35 s | legacy 45 s, F >365 s |
| `scljet-jdbc` | 30 / 33 s | 25 / 37 s | F: cap at 1200 s |
| `scljet-crud` | 11 / 15 s | 15 / 25 s | 2.8× when filed |
| `scljet-unique-index` | 53 / 49 s | 61 / 55 s | F: cap at 1800 s |

**Every case completes, and F is at parity with legacy across the family** — inside a run-to-run
spread that is still ±40 % on this host, which is why the rounds are printed rather than averaged.

**So the "~3× on scljet" this entry has carried since July, and the far larger gap that appeared
once F could reach these programs at all, were one mechanism**: generated bytecode asked a
`ConcurrentHashMap` for the prim on every single invocation. The census counted 161 240 733 of those
lookups in one run of `B`; the VM resolves the same prims once at compile time and never paid it.

**Why this took as long as it did, recorded because the next perf hunt will meet the same shapes:**

- Two conclusions were published and withdrawn before this one, both from a phase marker placed
  where it could not answer the question (Result 3b, Result 4).
- The census that eventually found it was blind on its first version — wired into the slots only, so
  `Emit.arith` reported nothing and "arithmetic is not hot" was never measured.
- The biggest number in the census was NOT the cause: 161 M `__eq__` appeared identically in the
  FAST variant, which is what proved it was F0's own compilation. It became the cause only when the
  same lookup was traced to every OTHER prim as well.
- Every wall-clock reading taken at load 64 was worthless, and one of them (`B` at 21 s against
  129 s for the same build) is what forced Result 5's suspension. The alternating protocol at load
  11 is what made the answer legible.

**What is NOT claimed:** a speed-up factor. The spread here is too wide to quote one honestly, and
the useful statement is categorical — cases that could not finish inside a 400–1800 s cap now finish
in 25–61 s, at parity with the other front.