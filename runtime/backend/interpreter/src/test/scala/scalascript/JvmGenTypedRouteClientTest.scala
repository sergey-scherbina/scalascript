...
class JvmGenTypedRouteClientTest extends AnyFunSuite:
  ...
  test("JavaFX JVM codegen emits typed route client objects with same runtime as Swing") {
    ...
    assert(code.contains("_ssc_ui_backend_transport.request("))
    assert(code.contains("object Messages:"))
    assert(code.contains("""def create(input: String, headers: Map[String, String] = Map.empty, cancelToken: _SscCancelToken = null): String = _ssc_api_request[String, String]("POST", "/api/messages", input, headers, cancelToken)"""))
    assert(code.contains("""def list(headers: Map[String, String] = Map.empty, cancelToken: _SscCancelToken = null): List[String] = _ssc_api_request[Unit, List[String]]("GET", "/api/messages", (), headers, cancelToken)"""))
