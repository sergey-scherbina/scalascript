...
/** Generates a Scala 3 + Apache Flink program from a ScalaScript module.
 *
 *  Model: parallel to `KafkaStreamsGen` but targets the Flink DataStream API.
 *
 *  DStream operator mappings (spec §9.4):
 *  - `Pipeline.create` + `InMemory.source(xs)` → driver-local shim (no cluster required)
 *  - `.map(f)`          → `.map(f)` on `DataStream`
 *  - `.filter(p)`       → `.filter(p)`
 *  - `.flatMap(f)`      → `.flatMap(f)`
 *  - `.keyBy(k)`        → `.keyBy(k)`
 *  - `.combinePerKey(f)` → `.keyBy(k).reduce(f)` on `KeyedStream`
 *  - `.window(fixed(d))` → `.window(TumblingEventTimeWindows.of(Time.milliseconds(d)))`
 *  - `.merge(other)`    → `env.union(left, right)`
 *  - `.run(Backend.Flink)` → `env.execute(name)` or driver-local Seq for bounded
 *
 *  For bounded `InMemory.source` inputs the generated code uses the same
 *  driver-local `Seq[Any]` shim as SparkGen/KafkaStreamsGen.  For unbounded
 *  `Kafka.source` inputs a live Flink cluster at `flinkMaster` is used (gated
 *  by `FLINK_MASTER` env var in integration tests). */
object FlinkGen:

  val DefaultFlinkVersion: String = "1.20.1"
  val DefaultScalaVersion: String = "3.7.1"
  val DefaultFlinkMaster:  String = "local"

  def generate(
      module:      Module,
      baseDir:     Option[os.Path] = None,
      flinkMaster: String          = DefaultFlinkMaster,
      parallelism: Int             = 1,
      checkpointDir: Option[String] = None
  ): String =
    FlinkGen(baseDir, flinkMaster, parallelism, checkpointDir).genModule(module)
...
private class FlinkGen(
  ...
  def genModule(module: Module): String =
    ...
    val needsDStream  = FlinkGen.containsDStream(joinedUserSrc)

    val sb = StringBuilder()

    sb.append(s"// Generated by ScalaScript — Apache Flink $DefaultFlinkVersion, Scala $DefaultScalaVersion\n")
    sb.append(s"// To run: scala-cli run this.scala\n")
    sb.append(s"//> using scala \"$DefaultScalaVersion\"\n")
    sb.append(s"//> using dep \"org.apache.flink:flink-streaming-scala_2.12:$DefaultFlinkVersion\"\n")
    sb.append(s"//> using dep \"org.apache.flink:flink-clients:$DefaultFlinkVersion\"\n")
    sb.append(s"//> using dep \"org.apache.flink:flink-connector-kafka:$DefaultFlinkVersion\"\n")
    sb.append("\n")

    sb.append(flinkImports)
    sb.append("\n")

    sb.append("@main def runFlinkJob(): Unit =\n")
    sb.append(s"""  val _flinkMaster      = "$flinkMaster"\n""")
    sb.append(s"""  val _flinkParallelism = $parallelism\n""")
    checkpointDir.foreach { d =>
      sb.append(s"""  val _flinkCheckpointDir = "$d"\n""")
    }
    sb.append("\n")

    if needsDStream then sb.append(dstreamFlinkShim)
  ...
  private def inlineImport(path: String): List[Block] =
    ...
    if importedFiles.contains(key) then Nil
      ...
      val nested = new FlinkGen(Some(resolved / os.up), flinkMaster, parallelism, checkpointDir)
  ...
  private val flinkImports: String =
    """|import org.apache.flink.streaming.api.scala.*
       |import org.apache.flink.streaming.api.windowing.time.Time
       |import org.apache.flink.streaming.api.windowing.windows.TimeWindow
       |import org.apache.flink.streaming.api.windowing.assigners.{TumblingEventTimeWindows, SlidingEventTimeWindows, EventTimeSessionWindows, GlobalWindows}
       |import org.apache.flink.streaming.api.environment.StreamExecutionEnvironment
       |import org.apache.flink.api.common.functions.{MapFunction, FilterFunction, ReduceFunction}
       |import java.time.Duration
       |""".stripMargin

  // ── v2.1.5 — DStream Flink shim ──────────────────────────────────────────
  //
  // Emitted inside `@main def runFlinkJob()` when DStream code is detected.
  // For bounded `InMemory.source` inputs: uses driver-local `Seq[Any]` (same as
  // SparkGen/KafkaStreamsGen shims), providing identical results to Backend.Direct
  // without requiring a running Flink cluster.
  //
  // `Backend.Flink` and `Backend.Beam` are declared so user code can select
  // either backend.  For live cluster tests with a real Flink job manager, the
  // `FLINK_MASTER` env var must be set (integration tests gate on this).
  private val dstreamFlinkShim: String =
    """|
       |  // ── DStream Flink shim (v2.1.5) ─────────────────────────────────────
       |  // Full DStream pipeline DSL backed by driver-local Seq[Any] for bounded
       |  // InMemory sources.  Flink DataStream API lowering for live Kafka sources
       |  // is gated by FLINK_MASTER env var.
       |
       ...
       |    stream.run(b)
       |
       |  // Flink environment helpers — used when user code calls toDataStream()
       |  def _flinkEnv(): StreamExecutionEnvironment =
