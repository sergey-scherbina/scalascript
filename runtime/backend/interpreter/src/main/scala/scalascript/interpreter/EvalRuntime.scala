  ...
  def eval(term: Term, env: Env, interp: Interpreter): Computation =
    ...
    case Term.ApplyInfix.After_4_6_0(lhs: Term.Name, op, _, argClause)
      ...
      if argClause.values.lengthCompare(1) == 0 then
        ...
        // All-pure fast path: variable read + RHS eval + arithmetic are all Pure — no lambda alloc.
        @inline def applyInfix(lhsV: Value, rv: Value): Computation =
          interp.infix2(lhsV, baseOp, rv, env) match
            case Pure(newV) => interp.globals(lhs.value) = newV; Computation.PureUnit
            case c          => FlatMap(c, { newV => interp.globals(lhs.value) = newV; Computation.PureUnit })
        (lhsC, rhsC) match
    ...
    case Term.Function.After_4_6_0(paramClause, body) =>
      ...
      // Cache paramNames + paramTypes by ParamClause identity: the same lambda
      // AST node evaluated in a tight loop would otherwise rebuild these Lists
      // on every iteration. IdentityHashMap gives O(1) lookup by reference.
    ...
    // Cache lookup done before scrutinee eval so the Pure fast path avoids
    // allocating the flatMap lambda.
    case t: Term.Match =>
              ...
              GivenRuntime.resolveGiven(key, Nil, env, interp) match
                ...
                case None =>
                  ...
                  if tcEnd > 0 then
                    ...
                    val syntheticName = s"${typeArgStr}$$${tc}"
                    env.getOrElse(syntheticName, null)
                  else null
