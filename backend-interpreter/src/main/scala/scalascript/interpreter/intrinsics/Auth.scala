...
/** Auth intrinsics for the tree-walking interpreter (Stage 5+/D).
 *
 *  Covers CSRF, Base64-url, WebAuthn, rate-limiting, TOTP, password hashing,
 *  cookie / session config, JWT (HS256 + RS256), and OAuth2 helpers.
 *  All operations call `scalascript.server.*` directly — none require
 *  interpreter context beyond the `NativeContext` carried by `NativeImpl`. */
val AuthIntrinsics: Map[QualifiedName, IntrinsicImpl] = Map(
