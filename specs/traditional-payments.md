...
ScalaScript v1.38 landed `specs/payment-request.md` (516 lines) and
...
placeholder calls sit in `specs/payment-request.md` with no backing code:

```scala
// specs/payment-request.md:314
val result = chargeCard(decrypted)

// specs/payment-request.md:331
val result = chargeCard(card)

// specs/payment-request.md:388
chargeApplePay(card)

// specs/payment-request.md:394
...
- Zero references to Stripe, PayPal, Braintree, Square, Adyen, Checkout.com,
  ...
  `specs/modularity.md:242` explicitly marks *"Integrations with third-party
...
`specs/micropayment-spi.md` (910 lines, phases 1-4 landed) is **crypto-only**:
...
**v1.53 fills this gap at the design level.**  The deliverable is `specs/traditional-payments.md`
...
accepted for x402 cross-compat (see `specs/x402.md`).
...
Error messages follow the format established in `specs/deploy.md §12`.
...
error: STRIPE_SECRET_KEY is not set
  ...
  see: specs/config-system.md §3 (secret resolvers)
```
...
- **Crypto-fiat bridge**: unify x402 (`specs/x402.md`) and v1.53 under one `Settle`
...
**3 — Apple Pay completion (closes `specs/payment-request.md:388`):**
```scala
...
- v1.38 Payment Request (`specs/payment-request.md`) already handles wallet token
