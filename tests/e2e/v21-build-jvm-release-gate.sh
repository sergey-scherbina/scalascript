...
# `set -e` exits 1 with no output at all: the CI log just stops after the last
# "JVM artifact written to …" line, naming neither the check nor the diff. That
# silence hid three stale expectations here for days and cost a full bisect to
# recover. Every output assertion goes through this.
expect_out() {
  local name=$1 want=$2 got=$3
  if [[ $got != "$want" ]]; then
    echo "v21-build-jvm-release-gate: FAILED check '$name'" >&2
...
expect_out app "$expected" "$(PATH="$clean_path" java -jar "$sandbox/a/app.jar" -- one two)"

cp "$ROOT/examples/hello.ssc" "$sandbox/other/hello.ssc"
PATH="$clean_path" SSC_NO_CDS=1 "$ROOT/bin/ssc" build-jvm \
  "$sandbox/other/hello.ssc" -o "$sandbox/hello.jar"
expect_out hello 'Hello, World!' "$(PATH="$clean_path" java -jar "$sandbox/hello.jar")"
...
expect_out import '42' "$(PATH="$clean_path" java -jar "$sandbox/import.jar")"

cp "$FIXTURES/sql-provider.ssc" "$sandbox/other/sql-provider.ssc"
PATH="$clean_path" SSC_NO_CDS=1 "$ROOT/bin/ssc" build-jvm \
  "$sandbox/other/sql-provider.ssc" -o "$sandbox/sql.jar"
expect_out sql $'1\n7\nAda\ntrue' "$(PATH="$clean_path" java -jar "$sandbox/sql.jar")"
...
expect_out sql-fence "$sql_fence_expected" "$(PATH="$clean_path" java -jar "$sandbox/sql-fence.jar")"
...
expect_out typed-sql '1/1:Buy oat milk:true' "$(PATH="$clean_path" java -jar "$sandbox/typed-sql.jar")"
...
expect_out yaml "$yaml_expected" "$(PATH="$clean_path" java -jar "$sandbox/yaml.jar")"
...
expect_out dataset "$dataset_expected" "$(PATH="$clean_path" java -jar "$sandbox/dataset.jar")"
...
expect_out generator "$generator_expected" "$(PATH="$clean_path" java -jar "$sandbox/generator.jar")"

cp "$ROOT/examples/async-demo.ssc" "$sandbox/other/async-demo.ssc"
PATH="$clean_path" SSC_NO_CDS=1 "$ROOT/bin/ssc" build-jvm \
  "$sandbox/other/async-demo.ssc" -o "$sandbox/async.jar"
async_expected=$'6\nList(1, 4, 9, 16)\nafter delay\nList(20, 40, 60)\n56'
expect_out async "$async_expected" "$(PATH="$clean_path" java -jar "$sandbox/async.jar")"
...
expect_out actors "$actors_expected" "$(PATH="$clean_path" java -jar "$sandbox/actors.jar")"
...
expect_out typed-actors "$typed_actors_expected" "$(PATH="$clean_path" java -jar "$sandbox/typed-actors.jar")"
...
expect_out distributed-join "$distributed_join_expected" "$(PATH="$clean_path" java -jar "$sandbox/distributed-join.jar" -- \
...
expect_out distributed-log "$distributed_log_expected" "$(PATH="$clean_path" java -jar "$sandbox/distributed-log.jar" -- \
...
expect_out graph 'imports:b.ssc' "$(PATH="$clean_path" java -jar "$sandbox/graph.jar")"
...
expect_out dsl-mini-language "$dsl_mini_language_expected" "$(PATH="$clean_path" java -jar "$sandbox/dsl-mini-language.jar")"
...
expect_out custom-derives "$custom_derives_expected" "$(PATH="$clean_path" java -jar "$sandbox/custom-derives.jar")"
...
expect_out direct-syntax "$direct_syntax_expected" "$(PATH="$clean_path" java -jar "$sandbox/direct-syntax.jar")"
...
expect_out named-copy "$named_copy_expected" "$(PATH="$clean_path" java -jar "$sandbox/named-copy.jar")"
...
expect_out lenses "$lenses_expected" "$(PATH="$clean_path" java -jar "$sandbox/lenses.jar")"
...
set -e
[[ $graph_rdf_rc -ne 0 ]]
expect_out check 'Stored two books.' "$(cat "$sandbox/graph-rdf4j.out")"
