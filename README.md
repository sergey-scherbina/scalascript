...
| [Full-stack in-process transport](docs/fullstack-in-process-transport.md) | Planned, partially implemented: `BackendTransport` types, `ssc run --transport http|in-process` parsing/diagnostics, interpreter/test-harness `InProcessBackendTransport`, same-process Swing runtime, Swing `fetchAction` and `fetchTable` dispatch through the generated JVM route registry, and a no-socket Swing full-stack example exist; typed clients and broader frontend selection remain planned |
| [JVM Desktop Frontend](docs/jvm-desktop-frontend.md) | Partially implemented: `frontend-swing` backend, ServiceLoader registration, `ssc run-jvm --frontend swing`, static Swing toolkit subset emission, local signal action bridge, Swing `fetchAction` / `fetchTable` backend dispatch in the same JVM process, a no-socket full-stack example, and a `swing-plugin` skeleton for future interpreter intrinsics; typed clients and JavaFX/Compose adapters remain planned |
| [Typed route clients](docs/typed-route-clients.md) | Planned, partially implemented: `apiClients:` / `api-clients:` front-matter endpoint metadata parses into AST and JVM codegen metadata; generated callable clients are not implemented yet |
...
|---------|---------------|
| Full-stack in-process transport | `ssc run` and `ssc run-jvm` recognize `--transport http|in-process`; interpreter route tests can use `InProcessBackendTransport`; Swing now runs in the same JVM process, and Swing `fetchAction` / `fetchTable` calls can dispatch to generated JVM backend routes without opening an HTTP socket; `examples/frontend/swing-fullstack/` demonstrates the no-socket path; typed clients remain planned |
...
| Typed route clients | `apiClients:` / `api-clients:` front matter preserves endpoint method/path/request/response metadata in AST and JVM codegen. Generated typed frontend clients will later encode case class inputs, call backend routes through `BackendTransport`, and decode typed responses |
